 ...
 */
@protocol YUIChatControllerDelegate <NSObject>
 ...
 */
- (TUIMessageCellData *)chatController:(YUIChatController *)controller onNewMessage:(V2TIMMessage *)msg;
 ...
 */
- (TUIMessageCell *)chatController:(YUIChatController *)controller onShowMessageData:(TUIMessageCellData *)cellData;
...
/**
 * 【模块名称】聊天界面组件（YUIChatController）
 *
 * 【功能说明】负责实现聊天界面的 UI 组件，包括消息展示区和消息输入区。
 *
 *  YUIChatController 类用于实现聊天视图的总控制器，负责将聊天消息控制器（YUIMessageController）、信息输入控制器（YUIInputController）和更多视频进行统一控制。
 *
...
@interface YUIChatController : UIViewController
 ...
 */
@property (weak) id<YUIChatControllerDelegate> delegate;
