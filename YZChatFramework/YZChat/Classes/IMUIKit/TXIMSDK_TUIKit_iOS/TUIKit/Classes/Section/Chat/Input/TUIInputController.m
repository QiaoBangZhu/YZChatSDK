...
//    self.view.backgroundColor = [UIColor d_colorWithColorLight:TInput_Background_Color dark:TInput_Background_Color_Dark];
    self.view.backgroundColor =  [UIColor colorWithRed:244/255.0 green:246/255.0 blue:249/255.0 alpha:1.0];
    ...
    //适配iPhoneX等
    UIView* view = [[UIView alloc]initWithFrame:CGRectMake(0, TTextView_Height, self.view.frame.size.width,Bottom_SafeHeight)];
    //颜色必须和InputBar背景色一致
...
- (TUIFaceView *)faceView
{
    if(!_faceView){
        ...
        [_faceView setData:[[TUIKit sharedInstance].config.faceGroups mutableCopy]];
    }
    return _faceView;
}
