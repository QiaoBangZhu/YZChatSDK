...
//
//  CIGAMCommonTableViewController.h
//  cigam
//
...
extern NSString *const CIGAMCommonTableViewControllerSectionHeaderIdentifier;
extern NSString *const CIGAMCommonTableViewControllerSectionFooterIdentifier;

/**
 *  可作为项目内所有 `UITableViewController` 的基类，注意是继承自 `CIGAMCommonViewController` 而不是 `UITableViewController`。
 *
 *  一般通过 `initWithStyle:` 方法初始化，对于要生成 `UITableViewStylePlain` 类型的列表，推荐使用 `init` 方法。
 *
 *  提供的功能包括：
 *
 *  1. 集成 `CIGAMSearchController`，可通过属性 `shouldShowSearchBar` 来快速为列表生成一个 searchBar 及 searchController，具体请查看 CIGAMCommonTableViewController (Search)。
 ...
 *  3. 自带一个 CIGAMEmptyView，作为 tableView 的 subview，可用于显示 loading、空或错误提示语等。
 *
 ...
 *
 *  @see CIGAMSearchController
 */
@interface CIGAMCommonTableViewController : CIGAMCommonViewController<CIGAMTableViewDelegate, CIGAMTableViewDataSource>
...
/// @note 直接把自定义 tableView 赋值给 self.tableView 也可以，但 CIGAM 将会多余地创建一次 CIGAMTableView，会造成浪费。
#if !TARGET_INTERFACE_BUILDER
@property(nonatomic, strong, null_resettable) IBOutlet __kindof CIGAMTableView *tableView;
#else
@property(nonatomic, strong, null_resettable) IBOutlet CIGAMTableView *tableView;
#endif
...
@interface CIGAMCommonTableViewController (CIGAMSubclassingHooks)
