...
@interface CIGAMNavigationController : UINavigationController

@end

@interface CIGAMNavigationController (UISubclassingHooks)
...
/// 与 CIGAMNavigationController push/pop 相关的一些方法
@protocol CIGAMNavigationControllerTransitionDelegate <NSObject>
 ...
 *  @param navigationController 当前正在手势返回的 CIGAMNavigationController，由于某些阶段下无法通过 vc.navigationController 获取到 nav 的引用，所以直接传一个参数
 *  @param gestureRecognizer 手势对象
 *  @param viewControllerWillDisappear 手势返回中顶部的那个 vc
 *  @param viewControllerWillAppear 手势返回中背后的那个 vc
 */
- (void)navigationController:(nonnull CIGAMNavigationController *)navigationController
...
/**
 *  当前界面正处于手势返回的过程中，可自行通过 gestureRecognizer.state 来区分手势返回的各个阶段。手势返回有多个阶段（手势返回开始、拖拽过程中、松手并成功返回、松手但不切换界面），不同阶段的 viewController 的状态可能不一样。
 *  @param navigationController 当前正在手势返回的 CIGAMNavigationController，请勿通过 vc.navigationController 获取 UINavigationController 的引用，而应该用本参数。因为某些手势阶段，vc.navigationController 得到的是 nil。
 *  @param gestureRecognizer 手势对象
...
/// 与 CIGAMNavigationController 外观样式相关的方法
@protocol CIGAMNavigationControllerAppearanceDelegate <NSObject>
...
/// 与 CIGAMNavigationController 控制 navigationBar 显隐/动画相关的方法
@protocol CIGAMCustomNavigationBarTransitionDelegate <NSObject>
...
/**
 *  在实现了系统的自定义转场情况下，导航栏转场的时候是否需要使用 CIGAM 自定义的 push / pop transition 效果，默认不实现的话则不会使用，只要前后其中一个 vc 实现并返回了 YES 则会使用。
 *  @see UINavigationController+NavigationBarTransition.h
 */
...
/**
 *  配合 CIGAMNavigationController 使用，当 navController 里的 UIViewController 实现了这个协议时，则可得到协议里各个方法的功能。
 *  CIGAMCommonViewController、CIGAMCommonTableViewController 默认实现了这个协议，所以子类无需再手动实现一遍。
 */
@protocol CIGAMNavigationControllerDelegate <UINavigationControllerDelegate, CIGAMNavigationControllerTransitionDelegate, CIGAMNavigationControllerAppearanceDelegate, CIGAMCustomNavigationBarTransitionDelegate>
