...
@class CIGAMNavigationTitleView;
@class CIGAMEmptyView;


/**
 *  可作为项目内所有 `UIViewController` 的基类，提供的功能包括：
 *
 *  1. 自带顶部标题控件 `CIGAMNavigationTitleView`，支持loading、副标题、下拉菜单，设置标题依然使用系统的 `-[UIViewController setTitle:]` 或 `-[UINavigationItem setTitle:]` 方法
 *
 *  2. 自带空界面控件 `CIGAMEmptyView`，支持显示loading、空文案、操作按钮
 *
 *  3. 统一约定的常用接口，例如初始化 subview、设置顶部 `navigationItem`、底部 `toolbarItem`、响应系统的动态字体大小变化、...，从而保证相同类型的代码集中到同一个方法内，避免多人交叉维护时代码分散难以查找
 *
 *  4. 配合 `CIGAMNavigationController` 使用时，可以得到 `willPopInNavigationControllerWithAnimated:`、`didPopInNavigationControllerWithAnimated:` 这两个时机
 *
 *  @see CIGAMNavigationTitleView
 *  @see CIGAMEmptyView
 */
@interface CIGAMCommonViewController : UIViewController {
    CIGAMEmptyView *_emptyView;
}
...
/**
 *  CIGAMCommonViewController默认都会增加一个CIGAMNavigationTitleView的titleView，然后重写了setTitle来间接设置titleView的值。所以设置title的时候就跟系统的接口一样：self.title = xxx。
 *
 *  同时，CIGAMNavigationTitleView提供了更多的功能，具体可以参考CIGAMNavigationTitleView的文档。<br/>
 *  @see CIGAMNavigationTitleView
 */
@property(nullable, nonatomic, strong, readonly) CIGAMNavigationTitleView *titleView;
 ...
 */
@property(nullable, nonatomic, strong) CIGAMEmptyView *emptyView;
...
@interface CIGAMCommonViewController (CIGAMSubclassingHooks)
...
@interface CIGAMCommonViewController (CIGAMNavigationController) <CIGAMNavigationControllerDelegate>

/**
 从 CIGAMNavigationControllerAppearanceDelegate 系列接口获取当前界面希望的导航栏样式并设置到导航栏上
 */
...
/**
 *  为了方便实现“点击空白区域降下键盘”的需求，CIGAMCommonViewController 内部集成一个 tap 手势对象并添加到 self.view 上，而业务只需要通过重写 -shouldHideKeyboardWhenTouchInView: 方法并根据当前被点击的 view 返回一个 BOOL 来控制键盘的显隐即可。
 *  @note 为了避免不必要的事件拦截，集成的手势 hideKeyboardTapGestureRecognizer：
 *  1. 默认的 enabled = NO。
 *  2. 如果当前 viewController 或其父类（非 CIGAMCommonViewController 那个层级的父类）没重写 -shouldHideKeyboardWhenTouchInView:，则永远 enabled = NO。
 *  3. 在键盘升起时，并且当前 viewController 重写了 -shouldHideKeyboardWhenTouchInView: 且处于可视状态下，此时手势的 enabled 才会被修改为 YES，并且在键盘消失时置为 NO。
 */
@interface CIGAMCommonViewController (CIGAMKeyboard)
...
@property(nullable, nonatomic, strong, readonly) CIGAMKeyboardManager *hideKeyboardManager;
