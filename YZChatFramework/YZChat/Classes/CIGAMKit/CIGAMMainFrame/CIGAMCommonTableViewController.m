...
//
//  CIGAMCommonTableViewController.m
//  cigam
//
...
NSString *const CIGAMCommonTableViewControllerSectionHeaderIdentifier = @"CIGAMSectionHeaderView";
NSString *const CIGAMCommonTableViewControllerSectionFooterIdentifier = @"CIGAMSectionFooterView";

@interface _CIGAMTableViewObserver : NSObject

@property(nonatomic, weak) CIGAMCommonTableViewController *viewController;
@end

@interface CIGAMCommonTableViewController ()

@property(nonatomic, assign) BOOL hasHideTableHeaderViewInitial;
@property(nonatomic, strong) _CIGAMTableViewObserver *_cigamTableViewObserver;
@end


@implementation CIGAMCommonTableViewController
...
- (void)dealloc {
    ...
    if (self._cigamTableViewObserver) {
        [_tableView removeObserver:self._cigamTableViewObserver forKeyPath:@"contentInset"];
    }
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!self.tableView.allowsMultipleSelection) {
        [self cigam_animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
            [self.tableView cigam_clearsSelection];
        } completion:nil];
    }
}
...
- (__kindof CIGAMTableView *)tableView {
    if (!_tableView) {
...
- (void)setTableView:(__kindof CIGAMTableView *)tableView {
    if (_tableView != tableView) {
        if (_tableView) {
            // 这里不用移除 delegate、dataSource，因为原本的值也不一定是指向 self，而且可能是个 CIGAMMultipleDelegate，反正这两个属性都是 weak 的
        ...
        [_tableView registerClass:[CIGAMTableViewHeaderFooterView class] forHeaderFooterViewReuseIdentifier:CIGAMCommonTableViewControllerSectionHeaderIdentifier];
        [_tableView registerClass:[CIGAMTableViewHeaderFooterView class] forHeaderFooterViewReuseIdentifier:CIGAMCommonTableViewControllerSectionFooterIdentifier];
        ...
        if (@available(iOS 11, *)) {
             ...
             */
            if (!self._cigamTableViewObserver) {
                self._cigamTableViewObserver = [[_CIGAMTableViewObserver alloc] init];
                self._cigamTableViewObserver.viewController = self;
            }
            [_tableView addObserver:self._cigamTableViewObserver forKeyPath:@"contentInset" options:NSKeyValueObservingOptionOld context:nil];
        }
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1156
...
- (void)hideTableHeaderViewInitialIfCanWithAnimated:(BOOL)animated force:(BOOL)force {
    if (self.tableView.tableHeaderView && [self shouldHideTableHeaderViewInitial] && (force || !self.hasHideTableHeaderViewInitial)) {
        CGPoint contentOffset = CGPointMake(self.tableView.contentOffset.x, -self.tableView.cigam_contentInset.top + CGRectGetHeight(self.tableView.tableHeaderView.frame));
...
- (BOOL)layoutEmptyView {
    ...
    UIEdgeInsets insets = self.tableView.cigam_contentInset;
...
#pragma mark - <CIGAMTableViewDelegate, CIGAMTableViewDataSource>
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (title) {
        CIGAMTableViewHeaderFooterView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:CIGAMCommonTableViewControllerSectionHeaderIdentifier];
        headerView.parentTableView = tableView;
        headerView.type = CIGAMTableViewHeaderFooterViewTypeHeader;
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (title) {
        CIGAMTableViewHeaderFooterView *footerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:CIGAMCommonTableViewControllerSectionFooterIdentifier];
        footerView.parentTableView = tableView;
        footerView.type = CIGAMTableViewHeaderFooterViewTypeFooter;
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    ...
    return PreferredValueForTableViewStyle(tableView.cigam_style, 0, TableViewGroupedSectionHeaderDefaultHeight, TableViewInsetGroupedSectionHeaderDefaultHeight);
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    ...
    // 分别测试过 iOS 13 及以下的所有版本，最终总结，对于 Plain 类型的 tableView 而言，要去掉 header / footer 请使用 0，对于 Grouped 类型的 tableView 而言，要去掉 header / footer 请使用 CGFLOAT_MIN
    return PreferredValueForTableViewStyle(tableView.cigam_style, 0, TableViewGroupedSectionFooterDefaultHeight, TableViewInsetGroupedSectionFooterDefaultHeight);
}
...
@implementation CIGAMCommonTableViewController (CIGAMSubclassingHooks)

- (void)initTableView {
    if (!_tableView) {
        self.tableView = [[CIGAMTableView alloc] initWithFrame:self.isViewLoaded ? self.view.bounds : CGRectZero style:self.style];
...
- (void)layoutTableView {
    ...
    if (shouldChangeTableViewFrame) {
        self.tableView.cigam_frameApplyTransform = self.view.bounds;
    }
}
...
@implementation _CIGAMTableViewObserver
