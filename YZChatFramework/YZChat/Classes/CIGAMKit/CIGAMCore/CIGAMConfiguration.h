...
@protocol CIGAMConfigurationTemplateProtocol <NSObject>
...
@optional
/// 当返回 YES 时，启动 App 的时候 CIGAMConfiguration 会自动应用这份配置表。但启动 App 时自动应用的配置表最多只允许一份，如果有多份则其他的会被忽略
/// CIGAMConfiguration automatically applies this template on launch when set to YES. Since only one copy of configuration template is allowed when the app launches, you'll have to call `applyConfigurationTemplate` manually if you have more than one configuration templates. 
- (BOOL)shouldApplyTemplateAutomatically;

@end

/**
 *  维护项目全局 UI 配置的单例，通过业务项目自己的 CIGAMConfigurationTemplate 来为这个单例赋值，而业务代码里则通过 CIGAMConfigurationMacros.h 文件里的宏来使用这些值。
 *  A singleton that contains various UI configurations. Use `CIGAMConfigurationTemplate` to set values; Use macros in `CIGAMConfigurationMacros.h` to get values.
 */
@interface CIGAMConfiguration : NSObject
...
@property(nonatomic, assign) CGFloat            windowLevelCIGAMAlertView;
@property(nonatomic, assign) CGFloat            windowLevelCIGAMConsole;
...
@property(nonatomic, assign) BOOL               shouldPrintCIGAMWarnLogToConsole;
...
@property(nonatomic, assign) BOOL               sendAnalyticsToCIGAMTeam;
...
@interface UINavigationBar (CIGAMConfiguration)
...
@interface UITabBar (CIGAMConfiguration)
...
@interface UIToolbar (CIGAMConfiguration)
...
@interface UITabBarItem (CIGAMConfiguration)
