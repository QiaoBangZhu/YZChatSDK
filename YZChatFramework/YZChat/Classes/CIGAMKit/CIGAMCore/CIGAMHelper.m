...
//  cigam
//
//  Created by CIGAM Team on 14/10/25.
//

#import "CIGAMHelper.h"
#import "CIGAMCore.h"
...
const CGPoint CIGAMBadgeInvalidateOffset = {-1000, -1000};

@implementation CIGAMHelper (Bundle)

+ (UIImage *)imageWithName:(NSString *)name {
    UIImage *image = [UIImage imageNamed:name inBundle:[NSBundle yzBundle] compatibleWithTraitCollection:nil];
    return image;
}
...
@implementation CIGAMHelper (DynamicType)
...
+ (CGFloat)heightForDynamicTypeCell:(NSArray *)heights {
    NSNumber *index = [CIGAMHelper preferredContentSizeLevel];
    return [((NSNumber *)[heights objectAtIndex:[index intValue]]) cigam_CGFloatValue];
}
@end

@implementation CIGAMHelper (Keyboard)

CIGAMSynthesizeBOOLProperty(keyboardVisible, setKeyboardVisible)
CIGAMSynthesizeCGFloatProperty(lastKeyboardHeight, setLastKeyboardHeight)

- (void)handleKeyboardWillShow:(NSNotification *)notification {
    self.keyboardVisible = YES;
    self.lastKeyboardHeight = [CIGAMHelper keyboardHeightWithNotification:notification];
}
...
+ (BOOL)isKeyboardVisible {
    BOOL visible = [CIGAMHelper sharedInstance].keyboardVisible;
    return visible;
}

+ (CGFloat)lastKeyboardHeightInApplicationWindowWhenVisible {
    return [CIGAMHelper sharedInstance].lastKeyboardHeight;
}
...
+ (CGFloat)keyboardHeightWithNotification:(NSNotification *)notification {
    return [CIGAMHelper keyboardHeightWithNotification:notification inView:nil];
}
...
+ (UIViewAnimationOptions)keyboardAnimationOptionsWithNotification:(NSNotification *)notification {
    UIViewAnimationOptions options = [CIGAMHelper keyboardAnimationCurveWithNotification:notification]<<16;
    return options;
}
...
@implementation CIGAMHelper (AudioSession)
...
@implementation CIGAMHelper (UIGraphic)
...
@implementation CIGAMHelper (Device)
        ...
        if (isNotchedScreen < 0) {
            if (@available(iOS 12.0, *)) {
                /*
                 ...
                 2. 与触发 [CIGAMHelper isNotchedScreen] 方法时的进程有关，例如 https://github.com/Tencent/CIGAM_iOS/issues/482#issuecomment-456051738 里提到的 [NSObject performSelectorOnMainThread:withObject:waitUntilDone:NO] 就会导致较多的异常。
                ...
                [[UIScreen mainScreen] cigam_performSelector:peripheryInsetsSelector withPrimitiveReturnValue:&peripheryInsets];
                ...
                isNotchedScreen = [CIGAMHelper is58InchScreen] ? 1 : 0;
            }
        }
    } else {
        isNotchedScreen = 0;
...
+ (BOOL)is65InchScreen {
    if (is65InchScreen < 0) {
        ...
        is65InchScreen = (DEVICE_WIDTH == self.screenSizeFor65Inch.width && DEVICE_HEIGHT == self.screenSizeFor65Inch.height && ([[CIGAMHelper deviceModel] isEqualToString:@"iPhone11,4"] || [[CIGAMHelper deviceModel] isEqualToString:@"iPhone11,6"] || [[CIGAMHelper deviceModel] isEqualToString:@"iPhone12,5"])) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreenAndiPhone12 {
    if (is61InchScreenAndiPhone12 < 0) {
        is61InchScreenAndiPhone12 = (DEVICE_WIDTH == self.screenSizeFor61InchAndiPhone12.width && DEVICE_HEIGHT == self.screenSizeFor61InchAndiPhone12.height && ([[CIGAMHelper deviceModel] isEqualToString:@"iPhone13,2"] || [[CIGAMHelper deviceModel] isEqualToString:@"iPhone13,3"])) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreen {
    if (is61InchScreen < 0) {
        is61InchScreen = (DEVICE_WIDTH == self.screenSizeFor61Inch.width && DEVICE_HEIGHT == self.screenSizeFor61Inch.height && ([[CIGAMHelper deviceModel] isEqualToString:@"iPhone11,8"] || [[CIGAMHelper deviceModel] isEqualToString:@"iPhone12,1"])) ? 1 : 0;
    }
...
+ (CGFloat)preferredLayoutAsSimilarScreenWidthForIPad {
    if (preferredLayoutWidth < 0) {
        ...
        for (NSInteger i = 0; i < widths.count; i++) {
            if (windowWidth <= widths[i].cigam_CGFloatValue) {
                preferredLayoutWidth = widths[i].cigam_CGFloatValue;
                continue;
            }
        }
    }
...
+ (UIEdgeInsets)safeAreaInsetsForDeviceWithNotch {
    ...
    NSString *deviceKey = [CIGAMHelper deviceModel];
    if (!dict[deviceKey]) {
        deviceKey = @"iPhone12,5";// 默认按 iPhone 11 Pro Max
    }
    if ([CIGAMHelper isZoomedMode]) {
...
+ (BOOL)isHighPerformanceDevice {
    if (isHighPerformanceDevice < 0) {
        NSString *model = [CIGAMHelper deviceModel];
        NSString *identifier = [model cigam_stringMatchedByPattern:@"\\d+"];
...
@implementation CIGAMHelper (UIApplication)
...
@implementation CIGAMHelper (Animation)
...
@implementation CIGAMHelper (SystemVersion)
...
+ (BOOL)isCurrentSystemAtLeastVersion:(NSString *)targetVersion {
    return [CIGAMHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedSame || [CIGAMHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedDescending;
}

+ (BOOL)isCurrentSystemLowerThanVersion:(NSString *)targetVersion {
    return [CIGAMHelper compareSystemVersion:[[UIDevice currentDevice] systemVersion] toVersion:targetVersion] == NSOrderedAscending;
}

@end

@implementation CIGAMHelper

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        [CIGAMHelper sharedInstance];// 确保内部的变量、notification 都正确配置
    });
}

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static CIGAMHelper *instance = nil;
    dispatch_once(&onceToken,^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:instance selector:@selector(handleAppSizeWillChange:) name:CIGAMAppSizeWillChangeNotification object:nil];
...
- (void)dealloc {
    // CIGAMHelper 若干个分类里有用到消息监听，所以在 dealloc 的时候注销一下
