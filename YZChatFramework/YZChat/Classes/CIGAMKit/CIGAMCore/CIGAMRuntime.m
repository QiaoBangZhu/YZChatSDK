...
//  CIGAMKit
//
//  Created by CIGAM Team on 2018/9/5.
//

#import "CIGAMRuntime.h"
#import "CIGAMCommonDefines.h"
#import "CIGAMHelper.h"
#include <mach-o/getsect.h>
#include <mach-o/dyld.h>

@implementation CIGAMPropertyDescriptor

+ (instancetype)descriptorWithProperty:(objc_property_t)property {
    CIGAMPropertyDescriptor *descriptor = [[self alloc] init];
    ...
    descriptor.type = [CIGAMPropertyDescriptor typeWithEncodeString:[NSString stringWithUTF8String:type]];
    if (type != NULL) {
        free(type);
    }
...
int cigam_getProjectClassList(classref_t **classes) {
    size_t count = 0;
    if (!!classes) {
...
BOOL cigam_exists_dyld_image(const char *target_image_name) {
    const uint32_t imageCount = _dyld_image_count();
    for (uint32_t i = 0; i < imageCount; i++) {
