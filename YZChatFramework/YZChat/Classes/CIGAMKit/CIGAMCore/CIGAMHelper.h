...
extern const CGPoint CIGAMBadgeInvalidateOffset;

@interface CIGAMHelper : NSObject
...
@interface CIGAMHelper (Bundle)

/// 获取 CIGAMKit.framework Images.xcassets 内的图片资源
/// @param name 图片名
+ (nullable UIImage *)imageWithName:(NSString *)name;

@end

@interface CIGAMHelper (SystemVersion)
...
@interface CIGAMHelper (DynamicType)
...
@interface CIGAMHelper (Keyboard)
...
/**
 * 获取当前键盘在屏幕上的可见高度，注意外接键盘（iPad那种）时，[CIGAMHelper keyboardRectWithNotification]得到的键盘rect里有一部分是超出屏幕，不可见的，如果直接拿rect的高度来计算就会与意图相悖。
 * @param notification 接收到的键盘事件的UINotification对象
 * @param view 要得到的键盘高度是相对于哪个View的键盘高度，若为nil，则等同于调用[CIGAMHelper keyboardHeightWithNotification:]
...
@interface CIGAMHelper (AudioSession)
...
@interface CIGAMHelper (UIGraphic)
...
/// @note 注意，这里普通/紧凑的标准是 CIGAM 自行制定的，与系统 UITraitCollection.horizontalSizeClass/verticalSizeClass 的值无关。只要是通常意义上的“大屏幕手机”（例如 Plus 系列）都会被视为 Regular Screen。
/// @NEW_DEVICE_CHECKER
...
@interface CIGAMHelper (UIApplication)
 ...
 * @note 在 iOS 13 以前  UIStatusBarStyleDefault 状态栏内容的颜色固定是黑色的，而在 iOS 13 UIStatusBarStyleDefault 会根据 user interface style 来决定状态栏的颜色，如果你需要一直黑色可以用 CIGAMStatusBarStyleDarkContent 来代替以前 UIStatusBarStyleDefault 的写法
...
@interface CIGAMHelper (Animation)
