...
//
//  CIGAMPopupContainerView.m
//  cigam
//
//  Created by CIGAM Team on 15/12/17.
//
...
@interface CIGAMPopupContainerViewWindow : UIWindow

@end

@interface CIGAMPopContainerViewController : CIGAMCommonViewController

@end

@interface CIGAMPopContainerMaskControl : UIControl

@property(nonatomic, weak) CIGAMPopupContainerView *popupContainerView;
@end

@interface CIGAMPopupContainerView () {
    UIImageView                     *_imageView;
    UILabel                         *_textLabel;
}

@property(nonatomic, strong) CIGAMPopupContainerViewWindow *popupWindow;
@property(nonatomic, weak) UIWindow *previousKeyWindow;
@property(nonatomic, assign) BOOL hidesByUserTap;
@end

@implementation CIGAMPopupContainerView
...
- (void)dealloc {
    _sourceView.cigam_frameDidChangeBlock = nil;
}
...
- (void)layoutSubviews {
    ...
    CGRect roundedRect = CGRectMake(self.borderWidth / 2.0 + (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionRight ? arrowSize.width : 0),
                                    self.borderWidth / 2.0 + (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow ? arrowSize.height : 0),
    ...
    if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionRight) {
        // 箭头向左
    ...
    if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionAbove) {
        // 箭头向下
        if (isUsingArrowImage) {
    ...
    if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionLeft) {
        // 箭头向右
        if (isUsingArrowImage) {
            arrowImageTransform = CGAffineTransformMakeRotation(AngleWithDegrees(-90));
    ...
    if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow) {
        // 箭头向上
        if (isUsingArrowImage) {
...
- (void)layoutDefaultSubviews {
    self.contentView.frame = CGRectMake(
                                        self.borderWidth + self.contentEdgeInsets.left + (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionRight ? self.arrowSizeAuto.width : 0),
                                        self.borderWidth + self.contentEdgeInsets.top + (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow ? self.arrowSizeAuto.height : 0),
...
- (void)setSourceBarItem:(__kindof UIBarItem *)sourceBarItem {
    ...
    sourceBarItem.cigam_viewLayoutDidChangeBlock = ^(__kindof UIBarItem * _Nonnull item, UIView * _Nullable view) {
        ...
        CGRect rect = [view cigam_convertRect:view.bounds toView:convertToView];
        weakSelf.sourceRect = rect;
    };
    if (sourceBarItem.cigam_view && sourceBarItem.cigam_viewLayoutDidChangeBlock) {
        sourceBarItem.cigam_viewLayoutDidChangeBlock(sourceBarItem, sourceBarItem.cigam_view);// update layout immediately
    }
}
...
- (void)setSourceView:(__kindof UIView *)sourceView {
    ...
    __weak __typeof(self)weakSelf = self;
    sourceView.cigam_frameDidChangeBlock = ^(__kindof UIView * _Nonnull view, CGRect precedingFrame) {
        if (!view.window || !weakSelf.superview) return;
        UIView *convertToView = weakSelf.popupWindow ? UIApplication.sharedApplication.delegate.window : weakSelf.superview;// 对于以 window 方式显示的情况，由于横竖屏旋转时，不同 window 的旋转顺序不同，所以可能导致 sourceBarItem 所在的 window 已经旋转了但 popupWindow 还没旋转（iOS 11 及以后），那么计算出来的坐标就错了，所以这里改为用 UIApplication window
        CGRect rect = [view cigam_convertRect:view.bounds toView:convertToView];
        weakSelf.sourceRect = rect;
    };
    sourceView.cigam_frameDidChangeBlock(sourceView, sourceView.frame);// update layout immediately
}
...
- (void)layoutWithTargetRect:(CGRect)targetRect {
    ...
    CGSize (^sizeToFitBlock)(void) = ^CGSize(void) {
        ...
        } else if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionLeft) {
            result.width = CGRectGetMinX(targetRect) - self.distanceBetweenSource - self.safetyMarginsAvoidSafeAreaInsets.left;
        } else if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionRight) {
        ...
        } else if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionAbove) {
            result.height = CGRectGetMinY(targetRect) - self.distanceBetweenSource - self.safetyMarginsAvoidSafeAreaInsets.top;
        } else if (self.currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow) {
    ...
    if (self.isVerticalLayoutDirection) {
        ...
        BOOL canShowAtAbove = [self canTipShowAtSpecifiedLayoutDirect:CIGAMPopupContainerViewLayoutDirectionAbove targetRect:targetRect tipSize:tipSize];
        BOOL canShowAtBelow = [self canTipShowAtSpecifiedLayoutDirect:CIGAMPopupContainerViewLayoutDirectionBelow targetRect:targetRect tipSize:tipSize];
        ...
        if (!canShowAtAbove && !canShowAtBelow) {
            ...
            _currentLayoutDirection = maximumHeightAbove > maximumHeightBelow ? CIGAMPopupContainerViewLayoutDirectionAbove : CIGAMPopupContainerViewLayoutDirectionBelow;
            
            CIGAMLog(NSStringFromClass(self.class), @"%@, 因为上下都不够空间，所以最大高度被强制改为%@, 位于目标的%@", self, @(self.maximumHeight), maximumHeightAbove > maximumHeightBelow ? @"上方" : @"下方");
            
        } else if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionAbove && !canShowAtAbove) {
            _currentLayoutDirection = CIGAMPopupContainerViewLayoutDirectionBelow;
            tipSize.height = [self sizeThatFits:CGSizeMake(tipSize.width, sizeToFitBlock().height)].height;
        } else if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow && !canShowAtBelow) {
            _currentLayoutDirection = CIGAMPopupContainerViewLayoutDirectionAbove;
            tipSize.height = [self sizeThatFits:CGSizeMake(tipSize.width, sizeToFitBlock().height)].height;
        }
        ...
        if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionAbove) {
        ...
        } else if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow) {
        ...
        BOOL canShowAtLeft = [self canTipShowAtSpecifiedLayoutDirect:CIGAMPopupContainerViewLayoutDirectionLeft targetRect:targetRect tipSize:tipSize];
        BOOL canShowAtRight = [self canTipShowAtSpecifiedLayoutDirect:CIGAMPopupContainerViewLayoutDirectionRight targetRect:targetRect tipSize:tipSize];
        ...
        if (!canShowAtLeft && !canShowAtRight) {
            ...
            _currentLayoutDirection = maximumWidthLeft > maximumWidthRight ? CIGAMPopupContainerViewLayoutDirectionLeft : CIGAMPopupContainerViewLayoutDirectionRight;
            
            CIGAMLog(NSStringFromClass(self.class), @"%@, 因为左右都不够空间，所以最大宽度被强制改为%@, 位于目标的%@", self, @(self.maximumWidth), maximumWidthLeft > maximumWidthRight ? @"左边" : @"右边");
            
        } else if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionLeft && !canShowAtLeft) {
            _currentLayoutDirection = CIGAMPopupContainerViewLayoutDirectionLeft;
            tipSize.width = [self sizeThatFits:CGSizeMake(sizeToFitBlock().width, tipSize.height)].width;
        } else if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow && !canShowAtRight) {
            _currentLayoutDirection = CIGAMPopupContainerViewLayoutDirectionRight;
            tipSize.width = [self sizeThatFits:CGSizeMake(sizeToFitBlock().width, tipSize.height)].width;
        }
        ...
        if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionLeft) {
        ...
        } else if (_currentLayoutDirection == CIGAMPopupContainerViewLayoutDirectionRight) {
...
- (CGPoint)tipOriginWithTargetRect:(CGRect)itemRect tipSize:(CGSize)tipSize preferLayoutDirection:(CIGAMPopupContainerViewLayoutDirection)direction {
    CGPoint tipOrigin = CGPointZero;
    switch (direction) {
        case CIGAMPopupContainerViewLayoutDirectionAbove:
            tipOrigin.y = CGRectGetMinY(itemRect) - tipSize.height - self.distanceBetweenSource;
            break;
        case CIGAMPopupContainerViewLayoutDirectionBelow:
            tipOrigin.y = CGRectGetMaxY(itemRect) + self.distanceBetweenSource;
            break;
        case CIGAMPopupContainerViewLayoutDirectionLeft:
            tipOrigin.x = CGRectGetMinX(itemRect) - tipSize.width - self.distanceBetweenSource;
            break;
        case CIGAMPopupContainerViewLayoutDirectionRight:
...
- (BOOL)canTipShowAtSpecifiedLayoutDirect:(CIGAMPopupContainerViewLayoutDirection)direction targetRect:(CGRect)itemRect tipSize:(CGSize)tipSize {
    BOOL canShow = NO;
    if (self.isVerticalLayoutDirection) {
        CGFloat tipMinY = [self tipOriginWithTargetRect:itemRect tipSize:tipSize preferLayoutDirection:direction].y;
        if (direction == CIGAMPopupContainerViewLayoutDirectionAbove) {
            canShow = tipMinY >= self.safetyMarginsAvoidSafeAreaInsets.top;
        } else if (direction == CIGAMPopupContainerViewLayoutDirectionBelow) {
        ...
        if (direction == CIGAMPopupContainerViewLayoutDirectionLeft) {
            canShow = tipMinX >= self.safetyMarginsAvoidSafeAreaInsets.left;
        } else if (direction == CIGAMPopupContainerViewLayoutDirectionRight) {
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (!self.superview) {
        ...
        CIGAMCommonViewController *viewController = (CIGAMCommonViewController *)self.popupWindow.rootViewController;
        viewController.supportedOrientationMask = [CIGAMHelper visibleViewController].supportedInterfaceOrientations;
...
- (void)hideCompletionWithWindowMode:(BOOL)windowMode completion:(void (^)(BOOL))completion {
    if (windowMode) {
        // 恢复 keyWindow 之前做一下检查，避免类似问题 https://github.com/Tencent/CIGAM_iOS/issues/90
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/75
...
- (void)initPopupContainerViewWindowIfNeeded {
    if (!self.popupWindow) {
        self.popupWindow = [[CIGAMPopupContainerViewWindow alloc] init];
        self.popupWindow.cigam_capturesStatusBarAppearance = NO;
        self.popupWindow.backgroundColor = UIColorClear;
        self.popupWindow.windowLevel = UIWindowLevelCIGAMAlertView;
        CIGAMPopContainerViewController *viewController = [[CIGAMPopContainerViewController alloc] init];
        ((CIGAMPopContainerMaskControl *)viewController.view).popupContainerView = self;
...
- (BOOL)isHorizontalLayoutDirection {
    return self.preferLayoutDirection == CIGAMPopupContainerViewLayoutDirectionLeft || self.preferLayoutDirection == CIGAMPopupContainerViewLayoutDirectionRight;
}

- (BOOL)isVerticalLayoutDirection {
    return self.preferLayoutDirection == CIGAMPopupContainerViewLayoutDirectionAbove || self.preferLayoutDirection == CIGAMPopupContainerViewLayoutDirectionBelow;
}
...
- (void)setArrowImage:(UIImage *)arrowImage {
    ...
    if (arrowImage) {
        ...
        if (!_arrowImageLayer) {
            ...
            [_arrowImageLayer cigam_removeDefaultAnimations];
...
- (UIEdgeInsets)safetyMarginsAvoidSafeAreaInsets {
    ...
    if (self.isHorizontalLayoutDirection) {
        result.left += self.superview.cigam_safeAreaInsets.left;
        result.right += self.superview.cigam_safeAreaInsets.right;
    } else {
        result.top += self.superview.cigam_safeAreaInsets.top;
        result.bottom += self.superview.cigam_safeAreaInsets.bottom;
    }
    return result;
}
...
@implementation CIGAMPopupContainerView (UISubclassingHooks)

- (void)didInitialize {
    _backgroundLayer = [CAShapeLayer layer];
    [_backgroundLayer cigam_removeDefaultAnimations];
...
@implementation CIGAMPopupContainerView (UIAppearance)
...
+ (void)setDefaultAppearance {
    CIGAMPopupContainerView *appearance = [CIGAMPopupContainerView appearance];
    ...
    appearance.preferLayoutDirection = CIGAMPopupContainerViewLayoutDirectionAbove;
    ...
    appearance.cigam_outsideEdge = UIEdgeInsetsZero;
    
}

@end

@implementation CIGAMPopContainerViewController

- (void)loadView {
    CIGAMPopContainerMaskControl *maskControl = [[CIGAMPopContainerMaskControl alloc] init];
    self.view = maskControl;
}

@end

@implementation CIGAMPopContainerMaskControl
...
@implementation CIGAMPopupContainerViewWindow
