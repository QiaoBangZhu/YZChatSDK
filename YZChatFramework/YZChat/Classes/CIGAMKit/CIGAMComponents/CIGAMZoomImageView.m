...
//
//  CIGAMZoomImageView.m
//  cigam
//
//  Created by CIGAM Team on 14-9-14.
//
...
// generate icon images needed by CIGAMZoomImageView
// 用于生成 CIGAMZoomImageView 所需的一些简单的图标图片
@interface CIGAMZoomImageViewImageGenerator : NSObject
...
@interface CIGAMZoomImageVideoPlayerView : UIView

@end

static NSUInteger const kTagForCenteredPlayButton = 1;

@interface CIGAMZoomImageView () <UIGestureRecognizerDelegate>

// video play
@property(nonatomic, strong) CIGAMZoomImageVideoPlayerView *videoPlayerView;
...
@implementation CIGAMZoomImageView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _scrollView = [[UIScrollView alloc] cigam_initWithSize:frame.size];
        ...
        _emptyView = [[CIGAMEmptyView alloc] init];
...
- (void)layoutSubviews {
    ...
    if (_videoToolbar) {
        _videoToolbar.frame = ({
            UIEdgeInsets margins = UIEdgeInsetsConcat(self.videoToolbarMargins, self.cigam_safeAreaInsets);
...
- (void)setImage:(UIImage *)image {
    ...
    self.imageView.cigam_frameApplyTransform = CGRectMakeWithSize(image.size);
...
- (void)setLivePhoto:(PHLivePhoto *)livePhoto {
    ...
    _livePhotoView.cigam_frameApplyTransform = CGRectMakeWithSize(livePhoto.size);
...
- (void)revertZooming {
    ...
    self.scrollView.maximumZoomScale = maximumZoomScale;
    self.contentView.frame = CGRectSetXY(self.contentView.frame, 0, 0);// 重置 origin 的目的是：https://github.com/Tencent/CIGAM_iOS/issues/400
...
- (void)setZoomScale:(CGFloat)zoomScale animated:(BOOL)animated {
    if (animated) {
        [UIView animateWithDuration:.25 delay:0.0 options:CIGAMViewAnimationOptionsCurveOut animations:^{
...
- (void)setVideoPlayerItem:(AVPlayerItem *)videoPlayerItem {
    ...
    self.videoPlayerView.cigam_frameApplyTransform = CGRectMakeWithSize(self.videoSize);
...
- (void)addPlayerTimeObserver {
    ...
    double interval = .1f;
    __weak CIGAMZoomImageView *weakSelf = self;
...
- (CIGAMZoomImageViewVideoToolbar *)videoToolbar {
    [self initVideoToolbarIfNeeded];
    return _videoToolbar;
}

- (CIGAMButton *)videoCenteredPlayButton {
...
- (void)initVideoPlayerLayerIfNeeded {
    ...
    self.videoPlayerView = [[CIGAMZoomImageVideoPlayerView alloc] init];
...
- (void)initVideoToolbarIfNeeded {
    ...
    _videoToolbar = ({
        CIGAMZoomImageViewVideoToolbar * b = [[CIGAMZoomImageViewVideoToolbar alloc] init];
...
- (void)initVideoCenteredPlayButtonIfNeeded {
    ...
    _videoCenteredPlayButton = ({
        CIGAMButton *b = [[CIGAMButton alloc] init];
        b.cigam_outsideEdge = UIEdgeInsetsMake(-60, -60, -60, -60);
...
- (CIGAMPieProgressView *)cloudProgressView {
...
- (void)initCloudProgressViewIfNeeded {
    ...
    _cloudProgressView = [[CIGAMPieProgressView alloc] init];
...
- (void)initCloudDownloadRetryButtonIfNeeded {
    ...
    _cloudDownloadRetryButton = [[CIGAMButton alloc] init];
    [_cloudDownloadRetryButton setImage:[CIGAMHelper imageWithName:@"CIGAM_icloud_download_fault"] forState:UIControlStateNormal];
    ...
    _cloudDownloadRetryButton.cigam_outsideEdge = UIEdgeInsetsMake(-6, -6, -6, -6);
...
- (void)setCloudDownloadStatus:(CIGAMAssetDownloadStatus)cloudDownloadStatus {
...
@interface CIGAMZoomImageView (UIAppearance)

@end

@implementation CIGAMZoomImageView (UIAppearance)
...
+ (void)setDefaultAppearance {
    CIGAMZoomImageView *appearance = [CIGAMZoomImageView appearance];
    appearance.videoToolbarMargins = UIEdgeInsetsMake(0, 25, 25, 18);
    appearance.videoCenteredPlayButtonImage = [CIGAMZoomImageViewImageGenerator largePlayImage];
}

@end

@implementation CIGAMZoomImageVideoPlayerView
...
@implementation CIGAMZoomImageViewImageGenerator

+ (UIImage *)largePlayImage {
    CGFloat width = 60;
    return [UIImage cigam_imageWithSize:CGSizeMake(width, width) opaque:NO scale:0 actions:^(CGContextRef contextRef) {
...
@implementation CIGAMZoomImageViewVideoToolbar

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        
        _playButton = [[CIGAMButton alloc] init];
        self.playButton.cigam_outsideEdge = UIEdgeInsetsMake(-10, -10, -10, -10);
        [self.playButton setImage:self.playButtonImage forState:UIControlStateNormal];
        [self addSubview:self.playButton];
        
        _pauseButton = [[CIGAMButton alloc] init];
        self.pauseButton.cigam_outsideEdge = UIEdgeInsetsMake(-10, -10, -10, -10);
        [self.pauseButton setImage:self.pauseButtonImage forState:UIControlStateNormal];
        [self addSubview:self.pauseButton];
        
        _slider = [[CIGAMSlider alloc] init];
        ...
        _sliderLeftLabel = [[UILabel alloc] cigam_initWithFont:UIFontMake(12) textColor:UIColorWhite];
        ...
        [self.sliderRightLabel cigam_setTheSameAppearanceAsLabel:self.sliderLeftLabel];
...
@interface CIGAMZoomImageViewVideoToolbar (UIAppearance)

@end

@implementation CIGAMZoomImageViewVideoToolbar (UIAppearance)
...
+ (void)setDefaultAppearance {
    CIGAMZoomImageViewVideoToolbar *appearance = [CIGAMZoomImageViewVideoToolbar appearance];
    appearance.playButtonImage = [CIGAMZoomImageViewImageGenerator smallPlayImage];
    appearance.pauseButtonImage = [CIGAMZoomImageViewImageGenerator pauseImage];
}
