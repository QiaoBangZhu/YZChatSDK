...
//
//  CIGAMImagePreviewViewTransitionAnimator.m
//  CIGAMKit
//
//  Created by MoLice on 2018/D/19.
//

#import "CIGAMImagePreviewViewTransitionAnimator.h"
#import "CIGAMCore.h"
#import "CALayer+CIGAM.h"

@implementation CIGAMImagePreviewViewTransitionAnimator
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self.cornerRadiusMaskLayer cigam_removeDefaultAnimations];
        self.cornerRadiusMaskLayer.backgroundColor = [UIColor whiteColor].CGColor;
        
        self.animationEnteringBlock = ^(__kindof CIGAMImagePreviewViewTransitionAnimator * _Nonnull animator, BOOL isPresenting, CIGAMImagePreviewViewControllerTransitioningStyle style, CGRect sourceImageRect, CIGAMZoomImageView * _Nonnull zoomImageView, id<UIViewControllerContextTransitioning>  _Nullable transitionContext) {
            
            UIView *previewView = animator.imagePreviewViewController.view;
            
            if (style == CIGAMImagePreviewViewControllerTransitioningStyleFade) {
                
                previewView.alpha = isPresenting ? 0 : 1;
                
            } else if (style == CIGAMImagePreviewViewControllerTransitioningStyleZoom) {
                ...
                CGFloat cornerRadius = animator.imagePreviewViewController.sourceImageCornerRadius == CIGAMImagePreviewViewControllerCornerRadiusAutomaticDimension && animator.imagePreviewViewController.sourceImageView ? animator.imagePreviewViewController.sourceImageView().layer.cornerRadius : MAX(animator.imagePreviewViewController.sourceImageCornerRadius, 0);
        ...
        self.animationBlock = ^(__kindof CIGAMImagePreviewViewTransitionAnimator * _Nonnull animator, BOOL isPresenting, CIGAMImagePreviewViewControllerTransitioningStyle style, CGRect sourceImageRect, CIGAMZoomImageView * _Nonnull zoomImageView, id<UIViewControllerContextTransitioning>  _Nullable transitionContext) {
            if (style == CIGAMImagePreviewViewControllerTransitioningStyleFade) {
        ...
        self.animationCompletionBlock = ^(__kindof CIGAMImagePreviewViewTransitionAnimator * _Nonnull animator, BOOL isPresenting, CIGAMImagePreviewViewControllerTransitioningStyle style, CGRect sourceImageRect, CIGAMZoomImageView * _Nonnull zoomImageView, id<UIViewControllerContextTransitioning>  _Nullable transitionContext) {
...
- (void)animateTransition:(nonnull id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    CIGAMImagePreviewViewControllerTransitioningStyle style = isPresenting ? self.imagePreviewViewController.presentingStyle : self.imagePreviewViewController.dismissingStyle;
    CGRect sourceImageRect = CGRectZero;
    if (style == CIGAMImagePreviewViewControllerTransitioningStyleZoom) {
    ...
    style = style == CIGAMImagePreviewViewControllerTransitioningStyleZoom && CGRectEqualToRect(sourceImageRect, CGRectZero) ? CIGAMImagePreviewViewControllerTransitioningStyleFade : style;// zoom 类型一定需要有个非 zero 的 sourceImageRect，否则不知道动画的起点/终点，所以当不存在 sourceImageRect 时强制改为用 fade 动画
    ...
    CIGAMZoomImageView *zoomImageView = [self.imagePreviewViewController.imagePreviewView zoomImageViewAtIndex:self.imagePreviewViewController.imagePreviewView.currentImageIndex];
