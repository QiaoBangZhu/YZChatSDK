...
//
//  CIGAMImagePreviewViewController.m
//  cigam
//
...
const CGFloat CIGAMImagePreviewViewControllerCornerRadiusAutomaticDimension = -1;

@implementation CIGAMImagePreviewViewController (UIAppearance)
...
+ (void)initAppearance {
    CIGAMImagePreviewViewController.appearance.backgroundColor = UIColorBlack;
}

@end

@interface CIGAMImagePreviewViewController ()

@property(nonatomic, strong) UIPanGestureRecognizer *dismissingGesture;
@property(nonatomic, assign) CGPoint gestureBeganLocation;
@property(nonatomic, weak) CIGAMZoomImageView *gestureZoomImageView;
...
@implementation CIGAMImagePreviewViewController

- (void)didInitialize {
    [super didInitialize];
    self.automaticallyAdjustsScrollViewInsets = NO;
    
    self.sourceImageCornerRadius = CIGAMImagePreviewViewControllerCornerRadiusAutomaticDimension;
    ...
    // present style
    self.transitioningAnimator = [[CIGAMImagePreviewViewTransitionAnimator alloc] init];
...
- (CIGAMImagePreviewView *)imagePreviewView {
    if (!_imagePreviewView) {
        _imagePreviewView = [[CIGAMImagePreviewView alloc] initWithFrame:self.isViewLoaded ? self.view.bounds : CGRectZero];
    }
...
- (void)viewDidLayoutSubviews {
    ...
    self.imagePreviewView.cigam_frameApplyTransform = self.view.bounds;
    
    UIViewController *backendViewController = [self visibleViewControllerWithViewController:self.presentingViewController];
    self.canShowPresentingViewControllerWhenGesturing = [CIGAMHelper interfaceOrientationMask:backendViewController.supportedInterfaceOrientations containsInterfaceOrientation:UIApplication.sharedApplication.statusBarOrientation];
}

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    if (self.cigam_isPresented) {
...
- (void)setPresentingStyle:(CIGAMImagePreviewViewControllerTransitioningStyle)presentingStyle {
    _presentingStyle = presentingStyle;
    self.dismissingStyle = presentingStyle;
}

- (void)setTransitioningAnimator:(__kindof CIGAMImagePreviewViewTransitionAnimator *)transitioningAnimator {
    _transitioningAnimator = transitioningAnimator;
    transitioningAnimator.imagePreviewViewController = self;
}

- (BOOL)prefersStatusBarHidden {
    if (self.cigam_visibleState < CIGAMViewControllerDidAppear || self.cigam_visibleState >= CIGAMViewControllerDidDisappear) {
...
// 不使用 cigam_visibleViewControllerIfExist 是因为不想考虑 presentedViewController
