...
@class CIGAMImagePreviewView;
@class CIGAMCollectionViewPagingLayout;

typedef NS_ENUM (NSUInteger, CIGAMImagePreviewMediaType) {
    CIGAMImagePreviewMediaTypeImage,
    CIGAMImagePreviewMediaTypeLivePhoto,
    CIGAMImagePreviewMediaTypeVideo,
    CIGAMImagePreviewMediaTypeOthers
};

@protocol CIGAMImagePreviewViewDelegate <CIGAMZoomImageViewDelegate>
...
@optional
// 返回要展示的媒体资源的类型（图片、live photo、视频），如果不实现此方法，则 CIGAMImagePreviewView 将无法选择最合适的 cell 来复用从而略微增大系统开销
- (CIGAMImagePreviewMediaType)imagePreviewView:(CIGAMImagePreviewView *)imagePreviewView assetTypeAtIndex:(NSUInteger)index;

/**
 *  当左右的滚动停止时会触发这个方法
 *  @param  imagePreviewView 当前预览的 CIGAMImagePreviewView
 *  @param  index 当前滚动到的图片所在的索引
 */
- (void)imagePreviewView:(CIGAMImagePreviewView *)imagePreviewView didScrollToIndex:(NSUInteger)index;
 ...
 *  5. 由于 CIGAMImagePreviewViewDelegate 继承自 CIGAMZoomImageViewDelegate，所以若需要响应单击、双击、长按事件，请实现 CIGAMZoomImageViewDelegate 里的对应方法。
 *  6. 若需要从指定的某一张图片开始查看，可使用 currentImageIndex 属性。
 *
 *  @see CIGAMImagePreviewViewController
 */
@interface CIGAMImagePreviewView : UIView<UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>

@property(nonatomic, weak) id<CIGAMImagePreviewViewDelegate> delegate;
@property(nonatomic, strong, readonly) UICollectionView *collectionView;
@property(nonatomic, strong, readonly) CIGAMCollectionViewPagingLayout *collectionViewLayout;
...
@interface CIGAMImagePreviewView (CIGAMZoomImageView) <CIGAMZoomImageViewDelegate>

/**
 *  获取某个 CIGAMZoomImageView 所对应的 index
