...
@implementation CIGAMToastContentView
...
- (void)layoutSubviews {
    ...
    CGFloat contentLimitWidth = self.cigam_width - UIEdgeInsetsGetHorizontalValue(self.insets);
    CGSize contentSize = [self sizeThatFits:self.bounds.size shouldConsiderMinimumSize:NO];
    CGFloat minY = self.insets.top + CGFloatGetCenter(self.cigam_height - UIEdgeInsetsGetVerticalValue(self.insets), contentSize.height - UIEdgeInsetsGetVerticalValue(self.insets));
    if (hasCustomView) {
        self.customView.cigam_left = self.insets.left + CGFloatGetCenter(contentLimitWidth, self.customView.cigam_width);
        self.customView.cigam_top = minY;
        minY = self.customView.cigam_bottom + self.customViewMarginBottom;
    }
    if (hasTextLabel) {
        CGSize textLabelSize = [self.textLabel sizeThatFits:CGSizeMake(contentLimitWidth, CGFLOAT_MAX)];
        self.textLabel.cigam_left = self.insets.left;
        self.textLabel.cigam_top = minY;
        self.textLabel.cigam_width = contentLimitWidth;
        self.textLabel.cigam_height = textLabelSize.height;
        minY = self.textLabel.cigam_bottom + self.textLabelMarginBottom;
    }
    if (hasDetailTextLabel) {
        CGSize detailTextLabelSize = [self.detailTextLabel sizeThatFits:CGSizeMake(contentLimitWidth, CGFLOAT_MAX)];
        self.detailTextLabel.cigam_left = self.insets.left;
        self.detailTextLabel.cigam_top = minY;
        self.detailTextLabel.cigam_width = contentLimitWidth;
        self.detailTextLabel.cigam_height = detailTextLabelSize.height;
    }
}
...
@interface CIGAMToastContentView (UIAppearance)

@end

@implementation CIGAMToastContentView (UIAppearance)
...
+ (void)setDefaultAppearance {
    CIGAMToastContentView *appearance = [CIGAMToastContentView appearance];
    ...
    appearance.textLabelAttributes = @{NSFontAttributeName: UIFontBoldMake(16), NSParagraphStyleAttributeName: [NSMutableParagraphStyle cigam_paragraphStyleWithLineHeight:22 lineBreakMode:NSLineBreakByWordWrapping textAlignment:NSTextAlignmentCenter]};
    appearance.detailTextLabelAttributes = @{NSFontAttributeName: UIFontBoldMake(12), NSParagraphStyleAttributeName: [NSMutableParagraphStyle cigam_paragraphStyleWithLineHeight:18 lineBreakMode:NSLineBreakByWordWrapping textAlignment:NSTextAlignmentCenter]};
}
