...
typedef NS_ENUM(NSUInteger, CIGAMPopupContainerViewLayoutDirection) {
    CIGAMPopupContainerViewLayoutDirectionAbove,
    CIGAMPopupContainerViewLayoutDirectionBelow,
    CIGAMPopupContainerViewLayoutDirectionLeft,
    CIGAMPopupContainerViewLayoutDirectionRight
};
 ...
 * CIGAMPopupContainerView 支持以两种方式显示在界面上：
 * 1. 添加到某个 UIView 上（适合于 viewController 切换时浮层跟着一起切换的场景），这种场景只能手动隐藏浮层。
 * 2. 在 CIGAMPopupContainerView 自带的 UIWindow 里显示（适合于用完就消失的场景，不要涉及界面切换），这种场景支持点击空白地方自动隐藏浮层。
 *
 * 使用步骤：
 * 1. 调用 init 方法初始化。
 * 2. 选择一种显示方式：
 ...
 */
@interface CIGAMPopupContainerView : UIControl {
...
/// 计算布局时期望的默认位置，默认为CIGAMPopupContainerViewLayoutDirectionAbove，也即在目标的上方
@property(nonatomic, assign) CIGAMPopupContainerViewLayoutDirection preferLayoutDirection UI_APPEARANCE_SELECTOR;

/// 最终的布局方向（preferLayoutDirection只是期望的方向，但有可能那个方向已经没有剩余空间可摆放控件了，所以会自动变换）
@property(nonatomic, assign, readonly) CIGAMPopupContainerViewLayoutDirection currentLayoutDirection;
...
/// rect 需要处于 CIGAMPopupContainerView 所在的坐标系内，例如如果 popup 使用 addSubview: 的方式添加到界面，则 sourceRect 应该是 superview 坐标系内的；如果 popup 使用 window 的方式展示，则 sourceRect 需要转换为 window 坐标系内。
...
@interface CIGAMPopupContainerView (UISubclassingHooks)
