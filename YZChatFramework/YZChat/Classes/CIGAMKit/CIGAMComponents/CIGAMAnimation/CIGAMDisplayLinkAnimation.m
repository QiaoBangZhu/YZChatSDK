 ...
 */
//
//  CIGAMDisplayLinkAnimation.m
//  WeRead
//
//  Created by zhoonchen on 2018/9/3.
//

#import "CIGAMDisplayLinkAnimation.h"
#import "CIGAMCore.h"

@interface CIGAMDisplayLinkAnimation ()
...
@implementation CIGAMDisplayLinkAnimation
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.duration = 0;
        self.repeatCount = 0;
        self.easing = CIGAMAnimationEasingsLinear;
        self.timeOffset = 0;
        self.animation = nil;
    }
    return self;
...
- (instancetype)initWithDuration:(CFTimeInterval)duration
                          easing:(CIGAMAnimationEasings)easing
                      animations:(void (^)(CIGAMDisplayLinkAnimation *animation, CGFloat curTime))animations {
    if (self = [self init]) {
        self.duration = duration;
        self.easing = easing;
...
- (void)startAnimation {
    if (!self.displayLink) {
        NSAssert(NO, @"CIGAMDisplayLinkAnimation 使用错误，当前没有 CADisplayLink 对象，请查看头文件再试试。");
        return;
    }
...
- (void)handleDisplayLink:(CADisplayLink *)displayLink {
    ...
    if (self.animations) {
        ...
        id curValue = [CIGAMAnimationHelper interpolateFromValue:self.fromValue toValue:self.toValue time:time easing:self.easing];
...
@implementation CIGAMDisplayLinkAnimation (ConvenienceClassMethod)

+ (instancetype)springAnimateWithFromValue:(id)fromValue
                                   toValue:(id)toValue
                                 animation:(void (^)(id curValue))animation
                              createdBlock:(void (^)(CIGAMDisplayLinkAnimation *animation))createdBlock {
    return [self animateWithDuration:SpringAnimationDefaultDuration
                              easing:CIGAMAnimationEasingsSpringKeyboard
                           fromValue:fromValue
                             toValue:toValue
                           animation:animation
...
+ (instancetype)animateWithDuration:(NSTimeInterval)duration
                          ...
                          animation:(void (^)(id curValue))animation
                       createdBlock:(void (^)(CIGAMDisplayLinkAnimation *animation))createdBlock
                       didStopBlock:(void (^)(CIGAMDisplayLinkAnimation *animation))didStopBlock {
    CIGAMDisplayLinkAnimation *displayLinkAnimation = [[self alloc] initWithDuration:duration
                                                                                                 easing:easing
                                                                                              fromValue:fromValue
                                                                                                toValue:toValue
                                                                                              animation:animation];
    ...
    __weak CIGAMDisplayLinkAnimation *weakDisplayLinkAnimation = displayLinkAnimation;
...
+ (instancetype)springAnimateWithAnimations:(void (^)(CIGAMDisplayLinkAnimation *animation, CGFloat curTime))animations
                               createdBlock:(void (^)(CIGAMDisplayLinkAnimation *animation))createdBlock {
    return [self animateWithDuration:SpringAnimationDefaultDuration
                              easing:CIGAMAnimationEasingsSpringKeyboard
