 ...
 * @warning: ⚠️⚠️⚠️ 当动画是无限循环的时候，需要在某个时机去 stop 动画（例如 dealloc 里面），否则 `CIGAMDisplayLinkAnimation` 对象永远都不会释放，对应的 CADisplayLink 在后台都会被调用。
 */

@interface CIGAMDisplayLinkAnimation : NSObject
...
/// 动画曲线
@property(nonatomic, assign) CIGAMAnimationEasings easing;
...
@property(nonatomic, copy) void (^animations)(CIGAMDisplayLinkAnimation *animation, CGFloat curTime);
...
@interface CIGAMDisplayLinkAnimation (ConvenienceClassMethod)

/*
 * 这些类方法在动画执行之后会自动销毁 CIGAMDisplayLinkAnimation 对象，因为此时没有人持有这个对象（有个坑就是如果这个动画是无限循环的，那么就一直无法销毁，需要业务手动销毁）。如果想要持有对象以便在后续操作，可以把返回值保存到其他属性里面。
