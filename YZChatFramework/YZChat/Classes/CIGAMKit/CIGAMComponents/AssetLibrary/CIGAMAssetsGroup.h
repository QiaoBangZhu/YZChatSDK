...
/// 相册展示内容的类型
typedef NS_ENUM(NSUInteger, CIGAMAlbumContentType) {
    CIGAMAlbumContentTypeAll,                                  // 展示所有资源
    CIGAMAlbumContentTypeOnlyPhoto,                            // 只展示照片
    CIGAMAlbumContentTypeOnlyVideo,                            // 只展示视频
    CIGAMAlbumContentTypeOnlyAudio                             // 只展示音频
};

/// 相册展示内容按日期排序的方式
typedef NS_ENUM(NSUInteger, CIGAMAlbumSortType) {
    CIGAMAlbumSortTypePositive,  // 日期最新的内容排在后面
    CIGAMAlbumSortTypeReverse  // 日期最新的内容排在前面
};


@interface CIGAMAssetsGroup : NSObject
 ...
 *                          可以以此作为判断枚举结束的标记
 */
- (void)enumerateAssetsUsingBlock:(void (^)(CIGAMAsset *result))enumerationBlock;
