...
void CIGAMImageWriteToSavedPhotosAlbumWithAlbumAssetsGroup(UIImage *image, CIGAMAssetsGroup *albumAssetsGroup, CIGAMWriteAssetCompletionBlock completionBlock) {
    [[CIGAMAssetsManager sharedInstance] saveImageWithImageRef:image.CGImage albumAssetsGroup:albumAssetsGroup orientation:image.imageOrientation completionBlock:completionBlock];
}

void CIGAMSaveImageAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *imagePath, CIGAMAssetsGroup *albumAssetsGroup, CIGAMWriteAssetCompletionBlock completionBlock) {
    [[CIGAMAssetsManager sharedInstance] saveImageWithImagePathURL:[NSURL fileURLWithPath:imagePath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}

void CIGAMSaveVideoAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(NSString *videoPath, CIGAMAssetsGroup *albumAssetsGroup, CIGAMWriteAssetCompletionBlock completionBlock) {
    [[CIGAMAssetsManager sharedInstance] saveVideoWithVideoPathURL:[NSURL fileURLWithPath:videoPath] albumAssetsGroup:albumAssetsGroup completionBlock:completionBlock];
}



@implementation CIGAMAssetsManager {
    PHCachingImageManager *_phCachingImageManager;
}

+ (CIGAMAssetsManager *)sharedInstance {
    static dispatch_once_t onceToken;
    static CIGAMAssetsManager *instance = nil;
...
+ (CIGAMAssetAuthorizationStatus)authorizationStatus {
    __block CIGAMAssetAuthorizationStatus status;
    // 获取当前应用对照片的访问授权状态
    PHAuthorizationStatus authorizationStatus = [PHPhotoLibrary authorizationStatus];
    if (authorizationStatus == PHAuthorizationStatusRestricted || authorizationStatus == PHAuthorizationStatusDenied) {
        status = CIGAMAssetAuthorizationStatusNotAuthorized;
    } else if (authorizationStatus == PHAuthorizationStatusNotDetermined) {
        status = CIGAMAssetAuthorizationStatusNotDetermined;
    } else {
        status = CIGAMAssetAuthorizationStatusAuthorized;
    }
    return status;
}
...
+ (void)requestAuthorization:(void(^)(CIGAMAssetAuthorizationStatus status))handler {
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus phStatus) {
        CIGAMAssetAuthorizationStatus status;
...
- (void)enumerateAllAlbumsWithAlbumContentType:(CIGAMAlbumContentType)contentType showEmptyAlbum:(BOOL)showEmptyAlbum showSmartAlbumIfSupported:(BOOL)showSmartAlbumIfSupported usingBlock:(void (^)(CIGAMAssetsGroup *resultAssetsGroup))enumerationBlock {
    ...
    for (NSUInteger i = 0; i < tempAlbumsArray.count; i++) {
        ...
        CIGAMAssetsGroup *assetsGroup = [[CIGAMAssetsGroup alloc] initWithPHCollection:phAssetCollection fetchAssetsOptions:phFetchOptions];
        if (enumerationBlock) {
                                    ...
                                    albumAssetCollection:albumPhAssetCollection
                                       ...
                                       completionHandler:^(BOOL success, NSDate *creationDate, NSError *error) {
                                           if (success) {
                                               ...
                                               CIGAMLog(@"CIGAMAssetLibrary", @"Get PHAsset of image error: %@", error);
                                    ...
                                    albumAssetCollection:albumPhAssetCollection
                                       completionHandler:^(BOOL success, NSDate *creationDate, NSError *error) {
                                           if (success) {
                                               ...
                                               CIGAMLog(@"CIGAMAssetLibrary", @"Get PHAsset of video Error: %@", error);
                                               completionBlock(nil, error);
                                           }
                                       }];
}
...
- (void)cigam_addImageToAlbum:(UIImage *)image imagePathURL:(NSURL *)imagePathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
    ...
    [self performChanges:^{
        ...
        if (image) {
            ...
            CIGAMLog(@"CIGAMAssetLibrary", @"Creating asset with empty data");
            return;
        }
        ...
        if (!success) {
            CIGAMLog(@"CIGAMAssetLibrary", @"Creating asset of image error : %@", error);
        }
...
- (void)cigam_addVideoToAlbum:(NSURL *)videoPathURL albumAssetCollection:(PHAssetCollection *)albumAssetCollection completionHandler:(void(^)(BOOL success, NSDate *creationDate, NSError *error))completionHandler {
    ...
    [self performChanges:^{
        ...
        if (!success) {
            CIGAMLog(@"CIGAMAssetLibrary", @"Creating asset of video error: %@", error);
        }
