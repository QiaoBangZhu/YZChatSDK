...
typedef NS_ENUM(NSUInteger, CIGAMAssetType) {
    CIGAMAssetTypeUnknow,
    CIGAMAssetTypeImage,
    CIGAMAssetTypeVideo,
    CIGAMAssetTypeAudio
};

typedef NS_ENUM(NSUInteger, CIGAMAssetSubType) {
    CIGAMAssetSubTypeUnknow,
    CIGAMAssetSubTypeImage,
    CIGAMAssetSubTypeLivePhoto NS_ENUM_AVAILABLE_IOS(9_1),
    CIGAMAssetSubTypeGIF
};

/// Status when download asset from iCloud
typedef NS_ENUM(NSUInteger, CIGAMAssetDownloadStatus) {
    CIGAMAssetDownloadStatusSucceed,
    CIGAMAssetDownloadStatusDownloading,
    CIGAMAssetDownloadStatusCanceled,
    CIGAMAssetDownloadStatusFailed
};
 ...
 *  @note CIGAMAsset 重写了 isEqual: 方法，只要两个 CIGAMAsset 的 identifier 相同，则认为是同一个对象，以方便在数组、字典等容器中对大量 CIGAMAsset 进行遍历查找等操作。
 */
@interface CIGAMAsset : NSObject

@property(nonatomic, assign, readonly) CIGAMAssetType assetType;
@property(nonatomic, assign, readonly) CIGAMAssetSubType assetSubType;

- (instancetype)initWithPHAsset:(PHAsset *)phAsset;

@property(nonatomic, strong, readonly) PHAsset *phAsset;
@property(nonatomic, assign, readonly) CIGAMAssetDownloadStatus downloadStatus; // 从 iCloud 下载资源大图的状态
...
@property (nonatomic, copy, readonly) NSString *identifier;// Asset 的标识，每个 CIGAMAsset 的 identifier 都不同。只要两个 CIGAMAsset 的 identifier 相同则认为它们是同一个 asset
 ...
 *
 *  @param completion        完成请求后调用的 block，参数中包含了请求的 Live Photo 以及相关信息，若 assetType 不是 CIGAMAssetTypeLivePhoto 则为 nil
 *  @param phProgressHandler 处理请求进度的 handler，不在主线程上执行，在 block 中修改 UI 时注意需要手工放到主线程处理。
 *
 ...
 *
 *  @param completion        完成请求后调用的 block，参数中包含了请求的 AVPlayerItem 以及相关信息，若 assetType 不是 CIGAMAssetTypeVideo 则为 nil
 *  @param phProgressHandler 处理请求进度的 handler，不在主线程上执行，在 block 中修改 UI 时注意需要手工放到主线程处理。
 *
 ...
 *  异步请求图片的 Data
 *
 *  @param completion 完成请求后调用的 block，参数中包含了请求的图片 Data（若 assetType 不是 CIGAMAssetTypeImage 或 CIGAMAssetTypeLivePhoto 则为 nil），该图片是否为 GIF 的判断值，以及该图片的文件格式是否为 HEIC
 */
- (void)requestImageData:(void (^)(NSData *imageData, NSDictionary<NSString *, id> *info, BOOL isGIF, BOOL isHEIC))completion;

/**
 * 获取图片的 UIImageOrientation 值，仅 assetType 为 CIGAMAssetTypeImage 或 CIGAMAssetTypeLivePhoto 时有效
 */
