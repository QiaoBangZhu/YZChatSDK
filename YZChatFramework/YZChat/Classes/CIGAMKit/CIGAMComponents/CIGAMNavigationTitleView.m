...
@implementation CIGAMNavigationTitleView

#pragma mark - 初始化

- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithStyle:CIGAMNavigationTitleViewStyleDefault frame:frame];
}

- (instancetype)initWithStyle:(CIGAMNavigationTitleViewStyle)style {
    return [self initWithStyle:style frame:CGRectZero];
}

- (instancetype)initWithStyle:(CIGAMNavigationTitleViewStyle)style frame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.accessoryType = CIGAMNavigationTitleViewAccessoryTypeNone;
        
        [self cigam_applyAppearance];
        self.horizontalTitleFont = CIGAMNavigationTitleView.appearance.horizontalTitleFont ?: UINavigationBar.cigam_appearanceConfigured.titleTextAttributes[NSFontAttributeName];
        self.horizontalSubtitleFont = CIGAMNavigationTitleView.appearance.horizontalSubtitleFont ?: self.horizontalTitleFont;
        
        self.adjustsSubviewsTintColorAutomatically = YES;
        self.tintColor = CIGAMCMIActivated ? NavBarTitleColor : UINavigationBar.cigam_appearanceConfigured.titleTextAttributes[NSForegroundColorAttributeName];
    }
    return self;
}
...
- (CGSize)contentSize {
    ...
    if (self.style == CIGAMNavigationTitleViewStyleSubTitleVertical) {
...
- (void)setHorizontalTitleFont:(UIFont *)horizontalTitleFont {
    ...
    if (self.style == CIGAMNavigationTitleViewStyleDefault) {
...
- (void)setAccessoryType:(CIGAMNavigationTitleViewAccessoryType)accessoryType {
    
    // 如果已设置了accessoryView，则accessoryType不生效
    if (self.accessoryView) {
        accessoryType = CIGAMNavigationTitleViewAccessoryTypeNone;
    }
    
    _accessoryType = accessoryType;
    
    if (accessoryType == CIGAMNavigationTitleViewAccessoryTypeNone) {
    ...
    if (accessoryType == CIGAMNavigationTitleViewAccessoryTypeDisclosureIndicator) {
        accessoryImage = [NavBarAccessoryViewTypeDisclosureIndicatorImage cigam_imageWithOrientation:UIImageOrientationUp];
    }
...
- (void)updateSubAccessoryViewHidden {
    if (self.subAccessoryView && self.subtitleLabel.text.length && self.style == CIGAMNavigationTitleViewStyleSubTitleVertical) {
...
- (void)setActive:(BOOL)active {
    ...
    if (self.accessoryType == CIGAMNavigationTitleViewAccessoryTypeDisclosureIndicator) {
        if (active) {
            [UIView animateWithDuration:.25f delay:0 options:CIGAMViewAnimationOptionsCurveIn animations:^(void){
...
- (void)setStyle:(CIGAMNavigationTitleViewStyle)style {
    _style = style;
    if (style == CIGAMNavigationTitleViewStyleSubTitleVertical) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UINavigationBar class], @selector(layoutSubviews), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UINavigationBar *selfObject) {
                ...
                CIGAMNavigationTitleView *titleView = (CIGAMNavigationTitleView *)selfObject.topItem.titleView;
        ...
        // 让 -[UIViewController setTitle:] 可以自动刷新 CIGAMNavigationTitle
        OverrideImplementation([UIViewController class], @selector(setTitle:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIViewController *selfObject, NSString *title) {
                ...
                if ([selfObject.navigationItem.titleView isKindOfClass:CIGAMNavigationTitleView.class]) {
                    ((CIGAMNavigationTitleView *)selfObject.navigationItem.titleView).title = title;
                }
            };
        });
        
        // 让 -[UINavigationItem setTitle:] 可以自动刷新 CIGAMNavigationTitleView
        OverrideImplementation([UINavigationItem class], @selector(setTitle:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UINavigationItem *selfObject, NSString *title) {
                ...
                if ([selfObject.titleView isKindOfClass:CIGAMNavigationTitleView.class]) {
                    ((CIGAMNavigationTitleView *)selfObject.titleView).title = title;
                }
            };
        });
            ...
            return ^(UINavigationItem *selfObject, CIGAMNavigationTitleView *titleView) {
                ...
                if ([titleView isKindOfClass:CIGAMNavigationTitleView.class]) {
                    if (titleView.title.length <= 0) {
                        NSString *title = selfObject.cigam_viewController.title ?: selfObject.title;
...
@interface CIGAMNavigationTitleView (UIAppearance)

@end

@implementation CIGAMNavigationTitleView (UIAppearance)
...
+ (void)setDefaultAppearance {
    CIGAMNavigationTitleView *appearance = [CIGAMNavigationTitleView appearance];
...
@implementation CIGAMNavigationTitleView (LargeTitleCompatibility)

- (void)setAlpha:(BOOL)alpha animated:(BOOL)animated {
    // 在 push 和 pop 过渡期间系统会对自定义的 titleView 的 alpha 进行调整，了避免和系统的设置冲突（比如设置 alpha 为 0 又被系统还原为 1）这里通过设置 contentView 的 alpha 来控制整个 CIGAMNavigationTitleView 显示与隐藏。
    [UIView cigam_animateWithAnimated:animated duration:0.25f animations:^{
        ...
        if (@available(iOS 11.0, *)) {
            OverrideImplementation([UINavigationController class], sel_registerName("_updateTopViewFramesToMatchScrollOffsetInViewController:contentScrollView:topLayoutType:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UINavigationController *selfObject, UIViewController *viewController, UIScrollView *scrollView, NSUInteger topLayoutType) {
                    ...
                    [selfObject cigam_updateTitleViewToMatchScrollOffsetInViewController:viewController contentScrollView:scrollView topLayoutType:topLayoutType];
                };
            });
        }
    });
...
- (void)cigam_updateTitleViewToMatchScrollOffsetInViewController:(UIViewController *)viewController contentScrollView:(UIScrollView *)contentScrollView topLayoutType:(NSInteger)topLayoutType {
    if (@available(iOS 11.0, *)) {
        UIView *titleView = viewController.navigationItem.titleView;
        if (!titleView || ![titleView isKindOfClass:[CIGAMNavigationTitleView class]]) {
            return;
        }
        
        NSAssert(viewController.navigationController == self, @"navigationController is nil");
        
        CIGAMNavigationTitleView *navigationTitleView = (CIGAMNavigationTitleView *)titleView;
        UIView *largeTitleView = self.navigationBar.cigam_largeTitleView;
        BOOL largeTitleLabelVisable = self.navigationBar.prefersLargeTitles && viewController.cigam_prefersLargeTitleDisplayed && largeTitleView.alpha != 0;
