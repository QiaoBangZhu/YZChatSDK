...
- (void)resetClassNameIfNeeded {
    if ([self.parentObject isKindOfClass:CALayer.class] || [self.parentObject isKindOfClass:CAAnimation.class]) {
        ...
        // 这里设置一个 CIGAMMultipleDelegates 的 subClass，其 className 由所有 delegate className 拼接而成。
        NSMutableString *className = [NSMutableString stringWithString:NSStringFromClass(CIGAMMultipleDelegates.class)];
        ...
        if (!class) {
            class = objc_allocateClassPair(CIGAMMultipleDelegates.class, className.UTF8String, 0);
...
- (BOOL)removeDelegate:(id)delegate {
    NSUInteger index = [self.delegates cigam_indexOfPointer:(__bridge void *)delegate];
    if (index != NSNotFound) {
...
- (BOOL)containsDelegate:(id)delegate {
    return [self.delegates cigam_containsPointer:(__bridge void *)delegate];
}
...
- (void)forwardInvocation:(NSInvocation *)anInvocation {
    ...
    // RAC 那边会把相同的 invocation 传回来 CIGAMMultipleDelegates，引发死循环，所以这里做了个屏蔽
    // https://github.com/Tencent/CIGAM_iOS/issues/970
    ...
    for (id delegate in delegates) {
        if ([delegate respondsToSelector:selector]) {
            // 当前 delegate 的实现可能再次调用原始 delegate 的实现，如果原始 delegate 是 CIGAMMultipleDelegates 就会造成死循环，所以要做 2 事：
            // 1、检测到循环就打破
...
- (BOOL)respondsToSelector:(SEL)aSelector {
    ...
    for (id delegate in delegates) {
        ...
        // 对 CIGAMMultipleDelegates 额外处理的解释在这里：https://github.com/Tencent/CIGAM_iOS/issues/357
        BOOL delegateCanRespondToSelector;
        if ([delegate isProxy] || [delegate isKindOfClass:CIGAMMultipleDelegates.class]) {
...
- (id)valueForKey:(NSString *)key {
    NSPointerArray *delegates = [self.delegates copy];
    for (id delegate in delegates) {
        if ([delegate cigam_canGetValueForKey:key]) {
...
- (void)setValue:(id)value forKey:(NSString *)key {
    NSPointerArray *delegates = [self.delegates copy];
    for (id delegate in delegates) {
        if ([delegate cigam_canSetValueForKey:key]) {
