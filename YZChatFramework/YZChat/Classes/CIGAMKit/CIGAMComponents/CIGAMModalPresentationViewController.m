...
//
//  CIGAMModalPresentationViewController.m
//  cigam
//
//  Created by CIGAM Team on 16/7/6.
//
...
@property(nonatomic, weak, readwrite) CIGAMModalPresentationViewController *cigam_modalPresentationViewController;
@end

@implementation CIGAMModalPresentationViewController (UIAppearance)
...
+ (void)initAppearance {
    CIGAMModalPresentationViewController *appearance = CIGAMModalPresentationViewController.appearance;
    appearance.animationStyle = CIGAMModalPresentationAnimationStyleFade;
...
@interface CIGAMModalPresentationViewController ()<CIGAMKeyboardManagerDelegate>

@property(nonatomic, strong) CIGAMModalPresentationWindow *containerWindow;
...
/// https://github.com/Tencent/CIGAM_iOS/issues/639
...
@implementation CIGAMModalPresentationViewController
...
- (void)didInitialize {
    ...
    UIViewController *visibleViewController = [CIGAMHelper visibleViewController];
...
- (void)viewDidLayoutSubviews {
    ...
    if (self.layoutBlock) {
        ...
        self.contentView.cigam_frameApplyTransform = contentViewFrame;
    }
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.isShownInWindowMode) {
        [CIGAMHelper dimmedApplicationWindow];
    }
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (self.isShownInWindowMode) {
        [CIGAMHelper resetDimmedApplicationWindow];
    }
    
    void (^didHiddenCompletion)(BOOL finished) = ^(BOOL finished) {
        
        if (self.shownInWindowMode) {
            // 恢复 keyWindow 之前做一下检查，避免这个问题 https://github.com/Tencent/CIGAM_iOS/issues/90
            if (UIApplication.sharedApplication.keyWindow == self.containerWindow) {
                if (self.previousKeyWindow.hidden) {
                    // 保护了这个 issue 记录的情况，避免主 window 丢失 keyWindow https://github.com/Tencent/CIGAM_iOS/issues/315
...
- (void)handleDimmingViewTapGestureRecognizer:(UITapGestureRecognizer *)tapGestureRecognizer {
    ...
    if (self.shownInWindowMode) {
        ...
        // 这里仅屏蔽点击遮罩时的 dismiss，如果是代码手动调用 dismiss 的，在 UIViewController(CIGAMModalPresentationViewController) 里会通过重写 dismiss 方法来屏蔽。
        // 为什么不能统一交给 UIViewController(CIGAMModalPresentationViewController) 里屏蔽，是因为点击遮罩触发的 dismiss 要调用 willHideByDimmingViewTappedBlock，而 UIViewController 那边不知道此次 dismiss 是否由点击遮罩触发的，所以分开两边写。
...
- (void)setContentViewController:(UIViewController<CIGAMModalPresentationContentViewControllerProtocol> *)contentViewController {
    if (![contentViewController isEqual:_contentViewController]) {
        _contentViewController.cigam_modalPresentationViewController = nil;
    }
    contentViewController.cigam_modalPresentationViewController = self;
...
- (void)showingAnimationWithCompletion:(void (^)(BOOL))completion {
    if (self.animationStyle == CIGAMModalPresentationAnimationStyleFade) {
        self.dimmingView.alpha = 0.0;
        self.contentView.alpha = 0.0;
        [UIView animateWithDuration:.2 delay:0.0 options:CIGAMViewAnimationOptionsCurveOut animations:^{
    ...
    } else if (self.animationStyle == CIGAMModalPresentationAnimationStylePopup) {
        self.dimmingView.alpha = 0.0;
        self.contentView.transform = CGAffineTransformMakeScale(0, 0);
        [UIView animateWithDuration:.3 delay:0.0 options:CIGAMViewAnimationOptionsCurveOut animations:^{
            self.dimmingView.alpha = 1.0;
    ...
    } else if (self.animationStyle == CIGAMModalPresentationAnimationStyleSlide) {
        self.dimmingView.alpha = 0.0;
...
- (void)showWithAnimated:(BOOL)animated completion:(void (^)(BOOL))completion {
    ...
    if (!self.containerWindow) {
        self.containerWindow = [[CIGAMModalPresentationWindow alloc] init];
        self.containerWindow.windowLevel = UIWindowLevelCIGAMAlertView;
...
- (void)updateContainerWindowStatusBarCapture {
    ...
    // 当以 window 的方式显示浮层时，状态栏交给 CIGAMModalPresentationViewController 控制
    self.containerWindow.cigam_capturesStatusBarAppearance = self.cigam_prefersStatusBarHiddenBlock || self.cigam_preferredStatusBarStyleBlock;
    if (self.containerWindow.cigam_capturesStatusBarAppearance) {
        [self setNeedsStatusBarAppearanceUpdate];
    }
}

// 当以 present 方式显示浮层时，状态栏允许由 contentViewController 控制，但 CIGAMModalPresentationViewController 的 cigam_prefersStatusBarHiddenBlock/cigam_preferredStatusBarStyleBlock 优先级会更高
...
@implementation CIGAMModalPresentationViewController (Manager)

+ (BOOL)isAnyModalPresentationViewControllerVisible {
    for (UIWindow *window in UIApplication.sharedApplication.windows) {
        if ([window isKindOfClass:[CIGAMModalPresentationWindow class]] && !window.hidden) {
            return YES;
        }
    }
    return NO;
...
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan {
    ...
    for (UIWindow *window in UIApplication.sharedApplication.windows) {
        if (![window isKindOfClass:[CIGAMModalPresentationWindow class]]) {
            continue;
        }
        ...
        CIGAMModalPresentationViewController *modalViewController = (CIGAMModalPresentationViewController *)window.rootViewController;
        BOOL canHide = YES;
        ...
        if (canHide) {
            // 如果某些控件的显隐能力是通过 CIGAMModalPresentationViewController 实现的，那么隐藏它们时，应该用它们自己的 hide 方法，而不是 CIGAMModalPresentationViewController 自带的 hideWithAnimated:completion:
            id<CIGAMModalPresentationComponentProtocol> modalPresentationComponent = nil;
            if ([modalViewController.contentViewController conformsToProtocol:@protocol(CIGAMModalPresentationComponentProtocol)]) {
                modalPresentationComponent = (id<CIGAMModalPresentationComponentProtocol>)modalViewController.contentViewController;
            } else if ([modalViewController.contentView conformsToProtocol:@protocol(CIGAMModalPresentationComponentProtocol)]) {
                modalPresentationComponent = (id<CIGAMModalPresentationComponentProtocol>)modalViewController.contentView;
            }
...
@implementation CIGAMModalPresentationWindow

- (void)layoutSubviews {
    [super layoutSubviews];
    // 避免来电状态时只 modal 的遮罩只盖住一部分的状态栏
    // 但在 iOS 13 及以后，来电状态下状态栏的高度不会再变化了
    // https://github.com/Tencent/CIGAM_iOS/issues/375
...
@implementation UIViewController (CIGAMModalPresentationViewController)

CIGAMSynthesizeIdWeakProperty(cigam_modalPresentationViewController, setCigam_modalPresentationViewController)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UIViewController class], @selector(dismissViewControllerAnimated:completion:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIViewController *selfObject, BOOL firstArgv, id secondArgv) {
                ...
                CIGAMModalPresentationViewController *modal = nil;
                if ([selfObject.presentedViewController isKindOfClass:CIGAMModalPresentationViewController.class]) {
                    modal = (CIGAMModalPresentationViewController *)selfObject.presentedViewController;
                } else if ([selfObject isKindOfClass:CIGAMModalPresentationViewController.class] && !selfObject.presentedViewController && selfObject.presentingViewController.presentedViewController == selfObject) {
                    modal = (CIGAMModalPresentationViewController *)selfObject;
                }
