...
@class CIGAMAlertController;


typedef NS_ENUM(NSInteger, CIGAMAlertActionStyle) {
    CIGAMAlertActionStyleDefault = 0,
    CIGAMAlertActionStyleCancel,
    CIGAMAlertActionStyleDestructive
};

typedef NS_ENUM(NSInteger, CIGAMAlertControllerStyle) {
    CIGAMAlertControllerStyleActionSheet = 0,
    CIGAMAlertControllerStyleAlert
};


@protocol CIGAMAlertControllerDelegate <NSObject>

@optional

- (void)willShowAlertController:(CIGAMAlertController *)alertController;
- (void)willHideAlertController:(CIGAMAlertController *)alertController;
- (void)didShowAlertController:(CIGAMAlertController *)alertController;
- (void)didHideAlertController:(CIGAMAlertController *)alertController;
- (BOOL)shouldHideAlertController:(CIGAMAlertController *)alertController;

@end


/**
 *  CIGAMAlertController的按钮，初始化完通过`CIGAMAlertController`的`addAction:`方法添加到 AlertController 上即可。
 */
@interface CIGAMAlertAction : NSObject

/**
 *  初始化`CIGAMAlertController`的按钮
 *
 *  @param title   按钮标题
 *  @param style   按钮style，跟系统一样，有 Default、Cancel、Destructive 三种类型
 *  @param handler 处理点击事件的block，注意 CIGAMAlertAction 点击后必定会隐藏 alertController，不需要手动在 handler 里 hide
 *
 *  @return CIGAMAlertController按钮的实例
 */
+ (instancetype)actionWithTitle:(nullable NSString *)title style:(CIGAMAlertActionStyle)style handler:(nullable void (^)(__kindof CIGAMAlertController *aAlertController, CIGAMAlertAction *action))handler;

/// `CIGAMAlertAction`对应的 button 对象
@property(nonatomic, strong, readonly) CIGAMButton *button;

/// `CIGAMAlertAction`对应的标题
@property(nullable, nonatomic, copy, readonly) NSString *title;

/// `CIGAMAlertAction`对应的样式
@property(nonatomic, assign, readonly) CIGAMAlertActionStyle style;

/// `CIGAMAlertAction`是否允许操作
@property(nonatomic, assign, getter=isEnabled) BOOL enabled;

/// `CIGAMAlertAction`按钮样式，默认nil。当此值为nil的时候，则使用`CIGAMAlertController`的`alertButtonAttributes`或者`sheetButtonAttributes`的值。
...
/**
 *  `CIGAMAlertController`是模仿系统`UIAlertController`的控件，所以系统有的功能在CIGAMAlertController里面基本都有。同时`CIGAMAlertController`还提供了一些扩展功能，例如：它的每个 button 都是开放出来的，可以对默认的按钮进行二次处理（比如加一个图片）；可以通过 appearance 在 app 启动的时候修改整个`CIGAMAlertController`的主题样式。
 */
@interface CIGAMAlertController : UIViewController<CIGAMModalPresentationComponentProtocol> {
...
/// alert标题样式，默认@{NSForegroundColorAttributeName:UIColorBlack,NSFontAttributeName:UIFontBoldMake(17),NSParagraphStyleAttributeName:[NSMutableParagraphStyle cigam_paragraphStyleWithLineHeight:0 lineBreakMode:NSLineBreakByTruncatingTail]}
@property(nullable, nonatomic, strong) NSDictionary<NSAttributedStringKey, id> *alertTitleAttributes UI_APPEARANCE_SELECTOR;

/// alert信息样式，默认@{NSForegroundColorAttributeName:UIColorBlack,NSFontAttributeName:UIFontMake(13),NSParagraphStyleAttributeName:[NSMutableParagraphStyle cigam_paragraphStyleWithLineHeight:0 lineBreakMode:NSLineBreakByTruncatingTail]}
...
@property(nonatomic, copy) UIEdgeInsets (^alertTextFieldMarginBlock)(__kindof CIGAMAlertController *aAlertController, NSInteger aTextFieldIndex);
...
/// sheet标题样式，默认@{NSForegroundColorAttributeName:UIColorMake(143, 143, 143),NSFontAttributeName:UIFontBoldMake(13),NSParagraphStyleAttributeName:[NSMutableParagraphStyle cigam_paragraphStyleWithLineHeight:0 lineBreakMode:NSLineBreakByTruncatingTail]}
@property(nullable, nonatomic, copy) NSDictionary<NSAttributedStringKey, id> *sheetTitleAttributes UI_APPEARANCE_SELECTOR;

/// sheet信息样式，默认@{NSForegroundColorAttributeName:UIColorMake(143, 143, 143),NSFontAttributeName:UIFontMake(13),NSParagraphStyleAttributeName:[NSMutableParagraphStyle cigam_paragraphStyleWithLineHeight:0 lineBreakMode:NSLineBreakByTruncatingTail]}
...
/// @see `CIGAMAlertControllerDelegate`
@property(nullable, nonatomic,weak) id<CIGAMAlertControllerDelegate>delegate;
...
/// 增加一个输入框
- (void)addTextFieldWithConfigurationHandler:(void (^_Nullable)(CIGAMTextField *textField))configurationHandler;
...
/// 增加一个自定义的view作为`CIGAMAlertController`的customView
- (void)addCustomView:(UIView *_Nullable)view;

/// 显示`CIGAMAlertController`
- (void)showWithAnimated:(BOOL)animated;

/// 隐藏`CIGAMAlertController`
- (void)hideWithAnimated:(BOOL)animated;

/// 所有`CIGAMAlertAction`对象
@property(nullable, nonatomic, copy, readonly) NSArray <CIGAMAlertAction *> *actions;

/// 当前所有通过`addTextFieldWithConfigurationHandler:`接口添加的输入框
@property(nullable, nonatomic, copy, readonly) NSArray <CIGAMTextField *> *textFields;

/// 设置自定义view。通过`addCustomView:`方法添加一个自定义的view，`CIGAMAlertController`会在布局的时候去调用这个view的`sizeThatFits:`方法来获取size，至于x和y坐标则由控件自己控制。
...
/// 当前样式style
@property(nonatomic, assign, readonly) CIGAMAlertControllerStyle preferredStyle;

/// 将`CIGAMAlertController`弹出来的`CIGAMModalPresentationViewController`对象
@property(nullable, nonatomic, strong, readonly) CIGAMModalPresentationViewController *modalPresentationViewController;
...
/**
 *  设置按钮的排序是否要由用户添加的顺序来决定，默认为NO，也即与系统原生`UIAlertController`一致，CIGAMAlertActionStyleDestructive 类型的action必定在最后面。
 *
 *  @warning 注意 CIGAMAlertActionStyleCancel 按钮不受这个属性的影响
 */
...
@interface CIGAMAlertController (UIAppearance)
...
@interface CIGAMAlertController (Manager)
