...
//
//  CIGAMCollectionViewPagingLayout.m
//  cigam
//
//  Created by CIGAM Team on 15/9/24.
//
...
@interface CIGAMCollectionViewPagingLayout () {
...
@implementation CIGAMCollectionViewPagingLayout (DefaultStyle)
...
- (void)setDebug:(BOOL)debug {
    _debug = debug;
    if (self.style == CIGAMCollectionViewPagingLayoutStyleDefault && debug && !self.debugLayer) {
        self.debugLayer = [CALayer layer];
        [self.debugLayer cigam_removeDefaultAnimations];
...
@implementation CIGAMCollectionViewPagingLayout (ScaleStyle)
...
const CGFloat CIGAMCollectionViewPagingLayoutRotationRadiusAutomatic = -1.0;

@implementation CIGAMCollectionViewPagingLayout (RotationStyle)
...
@implementation CIGAMCollectionViewPagingLayout

- (instancetype)initWithStyle:(CIGAMCollectionViewPagingLayoutStyle)style {
    if (self = [super init]) {
        _style = style;
        ...
        self.rotationRadius = CIGAMCollectionViewPagingLayoutRotationRadiusAutomatic;
...
- (instancetype)init {
    return [self initWithStyle:CIGAMCollectionViewPagingLayoutStyleDefault];
}
...
- (void)prepareLayout {
    ...
    if (self.debugLayer) {
        if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
            self.debugLayer.frame = CGRectFlatMake(0, self.collectionView.cigam_contentInset.top + self.sectionInset.top + _finalItemSize.height / 2, CGRectGetWidth(self.collectionView.bounds), PixelOne);
        } else {
            self.debugLayer.frame = CGRectFlatMake(self.collectionView.cigam_contentInset.left + self.sectionInset.left + _finalItemSize.width / 2, 0, PixelOne, CGRectGetHeight(self.collectionView.bounds));
        }
    }
}

- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    if (self.style == CIGAMCollectionViewPagingLayoutStyleScale || self.style == CIGAMCollectionViewPagingLayoutStyleRotation) {
        return YES;
    }
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (self.style == CIGAMCollectionViewPagingLayoutStyleDefault) {
    ...
    if (self.style == CIGAMCollectionViewPagingLayoutStyleScale) {
    ...
    if (self.style == CIGAMCollectionViewPagingLayoutStyleRotation) {
        if (self.rotationRadius == CIGAMCollectionViewPagingLayoutRotationRadiusAutomatic) {
...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity {
    ...
    UIEdgeInsets contentInset = self.collectionView.cigam_contentInset;
