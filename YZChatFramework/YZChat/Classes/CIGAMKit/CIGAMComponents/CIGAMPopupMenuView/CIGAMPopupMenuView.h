 ...
 *
 *  注意，CIGAMPopupMenuView 的大小默认是按内容自适应的（item 的 sizeThatFits），但同时又受 maximumWidth/minimumWidth 的限制。
 */
@interface CIGAMPopupMenuView : CIGAMPopupContainerView
...
/// 批量设置 item 的样式
@property(nonatomic, copy, nullable) void (^itemConfigurationHandler)(CIGAMPopupMenuView *aMenuView, __kindof CIGAMPopupMenuBaseItem *aItem, NSInteger section, NSInteger index);

/// 设置 item，均处于同一个 section 内
@property(nonatomic, copy, nullable) NSArray<__kindof CIGAMPopupMenuBaseItem *> *items;

/// 设置多个 section 的多个 item
@property(nonatomic, copy, nullable) NSArray<NSArray<__kindof CIGAMPopupMenuBaseItem *> *> *itemSections;
