...
@protocol CIGAMKeyboardManagerDelegate;
@class CIGAMKeyboardUserInfo;

/**
 *  `CIGAMKeyboardManager` 提供了方便、稳定的管理键盘事件的方案，使用的场景是需要跟随键盘的显示或者隐藏来更改界面的 UI，例如输入框跟随在键盘的顶部。
 *  由于键盘通知是整个 App 全局的，所以经常会遇到 A 的键盘监听回调里接收到 B 的键盘事件，这样的情况往往不是我们想要的，即使可以通过判断当前的 firstResponder 来区分，但还是不能完美的解决问题或者有时候解决起来非常麻烦。`CIGAMKeyboardManager` 通过 `delegateEnabled` 和 `targetResponder` 等属性来方便地控制 firstResponder，从而可以实现某个键盘监听回调方法只响应某个 UIResponder 或者某几个 UIResponder 触发的键盘通知。
 *  另外系统的“设置→辅助功能→动态效果→减弱动态效果→首选交叉淡出过渡效果”会改变系统键盘的动画，CIGAMKeyboardManager 也兼容了这种情况，如果业务自己处理，很容易会遗漏。
 *  
 *  使用方式：
 ...
 *
 *  另外 CIGAMKeyboardManager 同时集成在了 UITextField(CIGAM) 和 UITextView(CIGAM) 里，具体请查看对应文件。
 *  @see UITextField(CIGAM)
 *  @see UITextView(CIGAM)
 */
@interface CIGAMKeyboardManager : NSObject

/**
 *  指定初始化方法，以 delegate 的方式将键盘事件传递给监听者
 */
- (instancetype)initWithDelegate:(id<CIGAMKeyboardManagerDelegate>)delegate NS_DESIGNATED_INITIALIZER;

/**
 *  获取当前的 delegate
 */
@property(nonatomic, weak, readonly) id<CIGAMKeyboardManagerDelegate> delegate;
...
@interface CIGAMKeyboardUserInfo : NSObject

/**
 *  所在的KeyboardManager
 */
@property(nonatomic, weak, readonly) CIGAMKeyboardManager *keyboardManager;
...
/**
 *  `CIGAMKeyboardManagerDelegate`里面的方法是对应系统键盘通知的回调方法，具体请看delegate名字，`CIGAMKeyboardUserInfo`是对系统的userInfo做了一个封装，可以方便的获取userInfo的属性值。
 */
@protocol CIGAMKeyboardManagerDelegate <NSObject>
...
/// 持有KeyboardManager对象
@property(nonatomic, strong) CIGAMKeyboardManager *cigam_keyboardManager;

@end

@interface UITextField (CIGAM_KeyboardManager)

/// 键盘相关block，搭配CIGAMKeyboardManager一起使用

@property(nonatomic, copy) void (^cigam_keyboardWillShowNotificationBlock)(CIGAMKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^cigam_keyboardWillHideNotificationBlock)(CIGAMKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^cigam_keyboardWillChangeFrameNotificationBlock)(CIGAMKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^cigam_keyboardDidShowNotificationBlock)(CIGAMKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^cigam_keyboardDidHideNotificationBlock)(CIGAMKeyboardUserInfo *keyboardUserInfo);
@property(nonatomic, copy) void (^cigam_keyboardDidChangeFrameNotificationBlock)(CIGAMKeyboardUserInfo *keyboardUserInfo);

@end

@interface UITextView (CIGAM_KeyboardManager)
