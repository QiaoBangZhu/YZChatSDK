...
//
//  CIGAMEmotionInputManager.h
//  cigam
//
//  Created by CIGAM Team on 16/9/8.
//

#import <Foundation/Foundation.h>
#import <UIKit/UIKit.h>

@class CIGAMEmotionView;

/**
 *  提供一个常见的通用表情面板，能为绑定的`UITextField`或`UITextView`提供表情的相关功能，包括点击表情输入对应的表情名字、点击删除按钮删除表情。
 *  使用方式：
 *  1. 使用 init 方法初始化。
 *  2. 通过 `boundTextField` 或 `boundTextView` 关联一个输入框，建议这些输入框使用 `CIGAMTextField` 或 `CIGAMTextView`，原因看下面的 warning。
 *  3. 将所有表情通过 `self.emotionView.emotions` 设置进去，注意这个数组里的所有 `CIGAMEmotion` 的 `displayName` 都应该使用左右标识符包裹起来（例如中括号“[]”），并且所有表情的左右标识符都应该保持一致。
 *  4. 将 `self.emotionView` add 到界面上即可。
 *
 *  @warning 一个`CIGAMEmotionInputManager`无法同时绑定`boundTextField`和`boundTextView`，在两者都绑定的情况下，优先使用`boundTextField`。
 *  @warning 由于`CIGAMEmotionInputManager`里面多个地方会调用`boundTextView.text`，而`setText:`并不会触发`UITextViewDelegate`的`textViewDidChange:`或`UITextViewTextDidChangeNotification`，以及 `UITextField` 的 `UIControlEventEditingChanged` 事件，从而在刷新表情面板里的发送按钮的enabled状态时可能不及时，所以推荐使用 `CIGAMTextView` 代替 `UITextView`、用 `CIGAMTextField` 代替 `UITextField`，并确保它们的`shouldResponseToProgrammaticallyTextChanges`属性是 `YES`（默认即为 `YES`）。
 *  @warning 由于表情的插入、删除都会受当前输入框的光标所在位置的影响，所以请在适当的时机更新`selectedRangeForBoundTextInput`的值，具体情况请查看该属性的注释。
 */
@interface CIGAMEmotionInputManager : NSObject
...
/**
 *  表情面板，已被设置了默认的`didSelectEmotionBlock`和`didSelectDeleteButtonBlock`，在`CIGAMEmotionInputManager`初始化完后，即可将`emotionView`添加到界面上。
 */
@property(nonatomic, strong, readonly) CIGAMEmotionView *emotionView;
...
/**
 *  在 `UITextViewDelegate` 的 `textView:shouldChangeTextInRange:replacementText:` 或者 `CIGAMTextFieldDelegate` 的 `textField:shouldChangeTextInRange:replacementText:` 方法里调用，根据返回值来决定是否应该调用 `deleteEmotionDisplayNameAtCurrentSelectedRangeForce:`
