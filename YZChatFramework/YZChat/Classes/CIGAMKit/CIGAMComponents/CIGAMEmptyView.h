...
@protocol CIGAMEmptyViewLoadingViewProtocol <NSObject>
...
/**
 *  通用的空界面控件，支持显示 loading、标题和副标题提示语、占位图片，CIGAMCommonViewController 内已集成一个 emptyView，无需额外添加。
 */
@interface CIGAMEmptyView : UIView

// 布局顺序从上到下依次为：imageView, loadingView, textLabel, detailTextLabel, actionButton
@property(nonatomic, strong) UIView<CIGAMEmptyViewLoadingViewProtocol> *loadingView;   // 此控件通过设置 loadingView.hidden 来控制 loadinView 的显示和隐藏，因此请确保你的loadingView 没有类似于 hidesWhenStopped = YES 之类会使 view.hidden 失效的属性
@property(nonatomic, strong, readonly) UIImageView *imageView;
@property(nonatomic, strong, readonly) UILabel *textLabel;
@property(nonatomic, strong, readonly) UILabel *detailTextLabel;
@property(nonatomic, strong, readonly) CIGAMButton *actionButton;
...
/**
 *  如果要继承CIGAMEmptyView并添加新的子 view，则必须：
