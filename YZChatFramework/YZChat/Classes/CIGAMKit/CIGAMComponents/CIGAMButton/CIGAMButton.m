...
#import "UIButton+CIGAM.h"

const CGFloat CIGAMButtonCornerRadiusAdjustsBounds = -1;

@interface CIGAMButton ()

@property(nonatomic, strong) CALayer *highlightedBackgroundLayer;
@property(nonatomic, strong) UIColor *originBorderColor;
@end

@implementation CIGAMButton
...
- (void)didInitialize {
    ...
    self.imagePosition = CIGAMButtonImagePositionLeft;
}

// 系统访问 self.imageView 会触发 layout，而私有方法 _imageView 则是简单地访问 imageView，所以在 CIGAMButton layoutSubviews 里应该用这个方法
// https://github.com/Tencent/CIGAM_iOS/issues/1051
- (UIImageView *)_cigam_imageView {
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case CIGAMButtonImagePositionTop:
        case CIGAMButtonImagePositionBottom: {
            ...
            if (isImageViewShowing) {
                ...
                imageSize.width = MIN(imageSize.width, imageLimitWidth);// CIGAMButton sizeThatFits 时 self._imageView 为 nil 但 self.imageView 有值，而开启了 Bold Text 时，系统的 self.imageView sizeThatFits 返回值会比没开启 BoldText 时多 1pt（不知道为什么文字加粗与否会影响 imageView...），从而保证开启 Bold Text 后文字依然能完整展示出来，所以这里应该用 self.imageView 而不是 self._imageView
        ...
        case CIGAMButtonImagePositionLeft:
        case CIGAMButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 CIGAMButtonImagePositionLeft 并且titleLabel 多行的情况下，CIGAMButton 计算的结果与系统不一致
            ...
            if (isImageViewShowing) {
                ...
                imageSize.height = MIN(imageSize.height, imageLimitHeight);// CIGAMButton sizeThatFits 时 self._imageView 为 nil 但 self.imageView 有值，而开启了 Bold Text 时，系统的 self.imageView sizeThatFits 返回值会比没开启 BoldText 时多 1pt（不知道为什么文字加粗与否会影响 imageView...），从而保证开启 Bold Text 后文字依然能完整展示出来，所以这里应该用 self.imageView 而不是 self._imageView
                imageTotalSize = CGSizeMake(imageSize.width + UIEdgeInsetsGetHorizontalValue(self.imageEdgeInsets), imageSize.height + UIEdgeInsetsGetVerticalValue(self.imageEdgeInsets));
            }
...
- (void)layoutSubviews {
    ...
    if (self.cornerRadius == CIGAMButtonCornerRadiusAdjustsBounds) {
    ...
    if (isImageViewShowing) {
        ...
        CGSize imageSize = self._cigam_imageView.image ? [self._cigam_imageView sizeThatFits:imageLimitSize] : self.currentImage.size;
    ...
    // https://github.com/Tencent/CIGAM_iOS/issues/1012
    ...
    if (isImageViewShowing) {
        makesureBoundsPositive(self._cigam_imageView);
    }
    if (isTitleLabelShowing) {
        makesureBoundsPositive(self.titleLabel);
    }
    
    if (self.imagePosition == CIGAMButtonImagePositionTop || self.imagePosition == CIGAMButtonImagePositionBottom) {
        ...
        if (self.imagePosition == CIGAMButtonImagePositionTop) {
        ...
        if (isImageViewShowing) {
            ...
            self._cigam_imageView.frame = imageFrame;
        }
        if (isTitleLabelShowing) {
    ...
    } else if (self.imagePosition == CIGAMButtonImagePositionLeft || self.imagePosition == CIGAMButtonImagePositionRight) {
        ...
        if (self.imagePosition == CIGAMButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(CIGAMButtonImagePosition)imagePosition {
...
- (void)adjustsButtonHighlighted {
    if (self.highlightedBackgroundColor) {
        if (!self.highlightedBackgroundLayer) {
            ...
            [self.highlightedBackgroundLayer cigam_removeDefaultAnimations];
...
- (void)setCornerRadius:(CGFloat)cornerRadius {
    ...
    if (cornerRadius != CIGAMButtonCornerRadiusAdjustsBounds) {
