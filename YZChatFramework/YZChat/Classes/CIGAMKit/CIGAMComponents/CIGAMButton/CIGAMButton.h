...
/// 控制图片在UIButton里的位置，默认为CIGAMButtonImagePositionLeft
typedef NS_ENUM(NSUInteger, CIGAMButtonImagePosition) {
    CIGAMButtonImagePositionTop,             // imageView在titleLabel上面
    CIGAMButtonImagePositionLeft,            // imageView在titleLabel左边
    CIGAMButtonImagePositionBottom,          // imageView在titleLabel下面
    CIGAMButtonImagePositionRight,           // imageView在titleLabel右边
};

/**
 *  用于 `CIGAMButton.cornerRadius` 属性，当 `cornerRadius` 为 `CIGAMButtonCornerRadiusAdjustsBounds` 时，`CIGAMButton` 会在高度变化时自动调整 `cornerRadius`，使其始终保持为高度的 1/2。
 */
extern const CGFloat CIGAMButtonCornerRadiusAdjustsBounds;
 ...
 *  @warning CIGAMButton 重新定义了 UIButton.titleEdgeInests、imageEdgeInsets、contentEdgeInsets 这三者的布局逻辑，sizeThatFits: 里会把 titleEdgeInests 和 imageEdgeInsets 也考虑在内（UIButton 不会），以使这三个接口的使用更符合直觉。
 */
@interface CIGAMButton : UIButton
...
/**
 * 设置按钮里图标和文字的相对位置，默认为CIGAMButtonImagePositionLeft<br/>
 * 可配合imageEdgeInsets、titleEdgeInsets、contentHorizontalAlignment、contentVerticalAlignment使用
 */
@property(nonatomic, assign) CIGAMButtonImagePosition imagePosition;
...
@property(nonatomic, assign) IBInspectable CGFloat cornerRadius UI_APPEARANCE_SELECTOR;// 默认为 0。将其设置为 CIGAMButtonCornerRadiusAdjustsBounds 可自动保持圆角为按钮高度的一半。
