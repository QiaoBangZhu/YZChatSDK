...
//
//  CIGAMMoreOperationController.h
//  cigam
//
...
@class CIGAMMoreOperationController;
@class CIGAMMoreOperationItemView;

@protocol CIGAMMoreOperationControllerDelegate <NSObject>

@optional

/// 即将显示操作面板
- (void)willPresentMoreOperationController:(CIGAMMoreOperationController *)moreOperationController;

/// 已经显示操作面板
- (void)didPresentMoreOperationController:(CIGAMMoreOperationController *)moreOperationController;

/// 即将降下操作面板，cancelled参数是用来区分是否触发了maskView或者cancelButton按钮降下面板还是手动调用hide方法来降下面板。
- (void)willDismissMoreOperationController:(CIGAMMoreOperationController *)moreOperationController cancelled:(BOOL)cancelled;

/// 已经降下操作面板，cancelled参数是用来区分是否触发了maskView或者cancelButton按钮降下面板还是手动调用hide方法来降下面板。
- (void)didDismissMoreOperationController:(CIGAMMoreOperationController *)moreOperationController cancelled:(BOOL)cancelled;

/// itemView 点击事件，可以与 itemView.handler 共存，可通过 itemView.tag 或者 itemView.indexPath 来区分不同的 itemView
- (void)moreOperationController:(CIGAMMoreOperationController *)moreOperationController didSelectItemView:(CIGAMMoreOperationItemView *)itemView;
@end
 ...
 */
@interface CIGAMMoreOperationController : UIViewController <CIGAMModalPresentationContentViewControllerProtocol, CIGAMModalPresentationViewControllerDelegate, CIGAMModalPresentationComponentProtocol>
...
@property(nullable, nonatomic, weak) id<CIGAMMoreOperationControllerDelegate> delegate;
...
@property(nullable, nonatomic, copy) NSArray<NSArray<CIGAMMoreOperationItemView *> *> *items;
...
- (CIGAMMoreOperationItemView * _Nullable)itemViewWithTag:(NSInteger)tag;

/// 获取指定 itemView 在当前控件里的 indexPath，如果不存在则返回 nil
- (NSIndexPath * _Nullable)indexPathWithItemView:(CIGAMMoreOperationItemView *)itemView;
...
@interface CIGAMMoreOperationController (UIAppearance)
...
@interface CIGAMMoreOperationItemView : CIGAMButton
...
+ (instancetype)itemViewWithImage:(UIImage * _Nullable)image
                          ...
                          handler:(void (^ _Nullable)(CIGAMMoreOperationController *moreOperationController, CIGAMMoreOperationItemView *itemView))handler;
