...
@interface CIGAMTableViewCell : UITableViewCell
 ...
 *
 *  @note UITableViewCell(CIGAM) 内也有一个 cigam_cellPosition，那个需要在 willDisplayCell 后才有值，cellForRow 里是用不了的，所以 CIGAMTableViewCell 才增加 cellPosition。
 */
@property(nonatomic, assign, readonly) CIGAMTableViewCellPosition cellPosition;
...
@interface CIGAMTableViewCell (CIGAMSubclassingHooks)
