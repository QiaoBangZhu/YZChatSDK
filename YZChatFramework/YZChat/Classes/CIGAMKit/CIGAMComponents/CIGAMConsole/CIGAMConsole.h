...
/**
 ...
 1. 调用 [CIGAMConsole log:...] 直接打印 level 为 "Normal"、name 为 "Default" 的日志。
 2. 调用 [CIGAMConsole logWithLevel:name:logString:] 打印详细日志，则在控制台里可以按照 level 和 name 分类筛选显示。
 3. 当屏幕上出现小圆钮时，点击可以打开控制台，小圆钮会移动到控制台右上角，再次点击小圆钮即可收起控制台。
 4. 如果要隐藏小圆钮，长按即可。
 
 @note 默认只在 DEBUG 下才会显示窗口，其他环境下只会打印日志但不会出现控制台界面。可通过 canShow 属性修改这个策略。
 */
@interface CIGAMConsole : NSObject
...
/**
 ...
 @param logString 支持 NSString/NSAttributedString/NSObject，如果是 NSString 则默认样式由 [CIGAMConsole appearance].textAttributes 控制
 */
...
/**
 显示控制台。由于 CIGAMConsole.showConsoleAutomatically 默认为 YES，所以只要输出 log 就会自动显示控制台，一般无需手动调用 show 方法。
 */
+ (void)show;
...
@interface CIGAMConsole (UIAppearance)
