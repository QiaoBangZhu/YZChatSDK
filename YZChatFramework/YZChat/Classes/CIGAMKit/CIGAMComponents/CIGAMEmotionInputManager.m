...
//
//  CIGAMEmotionInputManager.m
//  cigam
//
//  Created by CIGAM Team on 16/9/8.
//

#import "CIGAMEmotionInputManager.h"
#import "CIGAMCore.h"
#import "NSString+CIGAM.h"
#import "CIGAMEmotionView.h"

@protocol CIGAMEmotionInputViewProtocol <UITextInput>

@property(nonatomic, copy) NSString *text;
@property(nonatomic, assign, readonly) NSRange selectedRange;
@end

@implementation CIGAMEmotionInputManager

- (instancetype)init {
    self = [super init];
    if (self) {
        _emotionView = [[CIGAMEmotionView alloc] init];
        __weak CIGAMEmotionInputManager *weakSelf = self;
        self.emotionView.didSelectEmotionBlock = ^(NSInteger index, CIGAMEmotion *emotion) {
...
- (UIView<CIGAMEmotionInputViewProtocol> *)boundInputView {
    if (self.boundTextField) {
        return (UIView<CIGAMEmotionInputViewProtocol> *)self.boundTextField;
    } else if (self.boundTextView) {
        return (UIView<CIGAMEmotionInputViewProtocol> *)self.boundTextView;
    }
    return nil;
}
    ...
    if (forceDelete) {
        if (NSMaxRange(selectedRange) <= text.length) {
            if (selectedRange.length > 0) {
                ...
                NSString *textAfterDelete = [text cigam_stringByRemoveCharacterAtIndex:selectedRange.location - 1];
            ...
            self.boundInputView.text = [text cigam_stringByRemoveLastCharacter];
