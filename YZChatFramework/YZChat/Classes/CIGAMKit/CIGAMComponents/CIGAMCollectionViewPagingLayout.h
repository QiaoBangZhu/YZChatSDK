...
//
//  CIGAMCollectionViewPagingLayout.h
//  cigam
//
...
typedef NS_ENUM(NSInteger, CIGAMCollectionViewPagingLayoutStyle) {
    CIGAMCollectionViewPagingLayoutStyleDefault, // 普通模式，水平滑动
    CIGAMCollectionViewPagingLayoutStyleScale,   // 缩放模式，两边的item会小一点，逐渐向中间放大
    CIGAMCollectionViewPagingLayoutStyleRotation // 旋转模式，围绕底部某个点为中心旋转
};
 ...
 */
@interface CIGAMCollectionViewPagingLayout : UICollectionViewFlowLayout

- (instancetype)initWithStyle:(CIGAMCollectionViewPagingLayoutStyle)style NS_DESIGNATED_INITIALIZER;

@property(nonatomic, assign, readonly) CIGAMCollectionViewPagingLayoutStyle style;
...
@interface CIGAMCollectionViewPagingLayout (DefaultStyle)
...
@interface CIGAMCollectionViewPagingLayout (ScaleStyle)
...
extern const CGFloat CIGAMCollectionViewPagingLayoutRotationRadiusAutomatic;

@interface CIGAMCollectionViewPagingLayout (RotationStyle)

/**
 *  旋转卡片相关
 *  左右两个卡片最终旋转的角度有 rotationRadius * 90 计算出来
 *  rotationRadius表示旋转的半径
 *  @warning 仅当 style 为 CIGAMCollectionViewPagingLayoutStyleRotation 时才生效
 */
