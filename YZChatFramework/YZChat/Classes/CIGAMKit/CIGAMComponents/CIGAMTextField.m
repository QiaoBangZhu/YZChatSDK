...
//  cigam
//
//  Created by CIGAM Team on 16-11-03
//
...
// 私有的类，专用于实现 CIGAMTextFieldDelegate，避免 self.delegate = self 的写法（以前是 CIGAMTextField 自己实现了 delegate）
@interface _CIGAMTextFieldDelegator : NSObject <CIGAMTextFieldDelegate, UIScrollViewDelegate>

@property(nonatomic, weak) CIGAMTextField *textField;
- (void)handleTextChangeEvent:(CIGAMTextField *)textField;
@end

@interface CIGAMTextField ()

@property(nonatomic, strong) _CIGAMTextFieldDelegator *delegator;
@end

@implementation CIGAMTextField
...
- (void)didInitialize {
    ...
    self.delegator = [[_CIGAMTextFieldDelegator alloc] init];
...
- (void)layoutSubviews {
    ...
    // 以下代码修复系统的 UITextField 在 iOS 10 下的 bug：https://github.com/Tencent/CIGAM_iOS/issues/64
...
- (void)fireTextDidChangeEventForTextField:(CIGAMTextField *)textField {
...
@implementation _CIGAMTextFieldDelegator

#pragma mark - <CIGAMTextFieldDelegate>

- (BOOL)textField:(CIGAMTextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if (textField.maximumTextLength < NSUIntegerMax) {
        ...
        if (NSMaxRange(range) > textField.text.length) {
            ...
            if (range.length > 0) {
                UITextRange *textRange = [self.textField cigam_convertUITextRangeFromNSRange:range];
                [self.textField replaceRange:textRange withText:string];
            }
            return NO;
        }
        
        NSUInteger rangeLength = textField.shouldCountingNonASCIICharacterAsTwo ? [textField.text substringWithRange:range].cigam_lengthWhenCountingNonASCIICharacterAsTwo : range.length;
        if ([textField lengthWithString:textField.text] - rangeLength + [textField lengthWithString:string] > textField.maximumTextLength) {
            ...
            if (substringLength > 0 && [textField lengthWithString:string] > substringLength) {
                NSString *allowedText = [string cigam_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, substringLength) lessValue:YES countingNonASCIICharacterAsTwo:textField.shouldCountingNonASCIICharacterAsTwo];
...
- (void)handleTextChangeEvent:(CIGAMTextField *)textField {
    ...
    if (!textField.markedTextRange) {
        if ([textField lengthWithString:textField.text] > textField.maximumTextLength) {
            textField.text = [textField.text cigam_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, textField.maximumTextLength) lessValue:YES countingNonASCIICharacterAsTwo:textField.shouldCountingNonASCIICharacterAsTwo];
            
            if ([textField.delegate respondsToSelector:@selector(textField:didPreventTextChangeInRange:replacementString:)]) {
                [textField.delegate textField:textField didPreventTextChangeInRange:textField.cigam_selectedRange replacementString:nil];
            }
        }
    }
}
