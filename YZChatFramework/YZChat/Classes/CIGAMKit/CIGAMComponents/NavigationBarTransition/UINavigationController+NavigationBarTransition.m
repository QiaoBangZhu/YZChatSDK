...
//  cigam
//
//  Created by CIGAM Team on 16/2/22.
//
...
@interface _CIGAMTransitionNavigationBar : UINavigationBar

@property(nonatomic, weak) UINavigationBar *originalNavigationBar;
@end

@implementation _CIGAMTransitionNavigationBar

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // iOS 14 开启 customNavigationBarTransitionKey 的情况下转场效果错误
        // https://github.com/Tencent/CIGAM_iOS/issues/1081
        if (@available(iOS 14.0, *)) {
            OverrideImplementation([_CIGAMTransitionNavigationBar class], NSSelectorFromString([NSString stringWithFormat:@"_%@_%@", @"accessibility", @"navigationController"]), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^UINavigationController *(_CIGAMTransitionNavigationBar *selfObject) {
...
- (void)setOriginalNavigationBar:(UINavigationBar *)originBar {
    ...
    if (backgroundImage && backgroundImage.size.width <= 0 && backgroundImage.size.height <= 0) {
        ...
        backgroundImage = [UIImage cigam_imageWithColor:UIColorClear];
    }
...
- (void)layoutSubviews {
    ...
    if (@available(iOS 11, *)) {
        ...
        self.cigam_backgroundView.frame = self.bounds;
    }
}
...
@property(nonatomic, strong) _CIGAMTransitionNavigationBar *transitionNavigationBar;
...
@property(nonatomic, strong) UIColor *cigam_specifiedTextColor;
@end

@implementation UILabel (NavigationBarTransition)

CIGAMSynthesizeIdStrongProperty(cigam_specifiedTextColor, setCigam_specifiedTextColor)
    ...
    dispatch_once(&onceToken, ^{
        OverrideImplementation(NSClassFromString(@"UIButtonLabel"), @selector(setAttributedText:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UILabel *selfObject, NSAttributedString *attributedText) {
                ...
                if (selfObject.cigam_specifiedTextColor) {
                    ...
                    [mutableAttributedText addAttributes:@{ NSForegroundColorAttributeName : selfObject.cigam_specifiedTextColor} range:NSMakeRange(0, mutableAttributedText.length)];
...
- (UILabel *)cigam_backButtonLabel {
    if (@available(iOS 11, *)) {
        __block UILabel *backButtonLabel = nil;
        [self.cigam_contentView.subviews enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(__kindof UIView * _Nonnull subview, NSUInteger idx, BOOL * _Nonnull stop) {
...
#pragma mark - UINavigationController cigam_didInitialize
        ExtendImplementationOfVoidMethodWithoutArguments([UINavigationController class], @selector(cigam_didInitialize), ^(UINavigationController *selfObject) {
            [selfObject cigam_addNavigationActionDidChangeBlock:^(CIGAMNavigationAction action, BOOL animated, __kindof UINavigationController * _Nullable weakNavigationController, __kindof UIViewController * _Nullable appearingViewController, NSArray<__kindof UIViewController *> * _Nullable disappearingViewControllers) {
                ...
                switch (action) {
                    case CIGAMNavigationActionWillPush: {
                        ...
                        break;
                    case CIGAMNavigationActionWillPop:
                    case CIGAMNavigationActionWillSet: {
                    ...
                    case CIGAMNavigationActionDidPop: {
                        ...
                        if (@available(iOS 13.0, *)) {
                            ...
                            [weakNavigationController cigam_animateAlongsideTransition:nil completion:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
                                ...
                                // https://github.com/Tencent/CIGAM_iOS/issues/1210
        ...
        OverrideImplementation([UIViewController class], @selector(viewWillLayoutSubviews), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIViewController *selfObject) {
                ...
                if (isCurrentToViewController && (selfObject.cigam_visibleState < CIGAMViewControllerDidAppear || selfObject.cigam_visibleState >= CIGAMViewControllerDidDisappear)) {
                    
                    BOOL shouldCustomNavigationBarTransition = NO;
                    UINavigationControllerOperation operation = toViewController.navigationController.cigam_isPushing ? UINavigationControllerOperationPush: UINavigationControllerOperationPop;
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/479
...
- (void)addTransitionNavigationBarIfNeeded {
    ...
    if (!self.view.cigam_visible || !self.navigationController.navigationBar) {
        return;
    }
    
    UINavigationBar *originBar = self.navigationController.navigationBar;
    _CIGAMTransitionNavigationBar *customBar = [[_CIGAMTransitionNavigationBar alloc] init];
...
- (void)resizeTransitionNavigationBarFrame {
    if (!self.view.cigam_visible) {
        return;
    }
    UIView *backgroundView = self.navigationController.navigationBar.cigam_backgroundView;
...
- (void)renderNavigationBarStyleAnimated:(BOOL)animated {
    ...
    if (![self conformsToProtocol:@protocol(CIGAMNavigationControllerAppearanceDelegate)]) {
        return;
    }
    
    // 以下用于控制 vc 的外观样式，如果某个方法有实现则用方法的返回值，否则再看配置表对应的值是否有配置，有配置就使用配置表，没配置则什么都不做，维持系统原生样式
    UIViewController<CIGAMNavigationControllerAppearanceDelegate> *vc = (UIViewController<CIGAMNavigationControllerAppearanceDelegate> *)self;
    ...
    } else if (CIGAMCMIActivated) {
        navigationController.navigationBar.barTintColor = UINavigationBar.cigam_appearanceConfigured.barTintColor;
    }
    ...
    if ([vc respondsToSelector:@selector(navigationBarBackgroundImage)]) {
        ...
        [navigationController.navigationBar setBackgroundImage:[UINavigationBar.cigam_appearanceConfigured backgroundImageForBarMetrics:UIBarMetricsDefault] forBarMetrics:UIBarMetricsDefault];
    }
    ...
    if ([vc respondsToSelector:@selector(navigationBarStyle)]) {
        ...
        navigationController.navigationBar.barStyle = UINavigationBar.cigam_appearanceConfigured.barStyle;
    }
    ...
    [vc respondsToSelector:@selector(navigationBarTintColor)] ? [vc navigationBarTintColor] :
                                             CIGAMCMIActivated ? NavBarTintColor : nil;
    if (tintColor) {
        if (@available(iOS 11, *)) {
            // https://github.com/Tencent/CIGAM_iOS/issues/654
            ...
            if (navigationController.cigam_isPopping) {
                UILabel *backButtonLabel = navigationController.navigationBar.cigam_backButtonLabel;
                if (backButtonLabel) {
                    backButtonLabel.cigam_specifiedTextColor = backButtonLabel.textColor;
                    [vc cigam_animateAlongsideTransition:nil completion:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
                        backButtonLabel.cigam_specifiedTextColor = nil;
                    }];
                }
            }
        }
       
        [vc cigam_animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext>  _Nonnull context) {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1190
        shouldRenderTitle = navigationController.cigam_navigationAction >= CIGAMNavigationActionUnknow && navigationController.cigam_navigationAction <= CIGAMNavigationActionPushCompleted;
    }
...
- (void)renderNavigationTitleStyleAnimated:(BOOL)animated {
    ...
    if ([vc respondsToSelector:@selector(titleViewTintColor)]) {
        ...
        if ([vc.navigationItem.titleView isKindOfClass:CIGAMNavigationTitleView.class]) {
            ((CIGAMNavigationTitleView *)vc.navigationItem.titleView).tintColor = tintColor;
...
- (BOOL)respondCustomNavigationBarTransitionIfBarHiddenable {
    ...
    // 如果当前界面正在搜索，由于 UISearchController 会自动把 navigationBar 移上去，所以这种时候 CIGAM 就不应该再去操作 bar 的显隐了
...
- (BOOL)shouldCustomTransitionAutomaticallyForOperation:(UINavigationControllerOperation)operation firstViewController:(UIViewController *)viewController1 secondViewController:(UIViewController *)viewController2 {
    ...
    UIViewController<CIGAMNavigationControllerDelegate> *vc1 = (UIViewController<CIGAMNavigationControllerDelegate> *)viewController1;
    UIViewController<CIGAMNavigationControllerDelegate> *vc2 = (UIViewController<CIGAMNavigationControllerDelegate> *)viewController2;
    
    if (![vc1 conformsToProtocol:@protocol(CIGAMNavigationControllerDelegate)] || ![vc2 conformsToProtocol:@protocol(CIGAMNavigationControllerDelegate)]) {
        return NO;// 只处理前后两个界面都是 CIGAM 系列的场景
    }
    
    BOOL vc1Clips = vc1.isViewLoaded && vc1.view.clipsToBounds && vc1.cigam_navigationBarMaxYInViewCoordinator < NavigationContentTopConstant;
    BOOL vc2Clips = vc2.isViewLoaded && vc2.view.clipsToBounds && vc2.cigam_navigationBarMaxYInViewCoordinator < NavigationContentTopConstant;
    if (vc1Clips || vc2Clips) {
        CIGAMLogWarn(@"UINavigationController (NavigationBarTransition)", @"因界面布局原因导致无法优化导航栏动画，vc1 = %@，maxY1 = %.0f, vc2 = %@，maxY2 = %.0f", vc1, vc1.cigam_navigationBarMaxYInViewCoordinator, vc2, vc2.cigam_navigationBarMaxYInViewCoordinator);
    ...
    UIImage *bg1 = [vc1 respondsToSelector:@selector(navigationBarBackgroundImage)] ? [vc1 navigationBarBackgroundImage] : [UINavigationBar.cigam_appearanceConfigured backgroundImageForBarMetrics:UIBarMetricsDefault];
    UIImage *bg2 = [vc2 respondsToSelector:@selector(navigationBarBackgroundImage)] ? [vc2 navigationBarBackgroundImage] : [UINavigationBar.cigam_appearanceConfigured backgroundImageForBarMetrics:UIBarMetricsDefault];
    if (bg1 || bg2) {
        if (!bg1 || !bg2) {
            return YES;// 一个有一个没有，则需要自定义
        }
        if (![bg1.cigam_averageColor isEqual:bg2.cigam_averageColor]) {
    ...
    if (!bg1 && !bg2) {
        UIColor *barTintColor1 = [vc1 respondsToSelector:@selector(navigationBarBarTintColor)] ? [vc1 navigationBarBarTintColor] : UINavigationBar.cigam_appearanceConfigured.barTintColor;
        UIColor *barTintColor2 = [vc2 respondsToSelector:@selector(navigationBarBarTintColor)] ? [vc2 navigationBarBarTintColor] : UINavigationBar.cigam_appearanceConfigured.barTintColor;
        ...
        UIBarStyle barStyle1 = [vc1 respondsToSelector:@selector(navigationBarStyle)] ? [vc1 navigationBarStyle] : UINavigationBar.cigam_appearanceConfigured.barStyle;
        UIBarStyle barStyle2 = [vc2 respondsToSelector:@selector(navigationBarStyle)] ? [vc2 navigationBarStyle] : UINavigationBar.cigam_appearanceConfigured.barStyle;
        if (barStyle1 != barStyle2) {
            return YES;
        }
    }
    
    UIImage *shadowImage1 = [vc1 respondsToSelector:@selector(navigationBarShadowImage)] ? [vc1 navigationBarShadowImage] : (vc1.navigationController.navigationBar ? vc1.navigationController.navigationBar.shadowImage : (CIGAMCMIActivated ? NavBarShadowImage : nil));
    UIImage *shadowImage2 = [vc2 respondsToSelector:@selector(navigationBarShadowImage)] ? [vc2 navigationBarShadowImage] : (vc2.navigationController.navigationBar ? vc2.navigationController.navigationBar.shadowImage : (CIGAMCMIActivated ? NavBarShadowImage : nil));
    if (shadowImage1 || shadowImage2) {
        if (!shadowImage1 || !shadowImage2) {
            return YES;
        }
        if (![shadowImage1.cigam_averageColor isEqual:shadowImage2.cigam_averageColor]) {
            return YES;
        }
    }
...
CIGAMSynthesizeIdStrongProperty(transitionNavigationBar, setTransitionNavigationBar)
CIGAMSynthesizeIdStrongProperty(originContainerViewBackgroundColor, setOriginContainerViewBackgroundColor)
...
- (void)setPrefersNavigationBarBackgroundViewHidden:(BOOL)prefersNavigationBarBackgroundViewHidden {
    ...
    if (prefersNavigationBarBackgroundViewHidden) {
        self.navigationController.navigationBar.cigam_backgroundView.layer.mask = [CALayer layer];
    } else {
        self.navigationController.navigationBar.cigam_backgroundView.layer.mask = nil;
    }
