...
@class CIGAMZoomImageView;
@class CIGAMEmptyView;
@class CIGAMButton;
@class CIGAMSlider;
@class CIGAMZoomImageViewVideoToolbar;
@class CIGAMPieProgressView;

@protocol CIGAMZoomImageViewDelegate <NSObject>
@optional
- (void)singleTouchInZoomingImageView:(CIGAMZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)doubleTouchInZoomingImageView:(CIGAMZoomImageView *)zoomImageView location:(CGPoint)location;
- (void)longPressInZoomingImageView:(CIGAMZoomImageView *)zoomImageView;
...
/// 是否支持缩放，默认为 YES
- (BOOL)enabledZoomViewInZoomImageView:(CIGAMZoomImageView *)zoomImageView;
 ...
 *  @see viewportRect
 *
 *  CIGAMZoomImageView 提供最基础的图片预览和缩放功能以及 loading、错误等状态的展示支持，其他功能请通过继承来实现。
 */
@interface CIGAMZoomImageView : UIView <UIScrollViewDelegate>

@property(nonatomic, weak) id<CIGAMZoomImageViewDelegate> delegate;
...
// @see CIGAMZoomImageViewVideoToolbar
@property(nonatomic, strong, readonly) CIGAMZoomImageViewVideoToolbar *videoToolbar;

// 视频底部控制条的 margins，会在此基础上自动叠加 CIGAMZoomImageView.cigam_safeAreaInsets，因此无需考虑在 iPhone X 下的兼容
// 默认值为 {0, 25, 25, 18}
@property(nonatomic, assign) UIEdgeInsets videoToolbarMargins UI_APPEARANCE_SELECTOR;

// 播放 video 时屏幕中央的播放按钮
@property(nonatomic, strong, readonly) CIGAMButton *videoCenteredPlayButton;

// 可通过此属性修改 video 播放时屏幕中央的播放按钮图片
@property(nonatomic, strong) UIImage *videoCenteredPlayButtonImage UI_APPEARANCE_SELECTOR;

// 从 iCloud 加载资源的进度展示
@property(nonatomic, strong) CIGAMPieProgressView *cloudProgressView;

// 从 iCloud 加载资源失败的重试按钮
@property(nonatomic, strong) CIGAMButton *cloudDownloadRetryButton;

// 当前展示的资源的下载状态
@property(nonatomic, assign) CIGAMAssetDownloadStatus cloudDownloadStatus;
...
/**
 *  获取当前正在显示的图片/视频在整个 CIGAMZoomImageView 坐标系里的 rect（会按照当前的缩放状态来计算）
 */
...
@property(nonatomic, strong, readonly) CIGAMEmptyView *emptyView;
...
@interface CIGAMZoomImageViewVideoToolbar : UIView

@property(nonatomic, strong, readonly) CIGAMButton *playButton;
@property(nonatomic, strong, readonly) CIGAMButton *pauseButton;
@property(nonatomic, strong, readonly) CIGAMSlider *slider;
