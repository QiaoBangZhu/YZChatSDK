...
//  cigam
//
//  Created by CIGAM Team on 2016/10/9.
//
...
- (void)didInitialize {
    ...
    _loadingView = (UIView<CIGAMEmptyViewLoadingViewProtocol> *)[[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    ...
    _actionButton = [[CIGAMButton alloc] init];
    self.actionButton.cigam_outsideEdge = UIEdgeInsetsMake(-20, -20, -20, -20);
    self.actionButton.cigam_automaticallyAdjustTouchHighlightedInScrollView = YES;
...
- (void)layoutSubviews {
    ...
    if (!self.textLabel.hidden) {
        self.textLabel.frame = CGRectFlatMake(self.textLabelInsets.left, originY + self.textLabelInsets.top, CGRectGetWidth(self.contentView.bounds) - UIEdgeInsetsGetHorizontalValue(self.textLabelInsets), CIGAMViewSelfSizingHeight);
        originY = CGRectGetMaxY(self.textLabel.frame) + self.textLabelInsets.bottom;
    }
    
    if (!self.detailTextLabel.hidden) {
        self.detailTextLabel.frame = CGRectFlatMake(self.detailTextLabelInsets.left, originY + self.detailTextLabelInsets.top, CGRectGetWidth(self.contentView.bounds) - UIEdgeInsetsGetHorizontalValue(self.detailTextLabelInsets), CIGAMViewSelfSizingHeight);
...
- (void)updateDetailTextLabelWithText:(NSString *)text {
    if (self.detailTextLabelFont && self.detailTextLabelTextColor && text) {
        NSAttributedString *string = [[NSAttributedString alloc] initWithString:text attributes:@{
                                                                                                  ...
                                                                                                  NSParagraphStyleAttributeName: [NSMutableParagraphStyle cigam_paragraphStyleWithLineHeight:self.detailTextLabelFont.pointSize + 10 lineBreakMode:NSLineBreakByWordWrapping textAlignment:NSTextAlignmentCenter]
                                                                                                  }];
...
- (void)setLoadingView:(UIView<CIGAMEmptyViewLoadingViewProtocol> *)loadingView {
...
@interface CIGAMEmptyView (UIAppearance)

@end

@implementation CIGAMEmptyView (UIAppearance)
...
+ (void)setDefaultAppearance {
    CIGAMEmptyView *appearance = [CIGAMEmptyView appearance];
