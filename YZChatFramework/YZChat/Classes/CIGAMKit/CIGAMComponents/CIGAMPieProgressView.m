...
//
//  CIGAMPieProgressView.m
//  cigam
//
//  Created by CIGAM Team on 15/9/8.
//

#import "CIGAMPieProgressView.h"
#import "CIGAMCore.h"

@interface CIGAMPieProgressLayer : CALayer
...
@property(nonatomic, assign) CIGAMPieProgressViewShape shape;

@end

@implementation CIGAMPieProgressLayer
...
- (void)drawInContext:(CGContextRef)context {
    ...
    switch (self.shape) {
        case CIGAMPieProgressViewShapeSector: {
            // 绘制扇形进度区域
        ...
        case CIGAMPieProgressViewShapeRing: {
            // 绘制环形进度区域
...
@implementation CIGAMPieProgressView

+ (Class)layerClass {
    return [CIGAMPieProgressLayer class];
}
...
- (void)setShape:(CIGAMPieProgressViewShape)shape {
    _shape = shape;
    self.progressLayer.shape = shape;
    [self setBorderWidth:_borderWidth];
}

- (CIGAMPieProgressLayer *)progressLayer {
    return (CIGAMPieProgressLayer *)self.layer;
}
