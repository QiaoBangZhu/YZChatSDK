...
//
//  CIGAMDialogViewController.m
//  WeRead
//
//  Created by CIGAM Team on 16/7/8.
//

#import "CIGAMDialogViewController.h"
#import "CIGAMCore.h"
...
@implementation CIGAMDialogViewController (UIAppearance)
...
+ (void)initialize {
    ...
    dispatch_once(&onceToken, ^{
        CIGAMDialogViewController *dialogViewControllerAppearance = CIGAMDialogViewController.appearance;
        ...
        dialogViewControllerAppearance.maximumContentViewWidth = [CIGAMHelper screenSizeFor55Inch].width - UIEdgeInsetsGetHorizontalValue(dialogViewControllerAppearance.dialogViewMargins);
...
@interface CIGAMDialogViewController ()

@property(nonatomic, assign) BOOL hasCustomContentView;
@property(nonatomic,copy) void (^cancelButtonBlock)(CIGAMDialogViewController *dialogViewController);
@property(nonatomic,copy) void (^submitButtonBlock)(CIGAMDialogViewController *dialogViewController);
@end

@implementation CIGAMDialogViewController
...
- (void)didInitialize {
    ...
    // 使用自带的CIGAMNavigationTitleView，支持loading、subTitle
    [self.headerView addSubview:self.titleView];
    
    // 加上分隔线
    _headerViewSeparatorLayer = [CALayer layer];
    [self.headerViewSeparatorLayer cigam_removeDefaultAnimations];
    ...
    [self.footerViewSeparatorLayer cigam_removeDefaultAnimations];
    ...
    [self.buttonSeparatorLayer cigam_removeDefaultAnimations];
    ...
    self.modalPresentationViewController = [[CIGAMModalPresentationViewController alloc] init];
...
- (void)viewDidLayoutSubviews {
    ...
    if (isFooterViewShowing) {
        ...
        if (buttonCount == 1) {
            CIGAMButton *button = self.cancelButton ? : self.submitButton;
...
- (void)addCancelButtonWithText:(NSString *)buttonText block:(void (^)(__kindof CIGAMDialogViewController *))block {
...
- (void)addSubmitButtonWithText:(NSString *)buttonText block:(void (^)(__kindof CIGAMDialogViewController *dialogViewController))block {
...
- (CIGAMButton *)generateButtonWithText:(NSString *)buttonText {
    CIGAMButton *button = [[CIGAMButton alloc] init];
...
- (void)handleCancelButtonEvent:(CIGAMButton *)cancelButton {
...
- (void)handleSubmitButtonEvent:(CIGAMButton *)submitButton {
...
@implementation CIGAMDialogSelectionViewController (UIAppearance)
...
+ (void)initialize {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        CIGAMDialogSelectionViewController.appearance.rowHeight = TableViewCellNormalHeight;
    });
}

@end

const NSInteger CIGAMDialogSelectionViewControllerSelectedItemIndexNone = -1;

@interface CIGAMDialogSelectionViewController ()

@property(nonatomic,strong,readwrite) CIGAMTableView *tableView;
@end

@implementation CIGAMDialogSelectionViewController

- (void)didInitialize {
    [super didInitialize];
    
    self.selectedItemIndex = CIGAMDialogSelectionViewControllerSelectedItemIndexNone;
    self.selectedItemIndexes = [[NSMutableSet alloc] init];
    
    self.tableView = [[CIGAMTableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    ...
    self.tableView.backgroundColor = self.contentViewBackgroundColor;// CIGAMDialogSelectionViewController 使用了 customContentView，所以默认不会自动应用到 self.contentViewBackgroundColor，这里手动应用一次
}

- (void)viewDidAppear:(BOOL)animated {
    [super viewDidAppear:animated];
    // 当前的分组不在可视区域内，则滚动到可视区域（只对单选有效）
    if (self.selectedItemIndex != CIGAMDialogSelectionViewControllerSelectedItemIndexNone && self.selectedItemIndex < self.items.count && ![self.tableView cigam_cellVisibleAtIndexPath:[NSIndexPath indexPathForRow:self.selectedItemIndex inSection:0]]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CIGAMTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[CIGAMTableViewCell alloc] initForTableView:tableView withStyle:UITableViewCellStyleSubtitle reuseIdentifier:identifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.allowsMultipleSelection) {
        ...
        if ([tableView cigam_cellVisibleAtIndexPath:indexPath]) {
        ...
        if (self.selectedItemIndex != CIGAMDialogSelectionViewControllerSelectedItemIndexNone) {
            ...
            isSelectedIndexPathBeforeVisible = [tableView cigam_cellVisibleAtIndexPath:selectedIndexPathBefore];
        }
...
@implementation CIGAMDialogTextFieldViewController (UIAppearance)
...
+ (void)initialize {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        CIGAMDialogTextFieldViewController *dialogTextFieldViewControllerAppearance = CIGAMDialogTextFieldViewController.appearance;
...
@interface CIGAMDialogTextFieldViewController ()<CIGAMTextFieldDelegate>

@property(nonatomic, strong) UIScrollView *scrollView;
@property(nonatomic, strong) NSMutableArray<CIGAMLabel *> *mutableTitleLabels;
@property(nonatomic, strong) NSMutableArray<CIGAMTextField *> *mutableTextFields;
@property(nonatomic, strong) NSMutableArray<CALayer *> *mutableSeparatorLayers;
@end

@implementation CIGAMDialogTextFieldViewController
...
- (void)addTextFieldWithTitle:(NSString *)textFieldTitle configurationHandler:(void (^)(CIGAMLabel *titleLabel, CIGAMTextField *textField, CALayer *separatorLayer))configurationHandler {
    CIGAMLabel *label = [self generateTextFieldTitleLabel];
    ...
    CIGAMTextField *textField = [self generateTextField];
...
- (CIGAMLabel *)generateTextFieldTitleLabel {
    CIGAMLabel *textFieldLabel = [[CIGAMLabel alloc] init];
...
- (CIGAMTextField *)generateTextField {
...
- (CALayer *)generateTextFieldSeparatorLayer {
    CALayer *textFieldSeparatorLayer = [CALayer cigam_separatorLayer];
...
- (void)viewDidLayoutSubviews {
    ...
    for (NSInteger i = 0; i < self.mutableTitleLabels.count; i++) {
        CIGAMLabel *label = self.mutableTitleLabels[i];
        CIGAMTextField *textField = self.mutableTextFields[i];
...
- (void)setTextFieldLabelFont:(UIFont *)textFieldLabelFont {
    ...
    [self.mutableTitleLabels enumerateObjectsUsingBlock:^(CIGAMLabel * _Nonnull label, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTextFieldFont:(UIFont *)textFieldFont {
    ...
    [self.mutableTextFields enumerateObjectsUsingBlock:^(CIGAMTextField * _Nonnull textField, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSArray<CIGAMLabel *> *)textFieldTitleLabels {
...
- (BOOL)shouldEnabledSubmitButton {
    ...
    if (self.enablesSubmitButtonAutomatically) {
        ...
        [self.mutableTextFields enumerateObjectsUsingBlock:^(CIGAMTextField * _Nonnull textField, NSUInteger idx, BOOL * _Nonnull stop) {
            NSInteger textLength = textField.text.cigam_trim.length;
...
- (void)handleTextFieldTextDidChangeEvent:(CIGAMTextField *)textField {
...
- (CGSize)preferredContentSizeInModalPresentationViewController:(CIGAMModalPresentationViewController *)controller keyboardHeight:(CGFloat)keyboardHeight limitSize:(CGSize)limitSize {
    ...
    for (CIGAMLabel *label in self.mutableTitleLabels) {
        if (!label.hidden) {
    ...
    CGFloat contentHeight = textFieldLabelHeight + textFieldHeight + separatorHeight + UIEdgeInsetsGetVerticalValue(self.scrollView.cigam_contentInset);
    CGFloat contentViewVerticalMargin = UIEdgeInsetsGetVerticalValue(self.contentViewMargins);
