...
//  cigam
//
//  Created by CIGAM Team on 16/9/6.
//
...
- (BOOL)isEqual:(id)object {
    ...
    return [self.identifier isEqualToString:((CIGAMEmotion *)object).identifier];
}
...
@class CIGAMEmotionPageView;

@protocol CIGAMEmotionPageViewDelegate <NSObject>

@optional
- (void)emotionPageView:(CIGAMEmotionPageView *)emotionPageView didSelectEmotion:(CIGAMEmotion *)emotion atIndex:(NSInteger)index;
- (void)emotionPageViewDidLayoutEmotions:(CIGAMEmotionPageView *)emotionPageView;
@end

/// 表情面板每一页的cell，在drawRect里将所有表情绘制上去，同时自带一个末尾的删除按钮
@interface CIGAMEmotionPageView : UICollectionViewCell

@property(nonatomic, weak) CIGAMEmotionView<CIGAMEmotionPageViewDelegate> *delegate;

/// 表情被点击时盖在表情上方用于表示选中的遮罩
@property(nonatomic, strong) UIView *emotionSelectedBackgroundView;

/// 表情面板右下角的删除按钮
@property(nonatomic, weak) CIGAMButton *deleteButton;
...
/// 分配给当前pageView的所有表情
@property(nonatomic, copy) NSArray<CIGAMEmotion *> *emotions;
...
@implementation CIGAMEmotionPageView
...
- (void)setEmotions:(NSArray<CIGAMEmotion *> *)emotions {
...
- (void)handleTapGestureRecognizer:(UITapGestureRecognizer *)gestureRecognizer {
    ...
    for (NSInteger i = 0; i < self.emotionHittingRects.count; i ++) {
        ...
        if (CGRectContainsPoint(rect, location)) {
            ...
            CIGAMEmotion *emotion = self.emotions[i];
            ...
            if (self.debug) {
                CIGAMLog(NSStringFromClass(self.class), @"点击的是当前页里的第 %@ 个表情，%@", @(i), emotion);
            }
            return;
        }
    }
...
- (void)updateDeleteButton:(CIGAMButton *)deleteButton {
...
@interface CIGAMEmotionVerticalScrollView : UIScrollView
@property(nonatomic, strong) CIGAMEmotionPageView *pageView;
@end

@implementation CIGAMEmotionVerticalScrollView

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        _pageView = [[CIGAMEmotionPageView alloc] init];
...
- (void)setEmotions:(NSArray<CIGAMEmotion *> *)emotions
   ...
   emotionSelectedBackgroundExtension:(UIEdgeInsets)emotionSelectedBackgroundExtension
    ...
    CIGAMEmotionPageView *pageView = self.pageView;
...
@interface CIGAMEmotionView ()<CIGAMEmotionPageViewDelegate>
/// 用于展示表情面板的竖向滚动 scrollView，布局撑满整个控件
@property(nonatomic, strong, readonly) CIGAMEmotionVerticalScrollView *verticalScrollView;
@property(nonatomic, strong) NSMutableArray<NSArray<CIGAMEmotion *> *> *pagedEmotions;
@property(nonatomic, assign) BOOL debug;
@end

@implementation CIGAMEmotionView
...
- (void)didInitializedWithFrame:(CGRect)frame {
    ...
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(self.cigam_safeAreaInsets.left, self.cigam_safeAreaInsets.top, CGRectGetWidth(frame) - UIEdgeInsetsGetHorizontalValue(self.cigam_safeAreaInsets), CGRectGetHeight(frame) - UIEdgeInsetsGetVerticalValue(self.cigam_safeAreaInsets)) collectionViewLayout:self.collectionViewLayout];
    if (@available(iOS 11, *)) {
    ...
    [self.collectionView registerClass:[CIGAMEmotionPageView class] forCellWithReuseIdentifier:@"page"];
    [self addSubview:self.collectionView];
    
    _verticalScrollView = [[CIGAMEmotionVerticalScrollView alloc] init];
    if (@available(iOS 11, *)) {
    ...
    _sendButton = [[CIGAMButton alloc] init];
    [self.sendButton setTitle:@"发送" forState:UIControlStateNormal];
    self.sendButton.contentEdgeInsets = UIEdgeInsetsMake(5, 17, 5, 17);
    [self addSubview:self.sendButton];

    _deleteButton = [[CIGAMButton alloc] init];
    self.deleteButton.cigam_automaticallyAdjustTouchHighlightedInScrollView = YES;
    __weak __typeof(self)weakSelf = self;
    self.deleteButton.cigam_tapBlock = ^(__kindof UIControl *sender) {
...
- (void)layoutSubviews {
    ...
    self.sendButton.cigam_right = self.cigam_width - self.cigam_safeAreaInsets.right - self.sendButtonMargins.right;
    self.sendButton.cigam_bottom = self.cigam_height - self.cigam_safeAreaInsets.bottom - self.sendButtonMargins.bottom;
    if (self.verticalAlignment) {
        CGRect verticalScrollViewFrame = CGRectInsetEdges(self.bounds, UIEdgeInsetsSetBottom(self.cigam_safeAreaInsets, 0));
        ...
        [self.verticalScrollView setEmotions:self.emotions
          ...
          emotionSelectedBackgroundExtension:self.emotionSelectedBackgroundExtension
                               paddingInPage:UIEdgeInsetsSetBottom(self.paddingInPage, self.paddingInPage.bottom + self.cigam_safeAreaInsets.bottom)];
        ...
        [self.deleteButton setImage:[self.deleteButtonImage cigam_imageWithAlpha:ButtonHighlightedAlpha] forState:UIControlStateHighlighted];
        self.deleteButton.bounds = CGRectMakeWithSize(CGSizeMake([self.deleteButton sizeThatFits:CGSizeZero].width, self.sendButton.cigam_height));
        static CGFloat spacingBetweenDeleteButtonAndSendButton = 4.0f;
        self.deleteButton.cigam_right = self.sendButton.cigam_left - spacingBetweenDeleteButtonAndSendButton + self.deleteButtonOffset.x;
        self.deleteButton.cigam_top = CGRectGetMinYVerticallyCenter(self.sendButton.frame, self.deleteButton.frame) + self.deleteButtonOffset.y;
        
    } else {
        CGRect collectionViewFrame = CGRectInsetEdges(self.bounds, self.cigam_safeAreaInsets);
        ...
        CGFloat pageControlMaxX = self.sendButton.cigam_left;
        CGFloat pageControlMinX = self.cigam_width - pageControlMaxX;
        self.pageControl.frame = CGRectMake(pageControlMinX, self.cigam_height - self.cigam_safeAreaInsets.bottom - self.pageControlMarginBottom - pageControlHeight, pageControlMaxX - pageControlMinX, pageControlHeight);
    }
}
...
- (void)pageEmotions {
    ...
    if (!CGRectIsEmpty(self.collectionView.bounds) && self.emotions.count && !CGSizeIsEmpty(self.emotionSize)) {
        ...
        for (NSInteger i = 0; i < pageCount; i ++) {
            ...
            NSArray<CIGAMEmotion *> *emotionForPage = [self.emotions objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:emotionRangeForPage]];
    ...
    [self.collectionView cigam_scrollToTop];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView == self.verticalScrollView) {
        ...
        if (ceil(index) == floor(index)) {
            ...
            CIGAMEmotionPageView *pageView = (CIGAMEmotionPageView *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    CIGAMEmotionPageView *pageView = [collectionView dequeueReusableCellWithReuseIdentifier:@"page" forIndexPath:indexPath];
...
#pragma mark - <CIGAMEmotionPageViewDelegate>
...
@interface CIGAMEmotionView (UIAppearance)

@end

@implementation CIGAMEmotionView (UIAppearance)
...
+ (void)setDefaultAppearance {
    CIGAMEmotionView *appearance = [CIGAMEmotionView appearance];
    appearance.backgroundColor = UIColorForBackground;
    appearance.deleteButtonImage = [CIGAMHelper imageWithName:@"CIGAM_emotion_delete"];
    ...
    UIPageControl *pageControlAppearance = [UIPageControl appearanceWhenContainedInInstancesOfClasses:@[[CIGAMEmotionView class]]];
