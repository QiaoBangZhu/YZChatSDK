...
@implementation CIGAMLogger

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static CIGAMLogger *instance = nil;
...
- (instancetype)init {
    if (self = [super init]) {
        self.logNameManager = [[CIGAMLogNameManager alloc] init];
    }
    return self;
}

- (void)printLogWithFile:(const char *)file line:(int)line func:(const char *)func logItem:(CIGAMLogItem *)logItem {
    // 禁用了某个 name 则直接退出
    ...
    if ([self.delegate respondsToSelector:@selector(printCIGAMLogWithFile:line:func:logItem:defaultString:)]) {
        [self.delegate printCIGAMLogWithFile:fileString line:line func:funcString logItem:logItem defaultString:defaultString];
    } else {
