...
@protocol CIGAMTextViewDelegate <UITextViewDelegate>
 ...
 */
- (void)textView:(CIGAMTextView *)textView newHeightAfterTextChanged:(CGFloat)height;
 ...
 */
- (BOOL)textViewShouldReturn:(CIGAMTextView *)textView;
 ...
 */
- (void)textView:(CIGAMTextView *)textView didPreventTextChangeInRange:(NSRange)range replacementText:(NSString *)replacementText;
 ...
 */
@interface CIGAMTextView : UITextView

@property(nonatomic, weak) id<CIGAMTextViewDelegate> delegate;
...
/**
 *  在使用 maximumTextLength 功能的时候，是否应该把文字长度按照 [NSString (CIGAM) cigam_lengthWhenCountingNonASCIICharacterAsTwo] 的方法来计算。
 *  默认为 NO。
 */
