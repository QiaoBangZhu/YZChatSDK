...
//
//  CIGAMKeyboardManager.m
//  cigam
//
//  Created by CIGAM Team on 2017/3/23.
//
...
@interface CIGAMKeyboardManager ()

@property(nonatomic, strong) NSMutableArray <NSValue *> *targetResponderValues;

@property(nonatomic, strong) CIGAMKeyboardUserInfo *lastUserInfo;
...
- (id)cigam_findFirstResponder {
    ...
    for (UIView *subView in self.subviews) {
        id responder = [subView cigam_findFirstResponder];
...
/// 系统自己的isFirstResponder有延迟，这里手动记录UIResponder是否isFirstResponder，CIGAMKeyboardManager内部自己使用
...
CIGAMSynthesizeIdStrongProperty(cigam_keyboardManager, setCigam_keyboardManager)
CIGAMSynthesizeBOOLProperty(keyboardManager_isFirstResponder, setKeyboardManager_isFirstResponder)
...
//                    selfObject.cigam_keyboardManager &&
//                    [selfObject.cigam_keyboardManager.allTargetResponders containsObject:selfObject]) {
//                    selfObject.cigam_keyboardManager.currentResponderWhenResign = selfObject;
//                }
                // call super
                BOOL (*originSelectorIMP)(id, SEL);
                originSelectorIMP = (BOOL (*)(id, SEL))originalIMPProvider();
...
@interface CIGAMKeyboardViewFrameObserver : NSObject
...
@implementation CIGAMKeyboardViewFrameObserver {
...
@interface CIGAMKeyboardUserInfo ()

@property(nonatomic, weak, readwrite) CIGAMKeyboardManager *keyboardManager;
...
@implementation CIGAMKeyboardUserInfo
...
- (CGFloat)width {
    CGRect keyboardRect = [CIGAMKeyboardManager convertKeyboardRect:_endFrame toView:nil];
...
- (CGFloat)heightInView:(UIView *)view {
    ...
    CGRect keyboardRect = [CIGAMKeyboardManager convertKeyboardRect:_endFrame toView:view];
 ...
 */
@implementation CIGAMKeyboardManager
...
- (instancetype)initWithDelegate:(id <CIGAMKeyboardManagerDelegate>)delegate {
    if (self = [super init]) {
        _delegate = delegate;
...
- (BOOL)addTargetResponder:(UIResponder *)targetResponder {
    ...
    targetResponder.cigam_keyboardManager = self;
...
- (UIResponder *)firstResponderInWindows {
    UIResponder *responder = [UIApplication.sharedApplication.keyWindow cigam_findFirstResponder];
    if (!responder) {
        for (UIWindow *window in UIApplication.sharedApplication.windows) {
            if (window != UIApplication.sharedApplication.keyWindow) {
                responder = [window cigam_findFirstResponder];
                if (responder) {
                    return responder;
                }
            }
...
- (void)keyboardWillShowNotification:(NSNotification *)notification {
    ...
    if (self.debug) {
        CIGAMLog(NSStringFromClass(self.class), @"keyboardWillShowNotification - %@", self);
    }
    
    if (![self isAppActive] || ![self isLocalKeyboard:notification]) {
        CIGAMLog(NSStringFromClass(self.class), @"app is not active");
        return;
    }
    ...
    CIGAMKeyboardUserInfo *userInfo = [self newUserInfoWithNotification:notification];
...
- (void)keyboardDidShowNotification:(NSNotification *)notification {
    ...
    if (self.debug) {
        CIGAMLog(NSStringFromClass(self.class), @"keyboardDidShowNotification - %@", self);
    }
...
- (void)keyboardWillHideNotification:(NSNotification *)notification {
    ...
    if (self.debug) {
        CIGAMLog(NSStringFromClass(self.class), @"keyboardWillHideNotification - %@", self);
    }
...
- (void)keyboardDidHideNotification:(NSNotification *)notification {
    ...
    if (self.debug) {
        CIGAMLog(NSStringFromClass(self.class), @"keyboardDidHideNotification - %@", self);
    }
...
- (void)keyboardWillChangeFrameNotification:(NSNotification *)notification {
    ...
    if (self.debug) {
        CIGAMLog(NSStringFromClass(self.class), @"keyboardWillChangeFrameNotification - %@", self);
    }
...
- (void)keyboardDidChangeFrameNotification:(NSNotification *)notification {
    ...
    if (self.debug) {
        CIGAMLog(NSStringFromClass(self.class), @"keyboardDidChangeFrameNotification - %@", self);
    }
...
- (CIGAMKeyboardUserInfo *)newUserInfoWithNotification:(NSNotification *)notification {
    CIGAMKeyboardUserInfo *userInfo = [[CIGAMKeyboardUserInfo alloc] init];
...
- (void)addFrameObserverIfNeeded {
    ...
    CIGAMKeyboardViewFrameObserver *observer = [CIGAMKeyboardViewFrameObserver observerForView:[self.class keyboardView]];
    if (!observer) {
        observer = [[CIGAMKeyboardViewFrameObserver alloc] init];
...
- (void)keyboardDidChangedFrame:(UIView *)keyboardView {
    ...
    if (self.delegateEnabled && [self.delegate respondsToSelector:@selector(keyboardWillChangeFrameWithUserInfo:)]) {
        ...
        // 自己构造一个CIGAMKeyboardUserInfo，一些属性使用之前最后一个keyboardUserInfo的值
        CIGAMKeyboardUserInfo *keyboardMoveUserInfo = [[CIGAMKeyboardUserInfo alloc] init];
        ...
        if (self.currentResponder) {
            ...
            if (mainWindow) {
                ...
                CGFloat distanceFromBottom = [CIGAMKeyboardManager distanceFromMinYToBottomInView:mainWindow keyboardRect:keyboardRect];
...
+ (void)animateWithAnimated:(BOOL)animated keyboardUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo animations:(void (^)(void))animations completion:(void (^)(BOOL finished))completion {
    if (animated) {
...
+ (void)handleKeyboardNotificationWithUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo showBlock:(void (^)(CIGAMKeyboardUserInfo *keyboardUserInfo))showBlock hideBlock:(void (^)(CIGAMKeyboardUserInfo *keyboardUserInfo))hideBlock {
    // 专门处理 iPad Pro 在键盘完全不显示的情况（不会调用willShow，所以通过是否focus来判断）
    // iPhoneX Max 这里键盘高度不是0，而是一个很小的值
    if ([CIGAMKeyboardManager visibleKeyboardHeight] <= 0 && !keyboardUserInfo.isTargetResponderFocused) {
        if (hideBlock) {
...
+ (CGFloat)visibleKeyboardHeight {
    ...
    if (!keyboardView || !keyboardWindow) {
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1173
...
@interface UITextField () <CIGAMKeyboardManagerDelegate>

@end

@implementation UITextField (CIGAM_KeyboardManager)
...
- (void (^)(CIGAMKeyboardUserInfo *))cigam_keyboardWillShowNotificationBlock {
    return (void (^)(CIGAMKeyboardUserInfo *))objc_getAssociatedObject(self, &kAssociatedObjectKey_keyboardWillShowNotificationBlock);
}
...
- (void (^)(CIGAMKeyboardUserInfo *))cigam_keyboardDidShowNotificationBlock {
    return (void (^)(CIGAMKeyboardUserInfo *))objc_getAssociatedObject(self, &kAssociatedObjectKey_keyboardDidShowNotificationBlock);
}
...
- (void (^)(CIGAMKeyboardUserInfo *))cigam_keyboardWillHideNotificationBlock {
    return (void (^)(CIGAMKeyboardUserInfo *))objc_getAssociatedObject(self, &kAssociatedObjectKey_keyboardWillHideNotificationBlock);
}
...
- (void (^)(CIGAMKeyboardUserInfo *))cigam_keyboardDidHideNotificationBlock {
    return (void (^)(CIGAMKeyboardUserInfo *))objc_getAssociatedObject(self, &kAssociatedObjectKey_keyboardDidHideNotificationBlock);
}
...
- (void (^)(CIGAMKeyboardUserInfo *))cigam_keyboardWillChangeFrameNotificationBlock {
    return (void (^)(CIGAMKeyboardUserInfo *))objc_getAssociatedObject(self, &kAssociatedObjectKey_keyboardWillChangeFrameNotificationBlock);
}
...
- (void (^)(CIGAMKeyboardUserInfo *))cigam_keyboardDidChangeFrameNotificationBlock {
...
- (void)keyboardWillShowWithUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo {
    if (self.cigam_keyboardWillShowNotificationBlock) {
        self.cigam_keyboardWillShowNotificationBlock(keyboardUserInfo);
    }
}

- (void)keyboardWillHideWithUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo {
    if (self.cigam_keyboardWillHideNotificationBlock) {
        self.cigam_keyboardWillHideNotificationBlock(keyboardUserInfo);
    }
}

- (void)keyboardWillChangeFrameWithUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo {
    if (self.cigam_keyboardWillChangeFrameNotificationBlock) {
        self.cigam_keyboardWillChangeFrameNotificationBlock(keyboardUserInfo);
    }
}

- (void)keyboardDidShowWithUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo {
    if (self.cigam_keyboardDidShowNotificationBlock) {
        self.cigam_keyboardDidShowNotificationBlock(keyboardUserInfo);
    }
}

- (void)keyboardDidHideWithUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo {
    if (self.cigam_keyboardDidHideNotificationBlock) {
        self.cigam_keyboardDidHideNotificationBlock(keyboardUserInfo);
    }
}

- (void)keyboardDidChangeFrameWithUserInfo:(CIGAMKeyboardUserInfo *)keyboardUserInfo {
    if (self.cigam_keyboardDidChangeFrameNotificationBlock) {
        self.cigam_keyboardDidChangeFrameNotificationBlock(keyboardUserInfo);
    }
}
...
@interface UITextView () <CIGAMKeyboardManagerDelegate>

@end

@implementation UITextView (CIGAM_KeyboardManager)
