 ...
 *  代表一个表情的数据对象
 */
@interface CIGAMEmotion : NSObject

/// 当前表情的标识符，可用于区分不同表情
@property(nonatomic, copy) NSString *identifier;

/// 当前表情展示出来的名字，可用于输入框里的占位文字，请务必使用统一的左右标识符将表情名称包裹起来（例如常见的“[]”），否则在 `CIGAMEmotionInputManager` 里会因为找不到标识符而无法准确识别出一串文本里的哪些字符是代表一个表情。合法的 displayName 例子：“[委屈]”
...
/**
 *  快速生成一个`CIGAMEmotion`对象，并且以`identifier`为图片名在当前项目里查找，作为表情的图片
 ...
 *
 *  本控件支持通过`UIAppearance`设置全局的默认样式。若要修改控件内的`UIPageControl`的样式，可通过`[UIPageControl appearanceWhenContainedInInstancesOfClasses:@[[CIGAMEmotionView class]]]`的方式来修改。
 */
@interface CIGAMEmotionView : UIView<UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
 ...
 *  @argv  emotion 被选中的表情对应的`CIGAMEmotion`对象
 *  @see CIGAMEmotion
 */
@property(nonatomic, copy) void (^didSelectEmotionBlock)(NSInteger index, CIGAMEmotion *emotion);
...
/// 控件右下角的发送按钮
@property(nonatomic, strong, readonly) CIGAMButton *sendButton;

/// 控件右下角的删除按钮
@property(nonatomic, strong, readonly) CIGAMButton *deleteButton;
...
/// 表情的图片大小，不管`CIGAMEmotion.image.size`多大，都会被缩放到`emotionSize`里显示，默认为{30, 30}
...
/// 表情面板右下角的删除按钮的图片，默认为`[CIGAMHelper imageWithName:@"CIGAM_emotion_delete"]`
