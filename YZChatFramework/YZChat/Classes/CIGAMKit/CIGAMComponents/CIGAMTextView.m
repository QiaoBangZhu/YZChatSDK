...
//  cigam
//
//  Created by CIGAM Team on 14-8-5.
//
...
const CGFloat kCIGAMSystemTextViewDefaultFontPointSize = 12.0f;

/// 当系统的 textView.textContainerInset 为 UIEdgeInsetsZero 时，文字与 textView 边缘的间距。实测得到，请勿修改（在输入框font大于13时准确，小于等于12时，y有-1px的偏差）。
const UIEdgeInsets kCIGAMSystemTextViewFixTextInsets = {0, 5, 0, 5};

// 私有的类，专用于实现 CIGAMTextViewDelegate，避免 self.delegate = self 的写法（以前是 CIGAMTextView 自己实现了 delegate）
@interface _CIGAMTextViewDelegator : NSObject <CIGAMTextViewDelegate>

@property(nonatomic, weak) CIGAMTextView *textView;
@end

@interface CIGAMTextView ()

@property(nonatomic, assign) BOOL debug;
@property(nonatomic, assign) BOOL postInitializationMethodCalled;
@property(nonatomic, strong) _CIGAMTextViewDelegator *delegator;
...
- (void)didInitialize {
    ...
    self.delegator = [[_CIGAMTextViewDelegator alloc] init];
    self.delegator.textView = self;
    self.delegate = self.delegator;
    
    self.scrollsToTop = NO;
    if (CIGAMCMIActivated) self.placeholderColor = UIColorPlaceholder;
    ...
    self.placeholderLabel.font = UIFontMake(kCIGAMSystemTextViewDefaultFontPointSize);
...
- (void)_cigam_setTextForShouldChange:(NSString *)text {
...
- (CGRect)preferredPlaceholderFrameWithSize:(CGSize)size {
    ...
    UIEdgeInsets labelMargins = UIEdgeInsetsMake(allInsets.top - self.cigam_contentInset.top, allInsets.left - self.cigam_contentInset.left, allInsets.bottom - self.cigam_contentInset.bottom, allInsets.right - self.cigam_contentInset.right);
...
- (void)handleTextChanged:(id)sender {
    CIGAMTextView *textView = nil;
    if ([sender isKindOfClass:[NSNotification class]]) {
        id object = ((NSNotification *)sender).object;
        if (object == self) {
            textView = (CIGAMTextView *)object;
        }
    } else if ([sender isKindOfClass:[CIGAMTextView class]]) {
        textView = (CIGAMTextView *)sender;
    }
    ...
    // https://github.com/Tencent/CIGAM_iOS/issues/1168
    ...
    if (!textView.markedTextRange && [textView lengthWithString:textView.text] > textView.maximumTextLength) {
        NSString *finalText = [textView.text cigam_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, textView.maximumTextLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
    ...
    if ([textView.delegate respondsToSelector:@selector(textView:newHeightAfterTextChanged:)]) {
        ...
        if (textView.debug) CIGAMLog(NSStringFromClass(textView.class), @"handleTextDidChange, text = %@, resultHeight = %f", textView.text, resultHeight);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [textView cigam_scrollCaretVisibleAnimated:NO];
    });
}
...
- (UIEdgeInsets)allInsets {
    return UIEdgeInsetsConcat(UIEdgeInsetsConcat(UIEdgeInsetsConcat(self.textContainerInset, self.placeholderMargins), kCIGAMSystemTextViewFixTextInsets), self.cigam_contentInset);
}
...
- (void)setFrame:(CGRect)frame {
    ...
    // https://github.com/Tencent/CIGAM_iOS/issues/557
...
- (NSUInteger)lengthWithString:(NSString *)string {
    return self.shouldCountingNonASCIICharacterAsTwo ? string.cigam_lengthWhenCountingNonASCIICharacterAsTwo : string.length;
}

- (void)setContentOffset:(CGPoint)contentOffset animated:(BOOL)animated {
    if (!self.shouldRejectSystemScroll) {
        [super setContentOffset:contentOffset animated:animated];
        if (self.debug) CIGAMLog(NSStringFromClass(self.class), @"%@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    } else {
        if (self.debug) CIGAMLog(NSStringFromClass(self.class), @"被屏蔽的 %@, contentOffset.y = %.2f", NSStringFromSelector(_cmd), contentOffset.y);
    }
}
...
@implementation _CIGAMTextViewDelegator

#pragma mark - <CIGAMTextViewDelegate>

- (BOOL)textView:(CIGAMTextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if (self.textView.debug) CIGAMLog(NSStringFromClass(self.class), @"textView.text(%@ | %@) = %@\nmarkedTextRange = %@\nrange = %@\ntext = %@", @(textView.text.length), @(textView.text.cigam_lengthWhenCountingNonASCIICharacterAsTwo), textView.text, textView.markedTextRange, NSStringFromRange(range), text);
    ...
    if (textView.maximumTextLength < NSUIntegerMax) {
        ...
        if (NSMaxRange(range) > textView.text.length) {
            ...
            // https://github.com/Tencent/CIGAM_iOS/issues/377
            // https://github.com/Tencent/CIGAM_iOS/issues/1170
            ...
            if (range.length > 0) {
                UITextRange *textRange = [self.textView cigam_convertUITextRangeFromNSRange:range];
                [self.textView replaceRange:textRange withText:text];
            }
            return NO;
        }
        
        NSUInteger rangeLength = textView.shouldCountingNonASCIICharacterAsTwo ? [textView.text substringWithRange:range].cigam_lengthWhenCountingNonASCIICharacterAsTwo : range.length;
        ...
        if (textWillOutofMaximumTextLength) {
            ...
            if (substringLength > 0 && [textView lengthWithString:text] > substringLength) {
                NSString *allowedText = [text cigam_substringAvoidBreakingUpCharacterSequencesWithRange:NSMakeRange(0, substringLength) lessValue:YES countingNonASCIICharacterAsTwo:textView.shouldCountingNonASCIICharacterAsTwo];
                if ([textView lengthWithString:allowedText] <= substringLength) {
                    [textView _cigam_setTextForShouldChange:[textView.text stringByReplacingCharactersInRange:range withString:allowedText]];
