...
//
//  CIGAMModalPresentationViewController.h
//  cigam
//
...
@class CIGAMModalPresentationViewController;
@class CIGAMModalPresentationWindow;

typedef NS_ENUM(NSUInteger, CIGAMModalPresentationAnimationStyle) {
    CIGAMModalPresentationAnimationStyleFade,    // 渐现渐隐，默认
    CIGAMModalPresentationAnimationStylePopup,   // 从中心点弹出
    CIGAMModalPresentationAnimationStyleSlide    // 从下往上升起
};

@protocol CIGAMModalPresentationContentViewControllerProtocol <NSObject>
...
@protocol CIGAMModalPresentationViewControllerDelegate <NSObject>
 ...
 *  支持 3 种方式显示浮层：
 *
 *  1. **推荐** 新起一个 `UIWindow` 盖在当前界面上，将 `CIGAMModalPresentationViewController` 以 `rootViewController` 的形式显示出来，可通过 `supportedOrientationMask` 支持横竖屏，不支持在浮层不消失的情况下做界面切换（因为 window 会把背后的 controller 盖住，看不到界面切换）。
 ...
 *  @endcode
 *
 *  3. 将浮层作为一个 subview 添加到 `superview` 上，从而能够实现在浮层不消失的情况下进行界面切换，但需要 `superview` 自行管理浮层的大小和横竖屏旋转，而且 `CIGAMModalPresentationViewController` 不能用局部变量来保存，会在显示后被释放，需要自行 retain。横竖屏跟随当前界面的设置。
 ...
 *
 *  @see CIGAMAlertController
 *  @see CIGAMDialogViewController
 *  @see CIGAMMoreOperationController
 */
@interface CIGAMModalPresentationViewController : UIViewController

@property(nullable, nonatomic, weak) IBOutlet id<CIGAMModalPresentationViewControllerDelegate> delegate;
 ...
 */
@property(nullable, nonatomic, strong) IBOutlet UIViewController<CIGAMModalPresentationContentViewControllerProtocol> *contentViewController;
 ...
 *
 *  `CIGAMModalPresentationViewController`会自动给自定义的`dimmingView`添加手势以实现点击遮罩隐藏浮层。
 */
...
/**
 *  设置要使用的显示/隐藏动画的类型，默认为`CIGAMModalPresentationAnimationStyleFade`。
 *  @warning 当使用了`showingAnimation`和`hidingAnimation`时，该属性无效
 */
@property(nonatomic, assign) CIGAMModalPresentationAnimationStyle animationStyle UI_APPEARANCE_SELECTOR;
...
/**
 *  管理自定义的浮层布局，将会在浮层显示前、控件的容器大小发生变化时（例如横竖屏、来电状态栏）被调用，请在 block 内主动为 view 设置期望的 frame，设置时建议用 cigam_frameApplyTransform 取代 setFrame:，否则在有键盘的情况下，显隐动画可能有错。
...
/**
 *  如果你有一个控件，内部通过 CIGAMModalPresentationViewController 实现显隐功能，那么这个控件建议实现这个协议，这样当 + [CIGAMModalPresentationViewController hideAllVisibleModalPresentationViewControllerIfCan] 被调用的时候，可以通过 hideModalPresentationComponent 来隐藏你的控件，否则会直接调用 CIGAMModalPresentationViewController 的 hide 方法，那样可能导致你的控件无法正确被隐藏。
 */
@protocol CIGAMModalPresentationComponentProtocol <NSObject>
...
@interface CIGAMModalPresentationViewController (Manager)
 ...
 *  @see    CIGAMModalPresentationComponentProtocol
 *  @warning 当要隐藏一个 modalPresentationViewController 时，如果这个 modal 有实现 CIGAMModalPresentationComponentProtocol 协议，则会调用它的 hideModalPresentationComponent 方法来隐藏，否则直接用 CIGAMModalPresentationViewController 的 hideWithAnimated:completion:
 */
+ (BOOL)hideAllVisibleModalPresentationViewControllerIfCan;
@end

@interface CIGAMModalPresentationViewController (UIAppearance)

+ (instancetype)appearance;

@end

/// 专用于CIGAMModalPresentationViewController的UIWindow，这样才能在`UIApplication.sharedApplication.windows`里方便地区分出来
@interface CIGAMModalPresentationWindow : UIWindow

@end


@interface UIViewController (CIGAMModalPresentationViewController)

/**
 *  获取弹出当前 vieController 的 CIGAMModalPresentationViewController
 */
@property(nullable, nonatomic, weak, readonly) CIGAMModalPresentationViewController *cigam_modalPresentationViewController;
@end
