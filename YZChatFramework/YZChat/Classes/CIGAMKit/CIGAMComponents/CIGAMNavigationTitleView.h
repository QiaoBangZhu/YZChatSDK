...
@protocol CIGAMNavigationTitleViewDelegate <NSObject>

@optional

/**
 点击 titleView 后的回调，只需设置 titleView.userInteractionEnabled = YES 后即可使用。不过一般都用于配合 CIGAMNavigationTitleViewAccessoryTypeDisclosureIndicator。

 @param titleView 被点击的 titleView
 @param isActive titleView 是否处于活跃状态（所谓的活跃，对应右边的箭头而言，就是点击后箭头向上的状态）
 */
- (void)didTouchTitleView:(CIGAMNavigationTitleView *)titleView isActive:(BOOL)isActive;
 ...
 */
- (void)didChangedActive:(BOOL)active forTitleView:(CIGAMNavigationTitleView *)titleView;

@end

/// 设置title和subTitle的布局方式，默认是水平布局。
typedef NS_ENUM(NSInteger, CIGAMNavigationTitleViewStyle) {
    CIGAMNavigationTitleViewStyleDefault,                // 水平
    CIGAMNavigationTitleViewStyleSubTitleVertical        // 垂直
};

/// 设置titleView的样式，默认没有任何修饰
typedef NS_ENUM(NSInteger, CIGAMNavigationTitleViewAccessoryType) {
    CIGAMNavigationTitleViewAccessoryTypeNone,                   // 默认
    CIGAMNavigationTitleViewAccessoryTypeDisclosureIndicator     // 有下拉箭头
};


/**
 *  可作为 UIViewController 顶部导航栏里的标题控件，通过 self.navigationItem.titleView 来设置。当调用 -[UIViewController setTitle:] 或 -[UINavigationItem setTitle:] 时，会自动更新 CIGAMNavigationTitleView 的内容。
 *
 ...
 *  2. 使用 CIGAMNavigationTitleViewDelegate 提供的接口。这种一般配合 titleView.accessoryType 来使用，这样就不用自己去做 accessoryView 的旋转、active 状态的维护等。
 */
@interface CIGAMNavigationTitleView : UIControl

@property(nonatomic, weak) id<CIGAMNavigationTitleViewDelegate> delegate;
@property(nonatomic, assign) CIGAMNavigationTitleViewStyle style;
...
/*
 *  当accessoryView不为空时，CIGAMNavigationTitleViewAccessoryType设置无效，一直都是None
 */
 ...
 */
@property(nonatomic, assign) CIGAMNavigationTitleViewAccessoryType accessoryType;
 ...
 *  @warn 为了美观考虑，该属性只对 CIGAMNavigationTitleViewStyleSubTitleVertical 有效
 */
