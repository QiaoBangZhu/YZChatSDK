...
/**
 *  配合 CIGAMImagePickerViewController 使用的工具类
 */
@interface CIGAMImagePickerHelper : NSObject
...
/**
 *  获取最近一次调用 updateLastAlbumWithAssetsGroup 方法调用时储存的 CIGAMAssetsGroup 对象
 *
 *  @param userIdentify 用户标识，由于每个用户可能需要分开储存一个最近调用过的 CIGAMAssetsGroup，因此增加一个标识区分用户。
 *  一个常见的应用场景是选择图片时保存图片所在相册的对应的 CIGAMAssetsGroup，并使用用户的 user id 作为区分不同用户的标识，
 *  当用户再次选择图片时可以根据已经保存的 CIGAMAssetsGroup 直接进入上次使用过的相册。
 */
+ (CIGAMAssetsGroup *)assetsGroupOfLastPickerAlbumWithUserIdentify:(NSString *)userIdentify;

/**
 *  储存一个 CIGAMAssetsGroup，从而储存一个对应的相册，与 assetsGroupOfLatestPickerAlbumWithUserIdentify 方法对应使用
 *
 *  @param assetsGroup   要被储存的 CIGAMAssetsGroup
