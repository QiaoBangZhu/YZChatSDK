...
//
//  CIGAMImagePickerViewController.m
//  cigam
//
...
#pragma mark - CIGAMImagePickerViewController (UIAppearance)

@implementation CIGAMImagePickerViewController (UIAppearance)
...
+ (void)initAppearance {
    CIGAMImagePickerViewController.appearance.minimumImageWidth = 75;
}

@end

#pragma mark - CIGAMImagePickerViewController

@interface CIGAMImagePickerViewController ()

@property(nonatomic, strong) CIGAMImagePickerPreviewViewController *imagePickerPreviewViewController;
@property(nonatomic, assign) BOOL isImagesAssetLoaded;// 这个属性的作用描述：https://github.com/Tencent/CIGAM_iOS/issues/219
...
@implementation CIGAMImagePickerViewController
...
- (void)setupNavigationItems {
    ...
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem cigam_itemWithTitle:@"取消" target:self action:@selector(handleCancelPickerImage:)];
}
...
- (void)viewDidLayoutSubviews {
    ...
    UIEdgeInsets contentInset = UIEdgeInsetsMake(self.cigam_navigationBarMaxYInViewCoordinator, self.collectionView.cigam_safeAreaInsets.left, MAX(operationToolBarViewHeight, self.collectionView.cigam_safeAreaInsets.bottom), self.collectionView.cigam_safeAreaInsets.right);
...
- (void)refreshWithAssetsGroup:(CIGAMAssetsGroup *)assetsGroup {
...
//        [self.selectedImageAssetArray removeAllObjects];
    ...
    // 通过 CIGAMAssetsGroup 获取该相册所有的图片 CIGAMAsset，并且储存到数组中
    CIGAMAlbumSortType albumSortType = CIGAMAlbumSortTypePositive;
    // 从 delegate 中获取相册内容的排序方式，如果没有实现这个 delegate，则使用 CIGAMAlbumSortType 的默认值，即最新的内容排在最后面
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [assetsGroup enumerateAssetsWithOptions:albumSortType usingBlock:^(CIGAMAsset *resultAsset) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (resultAsset) {
                    ...
                    self.isImagesAssetLoaded = YES;// 这个属性的作用描述： https://github.com/Tencent/CIGAM_iOS/issues/219
                    [self.collectionView reloadData];
...
- (void)scrollToInitialPositionIfNeeded {
    if (_collectionView.cigam_visible && self.isImagesAssetLoaded && !self.hasScrollToInitialPosition) {
        if ([self.imagePickerViewControllerDelegate respondsToSelector:@selector(albumSortTypeForImagePickerViewController:)] && [self.imagePickerViewControllerDelegate albumSortTypeForImagePickerViewController:self] == CIGAMAlbumSortTypeReverse) {
            [_collectionView cigam_scrollToTop];
        } else {
            [_collectionView cigam_scrollToBottom];
        }
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[CIGAMImagePickerCollectionViewCell class] forCellWithReuseIdentifier:kVideoCellIdentifier];
        [_collectionView registerClass:[CIGAMImagePickerCollectionViewCell class] forCellWithReuseIdentifier:kImageOrUnknownCellIdentifier];
        if (@available(iOS 11, *)) {
...
- (UIView *)operationToolBarView {
    if (!_operationToolBarView) {
        ...
        _operationToolBarView.cigam_borderPosition = CIGAMViewBorderPositionTop;
...
- (CIGAMButton *)sendButton {
    if (!_sendButton) {
        ...
        _sendButton.cigam_outsideEdge = UIEdgeInsetsMake(-12, -20, -12, -20);
...
- (CIGAMButton *)previewButton {
    if (!_previewButton) {
        _previewButton = [[CIGAMButton alloc] init];
        ...
        _previewButton.cigam_outsideEdge = UIEdgeInsetsMake(-12, -20, -12, -20);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    CIGAMAsset *imageAsset = [self.imagesAssetArray objectAtIndex:indexPath.item];
    ...
    CIGAMImagePickerCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    ...
    if (cell.selectable) {
        // 如果该图片的 CIGAMAsset 被包含在已选择图片的数组中，则控制该图片被选中
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    CIGAMAsset *imageAsset = self.imagesAssetArray[indexPath.item];
...
- (void)handleCheckBoxButtonClick:(UIButton *)checkboxButton {
    NSIndexPath *indexPath = [_collectionView cigam_indexPathForItemAtView:checkboxButton];
    ...
    CIGAMImagePickerCollectionViewCell *cell = (CIGAMImagePickerCollectionViewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
    CIGAMAsset *imageAsset = [self.imagesAssetArray objectAtIndex:indexPath.item];
    if (cell.checked) {
        // 移除选中状态
        ...
        if ([self.selectedImageAssetArray count] >= _maximumSelectImageCount) {
            ...
            CIGAMAlertController *alertController = [CIGAMAlertController alertControllerWithTitle:_alertTitleWhenExceedMaxSelectImageCount message:nil preferredStyle:CIGAMAlertControllerStyleAlert];
            [alertController addAction:[CIGAMAlertAction actionWithTitle:_alertButtonTitleWhenExceedMaxSelectImageCount style:CIGAMAlertActionStyleCancel handler:nil]];
...
- (void)updateImageCountAndCheckLimited {
    ...
    if (selectedImageCount > 0 && selectedImageCount >= _minimumSelectImageCount) {
        ...
        self.imageCountLabel.text = [NSString stringWithFormat:@"%@", @(selectedImageCount)];
        self.imageCountLabel.hidden = NO;
        [CIGAMImagePickerHelper springAnimationOfImageSelectedCountChangeWithCountLabel:self.imageCountLabel];
    } else {
        self.previewButton.enabled = NO;
        self.sendButton.enabled = NO;
        self.imageCountLabel.hidden = YES;
...
- (void)requestImageWithIndexPath:(NSIndexPath *)indexPath {
    ...
    imageAsset.requestID = [imageAsset requestOriginImageWithCompletion:^(UIImage *result, NSDictionary *info) {
        ...
        if (downloadSucceed) {
            ...
            cell.downloadStatus = CIGAMAssetDownloadStatusSucceed;
            
        } else if ([info objectForKey:PHImageErrorKey] ) {
            // 下载错误
            [imageAsset updateDownloadStatusWithDownloadResult:NO];
            cell.downloadStatus = CIGAMAssetDownloadStatusFailed;
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.collectionView cigam_itemVisibleAtIndexPath:indexPath]) {
                
                CIGAMLogInfo(@"CIGAMImagePickerLibrary", @"Download iCloud image, current progress is : %f", progress);
                
                if (cell.downloadStatus != CIGAMAssetDownloadStatusDownloading) {
                    cell.downloadStatus = CIGAMAssetDownloadStatusDownloading;
                    // 预先设置预览界面的下载状态
                    self.imagePickerPreviewViewController.downloadStatus = CIGAMAssetDownloadStatusDownloading;
                }
                if (error) {
