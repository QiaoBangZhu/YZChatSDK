...
NSString *const CIGAMThemeDidChangeNotification = @"CIGAMThemeDidChangeNotification";
...
@implementation CIGAMThemeManager
...
// 这个方法的声明放在 CIGAMThemeManagerCenter.m 里，简单实现 private 的效果
- (instancetype)initWithName:(__kindof NSObject<NSCopying> *)name {
    if (self = [super init]) {
        ...
        if (@available(iOS 13.0, *)) {
            [UITraitCollection cigam_addUserInterfaceStyleWillChangeObserver:self selector:@selector(handleUserInterfaceStyleWillChangeEvent:)];
        }
    }
    return self;
}
...
- (void)setCurrentThemeIdentifier:(NSObject<NSCopying> *)currentThemeIdentifier {
    ...
    NSAssert([self._themeIdentifiers containsObject:currentThemeIdentifier], @"%@ should be added to CIGAMThemeManager.themes before it becomes current theme identifier.", currentThemeIdentifier);
...
- (void)setCurrentTheme:(NSObject *)currentTheme {
    ...
    NSAssert([self._themes containsObject:currentTheme], @"%@ should be added to CIGAMThemeManager.themes before it becomes current theme.", currentTheme);
...
- (void)notifyThemeChanged {
    [[NSNotificationCenter defaultCenter] postNotificationName:CIGAMThemeDidChangeNotification object:self];
    ...
    [UIApplication.sharedApplication.windows enumerateObjectsUsingBlock:^(__kindof UIWindow * _Nonnull window, NSUInteger idx, BOOL * _Nonnull stop) {
        if (!window.hidden && window.alpha > 0.01 && window.rootViewController) {
            [window.rootViewController cigam_themeDidChangeByManager:self identifier:self.currentThemeIdentifier theme:self.currentTheme];
            
            // 某些 present style 情况下，window 上可能存在多个 viewController.view，因此需要遍历所有的 subviews，而不只是 window.rootViewController.view
            [window _cigam_themeDidChangeByManager:self identifier:self.currentThemeIdentifier theme:self.currentTheme shouldEnumeratorSubviews:YES];
        }
    }];
}
