 ...
 *  提供的属性请查看 @c CIGAMBadgeProtocol ，属性的默认值在 CIGAMConfigurationTemplate 配置表里设置，如果不使用配置表，则所有属性的默认值均为 0 或 nil。
 *
 *  @note 系统对 UIBarButtonItem 和 UITabBarItem 在横竖屏下均会有不同的布局，当你使用本控件时建议分别检查横竖屏下的表现是否正确。
 */
@interface UIBarItem (CIGAMBadge) <CIGAMBadgeProtocol>
