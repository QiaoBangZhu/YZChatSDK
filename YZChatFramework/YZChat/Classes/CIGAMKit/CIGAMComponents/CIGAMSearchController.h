...
@class CIGAMSearchController;

/**
 *  配合 CIGAMSearchController 使用的 protocol，主要负责两件事情：
 *
 ...
 */
@protocol CIGAMSearchControllerDelegate <CIGAMTableViewDataSource, CIGAMTableViewDelegate>
...
- (void)didDismissSearchController:(CIGAMSearchController *)searchController;
- (void)searchController:(CIGAMSearchController *)searchController didLoadSearchResultsTableView:(UITableView *)tableView;
- (void)searchController:(CIGAMSearchController *)searchController willShowEmptyView:(CIGAMEmptyView *)emptyView;
 ...
 *  4. 如果需要，可通过 @c cigam_preferredStatusBarStyleBlock 来控制搜索状态下的状态栏样式。
 *
 *  @note CIGAMCommonTableViewController 内部自带 CIGAMSearchController，只需将属性 shouldShowSearchBar 置为 YES 即可，无需自行初始化 CIGAMSearchController。
 */
@interface CIGAMSearchController : CIGAMCommonViewController<UISearchResultsUpdating, UISearchControllerDelegate>
...
@property(nonatomic, weak) id<CIGAMSearchControllerDelegate> searchResultsDelegate;
...
@interface CIGAMCommonTableViewController (Search) <CIGAMSearchControllerDelegate>
 ...
 *  默认为 `nil`
 *
 *  @see CIGAMTableViewDelegate
 */
@property(nonatomic, strong, readonly) CIGAMSearchController *searchController;
