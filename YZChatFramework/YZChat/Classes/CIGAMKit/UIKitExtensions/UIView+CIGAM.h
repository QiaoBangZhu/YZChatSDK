 ...
 */
@property(nonatomic, assign) CGRect cigam_frameApplyTransform;

/**
 在 iOS 11 及之后的版本，此属性将返回系统已有的 self.safeAreaInsets。在之前的版本此属性返回 UIEdgeInsetsZero
 */
@property(nonatomic, assign, readonly) UIEdgeInsets cigam_safeAreaInsets;

/**
 有修改过 tintColor，则不会再受 superview.tintColor 的影响
 */
@property(nonatomic, assign, readonly) BOOL cigam_tintColorCustomized;

/// 响应区域需要改变的大小，负值表示往外扩大，正值表示往内缩小
@property(nonatomic,assign) UIEdgeInsets cigam_outsideEdge;
...
+ (void)cigam_animateWithAnimated:(BOOL)animated duration:(NSTimeInterval)duration animations:(void (^ __nullable)(void))animations completion:(void (^ __nullable)(BOOL finished))completion;
 ...
 */
@property(nullable, nonatomic, copy) CGRect (^cigam_frameWillChangeBlock)(__kindof UIView *view, CGRect followingFrame);
 ...
 */
@property(nullable, nonatomic, copy) void (^cigam_frameDidChangeBlock)(__kindof UIView *view, CGRect precedingFrame);

/**
 在 - [UIView layoutSubviews] 调用后就调用的 block
 @param view 当前的 view 本身，方便使用，省去 weak 操作
 */
@property(nullable, nonatomic, copy) void (^cigam_layoutSubviewsBlock)(__kindof UIView *view);
 ...
 */
@property(nullable, nonatomic, copy) CGSize (^cigam_sizeThatFitsBlock)(__kindof UIView *view, CGSize size, CGSize superResult);

/**
 当 tintColorDidChange 被调用的时候会调用这个 block，就不用重写方法了
 @param view 当前的 view 本身，方便使用，省去 weak 操作
 */
@property(nullable, nonatomic, copy) void (^cigam_tintColorDidChangeBlock)(__kindof UIView *view);
...
/**
 ...
 @param super 的返回结果
 */
@property(nullable, nonatomic, copy) __kindof UIView * (^cigam_hitTestBlock)(CGPoint point, UIEvent *event, __kindof UIView *originalView);

@end

@interface UIView (CIGAM_ViewController)

/**
 判断当前的 view 是否属于可视（可视的定义为已存在于 view 层级树里，或者在所处的 UIViewController 的 [viewWillAppear, viewWillDisappear) 生命周期之间）
 */
@property(nonatomic, assign, readonly) BOOL cigam_visible;

/**
 当前的 view 是否是某个 UIViewController.view
 */
@property(nonatomic, assign) BOOL cigam_isControllerRootView;

/**
 获取当前 view 所在的 UIViewController，会递归查找 superview，因此注意使用场景不要有过于频繁的调用
 */
@property(nullable, nonatomic, weak, readonly) __kindof UIViewController *cigam_viewController;
@end


@interface UIView (CIGAM_Runtime)
 ...
 */
@interface UIView (CIGAM_Snapshotting)
...
/**
 ...
 // 现在可以这么写：
 view.frame = CGRectMake(x, y, width, CIGAMViewSelfSizingHeight);
 @endcode
 */
extern const CGFloat CIGAMViewSelfSizingHeight;

/**
 *  对 view.frame 操作的简便封装，注意 view 与 view 之间互相计算时，需要保证处于同一个坐标系内。
 */
@interface UIView (CIGAM_Layout)

/// 等价于 CGRectGetMinY(frame)
@property(nonatomic, assign) CGFloat cigam_top;

/// 等价于 CGRectGetMinX(frame)
@property(nonatomic, assign) CGFloat cigam_left;

/// 等价于 CGRectGetMaxY(frame)
@property(nonatomic, assign) CGFloat cigam_bottom;

/// 等价于 CGRectGetMaxX(frame)
@property(nonatomic, assign) CGFloat cigam_right;

/// 等价于 CGRectGetWidth(frame)
@property(nonatomic, assign) CGFloat cigam_width;

/// 等价于 CGRectGetHeight(frame)
@property(nonatomic, assign) CGFloat cigam_height;

/// 保持其他三个边缘的位置不变的情况下，将顶边缘拓展到某个指定的位置，注意高度会跟随变化。
@property(nonatomic, assign) CGFloat cigam_extendToTop;

/// 保持其他三个边缘的位置不变的情况下，将左边缘拓展到某个指定的位置，注意宽度会跟随变化。
@property(nonatomic, assign) CGFloat cigam_extendToLeft;

/// 保持其他三个边缘的位置不变的情况下，将底边缘拓展到某个指定的位置，注意高度会跟随变化。
@property(nonatomic, assign) CGFloat cigam_extendToBottom;

/// 保持其他三个边缘的位置不变的情况下，将右边缘拓展到某个指定的位置，注意宽度会跟随变化。
@property(nonatomic, assign) CGFloat cigam_extendToRight;

/// 获取当前 view 在 superview 内水平居中时的 left
@property(nonatomic, assign, readonly) CGFloat cigam_leftWhenCenterInSuperview;

/// 获取当前 view 在 superview 内垂直居中时的 top
@property(nonatomic, assign, readonly) CGFloat cigam_topWhenCenterInSuperview;
...
@property(nonatomic, assign, readonly) CGFloat cigam_scaleX;

/// 获取当前 view 的 transform scale y
@property(nonatomic, assign, readonly) CGFloat cigam_scaleY;

/// 获取当前 view 的 transform translation x
@property(nonatomic, assign, readonly) CGFloat cigam_translationX;

/// 获取当前 view 的 transform translation y
@property(nonatomic, assign, readonly) CGFloat cigam_translationY;
 ...
 */
@interface UIView (CIGAM_Debug)

/// 是否需要添加debug背景色，默认NO
@property(nonatomic, assign) BOOL cigam_shouldShowDebugColor;
/// 是否每个view的背景色随机，如果不随机则统一使用半透明红色，默认NO
@property(nonatomic, assign) BOOL cigam_needsDifferentDebugColor;
/// 标记一个view是否已经被添加了debug背景色，外部一般不使用
@property(nonatomic, assign, readonly) BOOL cigam_hasDebugColor;
