 ...
 *
 *  此时可以将 UIControl.cigam_automaticallyAdjustTouchHighlightedInScrollView 属性置为 YES，会使用自己的一套计算方式去判断触发 setHighlighted 的时机，从而保证既不影响 UIScrollView 的滚动，又能让 UIControl 在被快速点击时也能立马看到 setHighlighted 的效果。
 *
 ...
 */
@property(nonatomic, assign) BOOL cigam_automaticallyAdjustTouchHighlightedInScrollView;
...
/**
 ...
 @note 系统默认就会对同一点击区域短时间内触发的多次 touch 都归到同一组，所以如果10s内连续不断地快速点击同一个按钮，这10s的时间里也只会触发一次 UIControlEventTouchUpInside，因为这10s里的所有 touch 都被归到同一组事件里。但如果通过定时器实现，假设以1s为临界点，那么这10s的快速点击就会触发十次。CIGAM 的实现采用的是前一种。
 
 @warning 不能与 @c cigam_automaticallyAdjustTouchHighlightedInScrollView 同时开启。
 */
@property(nonatomic, assign) BOOL cigam_preventsRepeatedTouchUpInsideEvent;

/// setHighlighted: 方法的回调 block
@property(nonatomic, copy) void (^cigam_setHighlightedBlock)(BOOL highlighted);

/// 等同于 addTarget:action:forControlEvents:UIControlEventTouchUpInside
@property(nonatomic, copy) void (^cigam_tapBlock)(__kindof UIControl *sender);
