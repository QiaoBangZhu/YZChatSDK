...
//  cigam
//
//  Created by CIGAM Team on 2017/2/14.
//
...
NSInteger const kCIGAMLastTouchedTabBarItemIndexNone = -1;
NSString *const kCIGAMShouldCheckTabBarHiddenKey = @"kCIGAMShouldCheckTabBarHiddenKey";
...
@property(nonatomic, assign) BOOL cigamtb_hasSetEffect;
@property(nonatomic, assign) BOOL cigamtb_hasSetEffectForegroundColor;
@end

@implementation UITabBar (CIGAM)

CIGAMSynthesizeBOOLProperty(canItemRespondDoubleTouch, setCanItemRespondDoubleTouch)
CIGAMSynthesizeNSIntegerProperty(lastTouchedTabBarItemViewIndex, setLastTouchedTabBarItemViewIndex)
CIGAMSynthesizeNSIntegerProperty(tabBarItemViewTouchCount, setTabBarItemViewTouchCount)
CIGAMSynthesizeBOOLProperty(cigamtb_hasSetEffect, setCigamtb_hasSetEffect)
CIGAMSynthesizeBOOLProperty(cigamtb_hasSetEffectForegroundColor, setCigamtb_hasSetEffectForegroundColor)
    ...
    dispatch_once(&onceToken, ^{
        ...
        ExtendImplementationOfVoidMethodWithoutArguments([UITabBarController class], @selector(viewDidLoad), ^(UITabBarController *selfObject) {
            if (CIGAMCMIActivated) {
                if (@available(iOS 13.0, *)) {
                    // iOS 13 不使用 tintColor 了，改为用 UITabBarAppearance，具体请看 CIGAMConfiguration.m
                } else {
        ...
        if (@available(iOS 13.0, *)) {
            ...
            ExtendImplementationOfVoidMethodWithoutArguments(NSClassFromString(@"_UITabBarVisualProviderLegacyIOS"), NSSelectorFromString(@"_updateBackground"), ^(NSObject *selfObject) {
                UITabBar *tabBar = [selfObject cigam_valueForKey:@"tabBar"];
                if (!tabBar) return;
                UIImage *shadowImage = tabBar.shadowImage;// 就算 tabBar 显示系统的分隔线，但依然能从 shadowImage 属性获取到业务自己设置的图片
                UIImageView *shadowImageView = tabBar.cigam_shadowImageView;
        ...
        OverrideImplementation([UITabBar class], @selector(setItems:animated:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^void(UITabBar *selfObject, NSArray<UITabBarItem *> *items, BOOL animated) {
                ...
                [items enumerateObjectsUsingBlock:^(UITabBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
                    // 双击 tabBarItem 的功能需要在设置完 item 后才能获取到 cigam_view 来实现
                    UIControl *itemView = (UIControl *)item.cigam_view;
        ...
        OverrideImplementation([UITabBar class], @selector(setFrame:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITabBar *selfObject, CGRect frame) {
                if (CIGAMCMIActivated && ShouldFixTabBarTransitionBugInIPhoneX && IOS_VERSION < 11.2 && IS_58INCH_SCREEN) {
                    if (CGRectGetHeight(frame) == TabBarHeight && CGRectGetMaxY(frame) < CGRectGetHeight(selfObject.superview.bounds)) {
                        ...
                        // https://github.com/Tencent/CIGAM_iOS/issues/217
                        frame = CGRectSetY(frame, CGRectGetHeight(selfObject.superview.bounds) - CGRectGetHeight(frame));
                    }
                }
                
                // 修复这个 bug：https://github.com/Tencent/CIGAM_iOS/issues/309
                if (@available(iOS 11, *)) {
                    if (IS_NOTCHED_SCREEN && ((CGRectGetHeight(frame) == 49 || CGRectGetHeight(frame) == 32))) {// 只关注全面屏设备下的这两种非正常的 tabBar 高度即可
                        ...
                        if (bottomSafeAreaInsets == CGRectGetHeight(selfObject.frame)) {
                            return;// 由于这个系统 bug https://github.com/Tencent/CIGAM_iOS/issues/446，这里先暂时屏蔽本次 frame 变化
                        }
            ...
            if (@available(iOS 12.1.1, *)) {
                ...
                OverrideImplementation(NSClassFromString(@"UITabBarButton"), @selector(setFrame:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                    return ^(UIView *selfObject, CGRect firstArgv) {
                        ...
                        // https://github.com/Tencent/CIGAM_iOS/issues/410
                        ...
                        // https://github.com/Tencent/CIGAM_iOS/issues/422
        ...
        if (@available(iOS 13.0, *)) {
            ...
            UITabBarItem *(^tabBarItemOfLabelBlock)(UILabel *label) = ^UITabBarItem *(UILabel *label) {
                UIControl *tabBarButton = [label cigam_valueForKey:@"_tabBarButton"];
                UITabBar *tabBar = [tabBarButton cigam_valueForKey:@"tabBar"];
                ...
                [tabBar.items enumerateObjectsUsingBlock:^(UITabBarItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if (obj.cigam_view == tabBarButton) {
                        tabBarItem = obj;
                        *stop = YES;
                    }
                }];
            ...
            OverrideImplementation(tabBarButtonLabelClass, NSSelectorFromString(@"_setUnselectedTintColor:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UILabel *selfObject, UIColor *firstArgv) {
                    ...
                    if (item) {
                        UITabBar *tabBar = [[selfObject cigam_valueForKey:@"_tabBarButton"] cigam_valueForKey:@"tabBar"];
                        NSDictionary<NSAttributedStringKey,id> *normalAttributes = [item titleTextAttributesForState:UIControlStateNormal] ?: [UITabBarItem.cigam_appearanceConfigured titleTextAttributesForState:UIControlStateNormal];
            ...
            OverrideImplementation(tabBarButtonLabelClass, @selector(setSelected:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UILabel *selfObject, BOOL selected) {
                    ...
                    NSDictionary<NSAttributedStringKey,id> *normalAttributes = [item titleTextAttributesForState:UIControlStateNormal] ?: [UITabBarItem.cigam_appearanceConfigured titleTextAttributesForState:UIControlStateNormal];
                    NSDictionary<NSAttributedStringKey,id> *selectedAttributes = [item titleTextAttributesForState:UIControlStateSelected] ?: [UITabBarItem.cigam_appearanceConfigured titleTextAttributesForState:UIControlStateSelected];
        ...
        // iOS 14.0 测试过已不存在该问题
        // https://github.com/Tencent/CIGAM_iOS/issues/740
        if (@available(iOS 13.0, *)) {
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1110
        if (@available(iOS 14.0, *)) {
            OverrideImplementation(NSClassFromString(@"UITabBarButtonLabel"), @selector(sizeThatFits:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^CGSize(UILabel *selfObject, CGSize firstArgv) {
                    ...
                    if ([font respondsToSelector:selectorForCSSFontFamily]) {
                        BOOL forWebKit = YES;
                        NSString *fontFamily = [font cigam_performSelector:selectorForCSSFontFamily withArguments:&forWebKit, nil];
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1100
        if (@available(iOS 14.0, *)) {
            if (@available(iOS 14.2, *)) {
            } else {
                OverrideImplementation([UINavigationController class], @selector(cigam_didInitialize), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                            ...
                            switch (action) {
                                ...
                                case CIGAMNavigationActionWillSet: {
                                    ...
                                    if (animated && weakNavigationController.tabBarController && !appearingViewController.hidesBottomBarWhenPushed) {
                                        ...
                                        // https://github.com/Tencent/CIGAM_iOS/issues/1177
                                        ...
                                        if (index != NSNotFound) {
                                            ...
                                            if (!systemShouldHideTabBar) {
                                                [weakNavigationController cigam_bindBOOL:YES forKey:kCIGAMShouldCheckTabBarHiddenKey];
                                            }
                                        }
                                    }
                                }
                                ...
                                case CIGAMNavigationActionDidSet: {
                                    [weakNavigationController cigam_bindBOOL:NO forKey:kCIGAMShouldCheckTabBarHiddenKey];
                                }
                                    break;
                ...
                OverrideImplementation([UINavigationController class], NSSelectorFromString(@"_shouldBottomBarBeHidden"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                    return ^BOOL(UINavigationController *selfObject) {
                        ...
                        if ([selfObject cigam_getBoundBOOLForKey:kCIGAMShouldCheckTabBarHiddenKey]) {
                            result = NO;
                        }
                        return result;
                    };
        ...
        // 但这样有个风险，因为 CIGAMConfiguration 配置表里都是用 appearance 的方式去设置 standardAppearance，所以如果在 UITabBar 实例被添加到 window 之前修改过旧版任意一个样式接口，就会导致一个新的 UITabBarAppearance 对象被设置给 standardAppearance 属性，这样系统就会认为你这个 UITabBar 实例自定义了 standardAppearance，那么当它被 moveToWindow 时就不会自动应用 appearance 的值了，因此需要保证在添加到 window 前不要自行修改属性
        if (@available(iOS 13.0, *)) {
            ...
            void (^syncAppearance)(UITabBar *, void(^barActionBlock)(UITabBarAppearance *appearance), void (^itemActionBlock)(UITabBarItemAppearance *itemAppearance)) = ^void(UITabBar *tabBar, void(^barActionBlock)(UITabBarAppearance *appearance), void (^itemActionBlock)(UITabBarItemAppearance *itemAppearance)) {
                ...
                if (itemActionBlock) {
                    [appearance cigam_applyItemAppearanceWithBlock:itemActionBlock];
                }
...
- (void)handleTabBarItemViewEvent:(UIControl *)itemView {
    ...
    if (!self.selectedItem.cigam_doubleTapBlock) {
        return;
    }
    ...
    if (self.lastTouchedTabBarItemViewIndex == kCIGAMLastTouchedTabBarItemIndexNone) {
        // 记录第一次点击的 index
    ...
    if (self.tabBarItemViewTouchCount == 2) {
        ...
        if (item.cigam_doubleTapBlock) {
            item.cigam_doubleTapBlock(item, selectedIndex);
        }
        [self revertTabBarItemTouch];
    }
}
...
- (void)revertTabBarItemTouch {
    self.lastTouchedTabBarItemViewIndex = kCIGAMLastTouchedTabBarItemIndexNone;
    self.tabBarItemViewTouchCount = 0;
}

- (UIVisualEffectView *)cigam_effectView {
    for (UIView *subview in self.cigam_backgroundView.subviews) {
...
- (void)cigamtb_swizzleBackgroundView {
        ...
        OverrideImplementation(backgroundClass, @selector(didAddSubview:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIView *selfObject, UIView *subview) {
                ...
                if ([selfObject.superview isKindOfClass:UITabBar.class] && [subview isMemberOfClass:UIVisualEffectView.class]) {
                    ...
                    if (tabBar.cigamtb_hasSetEffect || tabBar.cigamtb_hasSetEffectForegroundColor) {
                        [tabBar cigamtb_updateEffect];
                    }
                }
            };
        });
        ...
        OverrideImplementation(UIVisualEffectView.class, NSSelectorFromString(@"setBackgroundEffects:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UIVisualEffectView *selfObject, NSArray<UIVisualEffect *> *firstArgv) {
                ...
                if ([selfObject.superview isKindOfClass:backgroundClass] && [selfObject.superview.superview isKindOfClass:UITabBar.class]) {
                    ...
                    if (tabBar.cigam_effectView == selfObject) {
                        if (tabBar.cigamtb_hasSetEffect) {
                            firstArgv = tabBar.cigamtb_backgroundEffects;
                        }
                    }
                }
    ...
    } oncePerIdentifier:@"UITabBar (CIGAM) effect"];
}

- (void)cigamtb_updateEffect {
    if (self.cigamtb_hasSetEffect) {
        ...
        NSArray<UIVisualEffect *> *effects = self.cigamtb_backgroundEffects;
        [self.cigam_effectView cigam_performSelector:NSSelectorFromString(@"setBackgroundEffects:") withArguments:&effects, nil];
    }
    if (self.cigamtb_hasSetEffectForegroundColor) {
        self.cigam_effectView.cigam_foregroundColor = self.cigam_effectForegroundColor;
    }
}

// UITabBar、UIVisualEffectView  都有一个私有的方法 backgroundEffects，当 UIVisualEffectView 应用于 UITabBar 场景时，磨砂的效果实际上被放在 backgroundEffects 内，而不是公开接口的 effect 属性里，这里为了方便，将 UITabBar (CIGAM).effect 转成可用于 backgroundEffects 的数组
- (NSArray<UIVisualEffect *> *)cigamtb_backgroundEffects {
    if (self.cigamtb_hasSetEffect) {
        return self.cigam_effect ? @[self.cigam_effect] : nil;
    }
    return nil;
}

static char kAssociatedObjectKey_effect;
- (void)setCigam_effect:(UIBlurEffect *)cigam_effect {
    if (cigam_effect) {
        [self cigamtb_swizzleBackgroundView];
    }
    
    BOOL valueChanged = self.cigam_effect != cigam_effect;
    objc_setAssociatedObject(self, &kAssociatedObjectKey_effect, cigam_effect, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (valueChanged) {
        self.cigamtb_hasSetEffect = YES;// CIGAMTheme 切换时会重新赋值，所以可能出现本来就是 nil，还给它又赋值了 nil，这种场景不应该导致 hasSet 标志位改变，所以要把标志位的设置放在 if (valueChanged) 里
        [self cigamtb_updateEffect];
    }
}

- (UIBlurEffect *)cigam_effect {
...
- (void)setCigam_effectForegroundColor:(UIColor *)cigam_effectForegroundColor {
    if (cigam_effectForegroundColor) {
        [self cigamtb_swizzleBackgroundView];
    }
    BOOL valueChanged = ![self.cigam_effectForegroundColor isEqual:cigam_effectForegroundColor];
    objc_setAssociatedObject(self, &kAssociatedObjectKey_effectForegroundColor, cigam_effectForegroundColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (valueChanged) {
        self.cigamtb_hasSetEffectForegroundColor = YES;// CIGAMTheme 切换时会重新赋值，所以可能出现本来就是 nil，还给它又赋值了 nil，这种场景不应该导致 hasSet 标志位改变，所以要把标志位的设置放在 if (valueChanged) 里
        [self cigamtb_updateEffect];
    }
}

- (UIColor *)cigam_effectForegroundColor {
    return (UIColor *)objc_getAssociatedObject(self, &kAssociatedObjectKey_effectForegroundColor);
}

@end

@implementation UITabBarAppearance (CIGAM)

- (void)cigam_applyItemAppearanceWithBlock:(void (^)(UITabBarItemAppearance * _Nonnull))block {
