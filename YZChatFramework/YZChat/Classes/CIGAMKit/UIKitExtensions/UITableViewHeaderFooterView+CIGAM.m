...
//
//  UITableViewHeaderFooterView+CIGAM.m
//  CIGAMKit
//
...
@implementation UITableViewHeaderFooterView (CIGAM)
...
@implementation UITableViewHeaderFooterView (CIGAM_InsetGrouped)
                    ...
                    [CIGAMHelper executeBlock:^{
                        OverrideImplementation(selfObject.class, @selector(sizeThatFits:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                            return ^CGSize(UITableViewHeaderFooterView *view, CGSize size) {
                                ...
                                BOOL shouldChangeWidth = view.cigam_tableView && view.cigam_tableView.cigam_style == CIGAMTableViewStyleInsetGrouped;
                                if (shouldChangeWidth) {
                                    size.width = size.width - UIEdgeInsetsGetHorizontalValue(view.cigam_tableView.cigam_safeAreaInsets) - view.cigam_tableView.cigam_insetGroupedHorizontalInset * 2;
                                }
        ...
        if (@available(iOS 11.0, *)) {
            ...
            OverrideImplementation([UITableViewHeaderFooterView class], NSSelectorFromString(@"_contentRectForWidth:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^CGRect(UITableViewHeaderFooterView *selfObject, CGFloat firstArgv) {
                    BOOL shouldChangeWidth = firstArgv > 0 && selfObject.cigam_tableView && selfObject.cigam_tableView.cigam_style == CIGAMTableViewStyleInsetGrouped;
                    if (shouldChangeWidth) {
                        firstArgv -= UIEdgeInsetsGetHorizontalValue(selfObject.cigam_tableView.cigam_safeAreaInsets) + selfObject.cigam_tableView.cigam_insetGroupedHorizontalInset * 2;
                    }
                    ...
                    if (shouldChangeWidth) {
                        result = CGRectSetX(result, selfObject.cigam_tableView.cigam_safeAreaInsets.left + selfObject.cigam_tableView.cigam_insetGroupedHorizontalInset);
                    }
                    return result;
                };
            });
            ...
            OverrideImplementation([UITableViewHeaderFooterView class], @selector(layoutSubviews), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UITableViewHeaderFooterView *selfObject) {
                    ...
                    if (selfObject.cigam_tableView && selfObject.cigam_tableView.cigam_style == CIGAMTableViewStyleInsetGrouped) {
                        selfObject.contentView.frame = CGRectMake(selfObject.cigam_tableView.cigam_safeAreaInsets.left + selfObject.cigam_tableView.cigam_insetGroupedHorizontalInset, CGRectGetMinY(selfObject.contentView.frame), selfObject.cigam_tableView.cigam_validContentWidth, CGRectGetHeight(selfObject.bounds));
                    }
                };
            });
        }
