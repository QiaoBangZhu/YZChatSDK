...
/**
 ...
 因此当我们在某些情况下利用 UIWindow 去实现遮罩、浮层等效果时，会错误地导致原来的 window 内的 viewController 丢失了对 statusBar 的控制权（因为你新加的 window 满足了上文所有条件），为了避免这种情况，可以将你自己的 window.cigam_capturesStatusBarAppearance = NO，这样你的 window 就不会影响原 window 对 statusBar 的控制权。同理，如果你的 window 本身就不需要盖住整个屏幕，那就算你不设置 cigam_capturesStatusBarAppearance 也不会影响原 window 的表现。
 
 @warning 如果你自己创建的 window 不满足以上2点，那么就算 cigam_capturesStatusBarAppearance 为 YES，也无法得到 statusBar 的控制权。
 */
@property(nonatomic, assign) BOOL cigam_capturesStatusBarAppearance;
@end
