 ...
 */
//
//  UITraitCollection+CIGAM.m
//  CIGAMKit
//
//  Created by ziezheng on 2019/7/19.
//

#import "UITraitCollection+CIGAM.h"
#import "CIGAMCore.h"
#import <dlfcn.h>

@implementation UITraitCollection (CIGAM)

static NSHashTable *_eventObservers;
static NSString * const kCIGAMUserInterfaceStyleWillChangeSelectorsKey = @"cigam_userInterfaceStyleWillChangeObserver";

+ (void)cigam_addUserInterfaceStyleWillChangeObserver:(id)observer selector:(SEL)aSelector {
    if (@available(iOS 13.0, *)) {
        @synchronized (self) {
            [UITraitCollection _cigam_overrideTraitCollectionMethodIfNeeded];
            if (!_eventObservers) {
                _eventObservers = [NSHashTable weakObjectsHashTable];
            }
            NSMutableSet *selectors = [observer cigam_getBoundObjectForKey:kCIGAMUserInterfaceStyleWillChangeSelectorsKey];
            if (!selectors) {
                selectors = [NSMutableSet set];
                [observer cigam_bindObject:selectors forKey:kCIGAMUserInterfaceStyleWillChangeSelectorsKey];
            }
...
+ (void)_cigam_notifyUserInterfaceStyleWillChangeEvents:(UITraitCollection *)traitCollection {
...
+ (void)load {
    // 以下代码只会在 DEBUG 生效，主要是屏蔽 Main Thread Checker 对 CIGAM swizzle traitCollection 的检测
    // iOS 14 首次弹起键盘，UIKit 内部会在在子线程访问 -[UIWindow traitCollection]，该方法一旦被 swizzle，Main Thread Checker 就会误判为业务在子线程主动调用 UIKit 方法从而引发卡顿和警告。 https://github.com/Tencent/CIGAM_iOS/issues/1087
    // Main Thread Checker 的原理是在启动的时候替换相关方法的实现为 Main Thread Checker 自身的 trampoline，触发相关方法时会先在 trampoline 中实现线程检测逻辑并告警，所以这里唯一可行的屏蔽方法就是获取原始的 IMP 实现，并直接调用从而绕过 Main Thread Checker, 由于 CIGAM 在 +load 到时候已经晚于这个时机，无法获取原始的实现方法，观察发现 -[UIWindow traitCollection] 内部会调用 -[UIWindow _updateWindowTraitsAndNotify:]，因此可以借助该方法回溯到 traitCollection 的真实地址。
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (cigam_exists_dyld_image("libMainThreadChecker.dylib")) {
...
+ (void)_cigam_overrideTraitCollectionMethodIfNeeded {
    if (@available(iOS 13.0, *)) {
        [CIGAMHelper executeBlock:^{
            static BOOL _isOverridedMethodProcessing = NO;
            static UIUserInterfaceStyle cigam_lastNotifiedUserInterfaceStyle;
            cigam_lastNotifiedUserInterfaceStyle = [UITraitCollection currentTraitCollection].userInterfaceStyle;
                ...
                return ^UITraitCollection *(UIWindow *selfObject) {
                    ...
                    if (selfObject.windowScene && !snapshotFinishedOnBackground) {
                        ...
                        if (selfObject == firstValidatedWindow) {
                            if (cigam_lastNotifiedUserInterfaceStyle != traitCollection.userInterfaceStyle) {
                                cigam_lastNotifiedUserInterfaceStyle = traitCollection.userInterfaceStyle;
                                [self _cigam_notifyUserInterfaceStyleWillChangeEvents:traitCollection];
                            }
