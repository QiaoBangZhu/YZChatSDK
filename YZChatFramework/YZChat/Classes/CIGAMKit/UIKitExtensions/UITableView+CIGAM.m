...
const NSUInteger kCIGAMFloatValuePrecision = 4;// 统一一个小数点运算精度

@interface UITableView ()
@property(nonatomic, assign, readwrite) UITableViewStyle cigam_style;
@property(nonatomic, assign, readonly) CGRect cigam_indexFrame;
@end

@implementation UITableView (CIGAM)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UITableView class], @selector(initWithFrame:style:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^UITableView *(UITableView *selfObject, CGRect firstArgv, UITableViewStyle secondArgv) {
                ...
                if (@available(iOS 13.0, *)) {
                    // iOS 13 cigam_style 的 getter 直接返回 tableView.style，所以这里不需要给 cigam_style 赋值
                } else {
                    selfObject.cigam_style = secondArgv;
                    if (secondArgv == CIGAMTableViewStyleInsetGrouped) {
                ...
                // 而当使用 estimate 效果时，会导致 contentSize 之类的计算不准确，所以这里给一个途径让项目可以方便地控制 UITableView（不包含子类，例如 UIPickerTableView）的 estimatedRowHeight 效果的开关，至于 CIGAMTableView 会在自己内部 init 时调用
                // https://github.com/Tencent/CIGAM_iOS/issues/313
                if (CIGAMCMIActivated && [NSStringFromClass(selfObject.class) isEqualToString:@"UITableView"]) {
                    [selfObject _cigam_configEstimatedRowHeight];
                }
        ...
        OverrideImplementation([UITableView class], @selector(scrollToRowAtIndexPath:atScrollPosition:animated:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITableView *selfObject, NSIndexPath *indexPath, UITableViewScrollPosition scrollPosition, BOOL animated) {
                ...
                if (!isIndexPathLegal) {
                    CIGAMLogWarn(@"UITableView (CIGAM)", @"%@ - target indexPath : %@ ，不合法的indexPath。\n%@", selfObject, indexPath, [NSThread callStackSymbols]);
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1161
        void (^fixBugOfTableViewContentSize)(UITableView *) = ^void(UITableView *tableView) {
            BOOL estimatesRowHeight = NO;
            [tableView cigam_performSelector:NSSelectorFromString(@"_estimatesRowHeights") withPrimitiveReturnValue:&estimatesRowHeight];
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1215
        if (@available(iOS 11.0, *)) {
            OverrideImplementation([UITableView class], @selector(reloadData), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UITableView *selfObject) {
                    ...
                    if (!selfObject.window && selfObject.superview && [selfObject.tableHeaderView isKindOfClass:UISearchBar.class]) {
                        [selfObject cigam_bindBOOL:YES forKey:@"kShouldFixContentSizeBugKey"];
                    }
                };
            });
            OverrideImplementation([UITableView class], @selector(didMoveToWindow), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^(UITableView *selfObject) {
                    ...
                    if ([selfObject cigam_getBoundBOOLForKey:@"kShouldFixContentSizeBugKey"]) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [selfObject reloadData];
                        });
                        [selfObject cigam_bindBOOL:NO forKey:@"kShouldFixContentSizeBugKey"];
                    }
                };
            });
        }
        ...
        // https://github.com/Tencent/CIGAM_iOS/issues/1213
        OverrideImplementation([UITableView class], NSSelectorFromString(@"_removeIndex"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITableView *selfObject) {
                ...
                if ([searchBar isKindOfClass:UISearchBar.class]) {
                    ...
                    [searchBar cigam_performSelector:NSSelectorFromString(@"_updateInsetsForTableView:") withArguments:&selfObject, nil];
                }
            };
        });
    });
...
- (void)cigam_scrollToRowAtIndexPath:(NSIndexPath *)indexPath atScrollPosition:(UITableViewScrollPosition)scrollPosition animated:(BOOL)animated {
    if (!indexPath) {
        return;
    }
    ...
    if (!isIndexPathLegal) {
        ...
        [self cigam_scrollToRowAtIndexPath:indexPath atScrollPosition:scrollPosition animated:animated];
    }
}

- (void)cigam_styledAsCIGAMTableView {
    
    if (!CIGAMCMIActivated) return;
    
    [self _cigam_configEstimatedRowHeight];
    
    self.backgroundColor = PreferredValueForTableViewStyle(self.cigam_style, TableViewBackgroundColor, TableViewGroupedBackgroundColor, TableViewInsetGroupedBackgroundColor);
    self.separatorColor = PreferredValueForTableViewStyle(self.cigam_style, TableViewSeparatorColor, TableViewGroupedSeparatorColor, TableViewInsetGroupedSeparatorColor);
    
    // 去掉空白的cell
    if (self.cigam_style == UITableViewStylePlain) {
    ...
    self.cigam_insetGroupedCornerRadius = TableViewInsetGroupedCornerRadius;
    self.cigam_insetGroupedHorizontalInset = TableViewInsetGroupedHorizontalInset;
}

- (void)_cigam_configEstimatedRowHeight {
...
- (NSIndexPath *)cigam_indexPathForRowAtView:(UIView *)view {
    ...
    return [self cigam_indexPathForRowAtView:view.superview];
}

- (NSInteger)cigam_indexForSectionHeaderAtView:(UIView *)view {
    ...
    origin = CGPointToFixed(origin, kCIGAMFloatValuePrecision);// 避免一些浮点数精度问题导致的计算错误
    ...
    while (low <= high) {
        ...
        rectForSection = CGRectToFixed(rectForSection, kCIGAMFloatValuePrecision);
...
- (NSArray<NSNumber *> *)cigam_indexForVisibleSectionHeaders {
    ...
    for (NSInteger i = 0; i < visibleSections.count; i++) {
        ...
        if ([self cigam_isHeaderVisibleForSection:section]) {
...
- (NSInteger)cigam_indexOfPinnedSectionHeader {
    NSArray<NSNumber *> *visibleSectionIndex = [self cigam_indexForVisibleSectionHeaders];
    for (NSInteger i = 0; i < visibleSectionIndex.count; i++) {
        NSInteger section = visibleSectionIndex[i].integerValue;
        if ([self cigam_isHeaderPinnedForSection:section]) {
            return section;
        } else {
            continue;
        }
...
- (BOOL)cigam_isHeaderPinnedForSection:(NSInteger)section {
    if (self.cigam_style != UITableViewStylePlain) return NO;
    ...
    BOOL isSectionScrollIntoContentInsetTop = self.contentOffset.y + self.cigam_contentInset.top > CGRectGetMinY(rectForSection);// 表示这个 section 已经往上滚动，超过 contentInset.top 那条线了
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.cigam_contentInset.top <= CGRectGetMaxY(rectForSection) - CGRectGetHeight(rectForHeader);// 表示这个 section 还没被完全滚走
    BOOL isPinned = isSectionScrollIntoContentInsetTop && isSectionStayInContentInsetTop;
    return isPinned;
}

- (BOOL)cigam_isHeaderVisibleForSection:(NSInteger)section {
    if (self.cigam_style != UITableViewStylePlain) return NO;
    if (section >= [self numberOfSections]) return NO;
    ...
    BOOL isSectionStayInContentInsetTop = self.contentOffset.y + self.cigam_contentInset.top < CGRectGetMaxY(rectForSection);// 表示这个 section 还没被完全滚走
    BOOL isVisible = isSectionScrollIntoBounds && isSectionStayInContentInsetTop;
    return isVisible;
}

- (CIGAMTableViewCellPosition)cigam_positionForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSInteger numberOfRowsInSection = [self.dataSource tableView:self numberOfRowsInSection:indexPath.section];
    if (numberOfRowsInSection == 1) {
        return CIGAMTableViewCellPositionSingleInSection;
    }
    if (indexPath.row == 0) {
        return CIGAMTableViewCellPositionFirstInSection;
    }
    if (indexPath.row == numberOfRowsInSection - 1) {
        return CIGAMTableViewCellPositionLastInSection;
    }
    return CIGAMTableViewCellPositionMiddleInSection;
}

- (BOOL)cigam_cellVisibleAtIndexPath:(NSIndexPath *)indexPath {
    NSArray<NSIndexPath *> *visibleCellIndexPaths = self.indexPathsForVisibleRows;
...
- (void)cigam_scrollToRowFittingOffsetY:(CGFloat)offsetY atIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
    [self alertEstimatedHeightUsageIfDetected];
    ...
    if (canScrollRowToTop) {
        ...
        [self cigam_scrollToBottomAnimated:animated];
    }
}

- (CGFloat)cigam_validContentWidth {
    CGRect indexFrame = self.cigam_indexFrame;
    CGFloat rightInset = MAX(self.cigam_safeAreaInsets.right + (self.cigam_style == CIGAMTableViewStyleInsetGrouped ? self.cigam_insetGroupedHorizontalInset : 0), CGRectGetWidth(indexFrame));
    CGFloat leftInset = self.cigam_safeAreaInsets.left + (self.cigam_style == CIGAMTableViewStyleInsetGrouped ? self.cigam_insetGroupedHorizontalInset : 0);
    CGFloat width = CGRectGetWidth(self.bounds) - leftInset - rightInset;
    return width;
}

- (CGSize)cigam_realContentSize {
    [self alertEstimatedHeightUsageIfDetected];
...
- (BOOL)cigam_canScroll {
    ...
    if ([self.tableHeaderView isKindOfClass:[UISearchBar class]]) {
        BOOL canScroll = self.cigam_realContentSize.height + UIEdgeInsetsGetVerticalValue(self.cigam_contentInset) > CGRectGetHeight(self.bounds);
        return canScroll;
    } else {
        return [super cigam_canScroll];
    }
}
...
- (void)alertEstimatedHeightUsageIfDetected {
    ...
    if (usingEstimatedRowHeight || usingEstimatedSectionHeaderHeight || usingEstimatedSectionFooterHeight) {
        [self CIGAMSymbolicUsingTableViewEstimatedHeightMakeWarning];
    }
}

- (void)CIGAMSymbolicUsingTableViewEstimatedHeightMakeWarning {
    CIGAMLog(@"UITableView (CIGAM)", @"当开启了 UITableView 的 estimatedRow(SectionHeader / SectionFooter)Height 功能后，不应该手动修改 contentOffset 和 contentSize，也会影响 contentSize、sizeThatFits:、rectForXxx 等方法的计算，请注意确认当前是否存在不合理的业务代码。可添加 '%@' 的 Symbolic Breakpoint 以捕捉此类信息\n%@", NSStringFromSelector(_cmd), [NSThread callStackSymbols]);
}

- (void)cigam_performBatchUpdates:(void (NS_NOESCAPE ^ _Nullable)(void))updates completion:(void (^ _Nullable)(BOOL finished))completion {
    if (@available(iOS 11.0, *)) {
        ...
        if (!updates && completion) {
            ...
            [self cigam_performSelector:NSSelectorFromString([NSString stringWithFormat:@"_%@BatchUpdates:%@:", @"perform", @"completion"]) withArguments:&updates, &completion, nil];
        }
    }
}

- (CGRect)cigam_indexFrame {
    CGRect indexFrame = CGRectZero;
    [self cigam_performSelector:NSSelectorFromString(@"indexFrame") withPrimitiveReturnValue:&indexFrame];
    return indexFrame;
}

@end

@interface UITableViewCell (CIGAM_Private)

@property(nonatomic, assign, readwrite) CIGAMTableViewCellPosition cigam_cellPosition;
@end

const UITableViewStyle CIGAMTableViewStyleInsetGrouped = UITableViewStyleGrouped + 1;

@implementation UITableView (CIGAM_InsetGrouped)
    ...
    dispatch_once(&onceToken, ^{
        ...
        OverrideImplementation([UITableView class], NSSelectorFromString(@"_configureCellForDisplay:forIndexPath:"), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^(UITableView *selfObject, UITableViewCell *cell, NSIndexPath *indexPath) {
                ...
                // UITableViewCell(CIGAM) 内会根据 cellPosition 调整 separator 的布局，所以先在这里赋值以供那边使用
                CIGAMTableViewCellPosition position = [selfObject cigam_positionForRowAtIndexPath:indexPath];
                cell.cigam_cellPosition = position;
                
                if (selfObject.cigam_style == CIGAMTableViewStyleInsetGrouped) {
                    CIGAMCornerMask mask = CIGAMLayerAllCorner;
                    CGFloat cornerRadius = selfObject.cigam_insetGroupedCornerRadius;
                    switch (position) {
                        case CIGAMTableViewCellPositionFirstInSection:
                            mask = CIGAMLayerMinXMinYCorner|CIGAMLayerMaxXMinYCorner;
                            break;
                        case CIGAMTableViewCellPositionLastInSection:
                            mask = CIGAMLayerMinXMaxYCorner|CIGAMLayerMaxXMaxYCorner;
                            break;
                        case CIGAMTableViewCellPositionMiddleInSection:
                        case CIGAMTableViewCellPositionNone:
                            cornerRadius = 0;
                            break;
                        default:
                            break;
                    ...
                    if (@available(iOS 13.0, *)) {
                        ...
                        cell.layer.cigam_maskedCorners = mask;
                        cell.layer.masksToBounds = YES;
                    }
                    cell.layer.cornerRadius = cornerRadius;
                }
                
                if (cell.cigam_configureStyleBlock) {
                    cell.cigam_configureStyleBlock(selfObject, cell, indexPath);
                }
            };
        });
        ...
        if (@available(iOS 13.0, *)) {
            OverrideImplementation([UITableView class], @selector(layoutMargins), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
                return ^UIEdgeInsets(UITableView *selfObject) {
                    ...
                    if (selfObject.cigam_style == CIGAMTableViewStyleInsetGrouped) {
                        result.left = selfObject.cigam_safeAreaInsets.left + selfObject.cigam_insetGroupedHorizontalInset;
                        result.right = selfObject.cigam_safeAreaInsets.right + selfObject.cigam_insetGroupedHorizontalInset;
                    }
...
- (void)setCigam_style:(UITableViewStyle)cigam_style {
    if (@available(iOS 13.0, *)) {
    } else {
        objc_setAssociatedObject(self, &kAssociatedObjectKey_style, @(cigam_style), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}

- (UITableViewStyle)cigam_style {
    if (@available(iOS 13.0, *)) {
        return self.style;
    }
...
- (void)setCigam_insetGroupedCornerRadius:(CGFloat)cigam_insetGroupedCornerRadius {
    objc_setAssociatedObject(self, &kAssociatedObjectKey_insetGroupedCornerRadius, @(cigam_insetGroupedCornerRadius), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.cigam_style == CIGAMTableViewStyleInsetGrouped && self.indexPathsForVisibleRows.count) {
        [self reloadData];
    }
}

- (CGFloat)cigam_insetGroupedCornerRadius {
    ...
    return associatedValue.cigam_CGFloatValue;
}

static char kAssociatedObjectKey_insetGroupedHorizontalInset;
- (void)setCigam_insetGroupedHorizontalInset:(CGFloat)cigam_insetGroupedHorizontalInset {
    objc_setAssociatedObject(self, &kAssociatedObjectKey_insetGroupedHorizontalInset, @(cigam_insetGroupedHorizontalInset), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.cigam_style == CIGAMTableViewStyleInsetGrouped && self.indexPathsForVisibleRows.count) {
        [self reloadData];
    }
}

- (CGFloat)cigam_insetGroupedHorizontalInset {
