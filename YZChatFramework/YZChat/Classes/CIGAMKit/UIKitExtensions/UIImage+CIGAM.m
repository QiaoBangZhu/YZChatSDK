        ...
        OverrideImplementation([UIImage class], @selector(resizableImageWithCapInsets:resizingMode:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP (^originalIMPProvider)(void)) {
            return ^UIImage *(UIImage *selfObject, UIEdgeInsets capInsets, UIImageResizingMode resizingMode) {
                ...
                if (!CGSizeIsEmpty(selfObject.size) && (UIEdgeInsetsGetHorizontalValue(capInsets) >= selfObject.size.width || UIEdgeInsetsGetVerticalValue(capInsets) >= selfObject.size.height)) {
                    ...
                    CIGAMLogWarn(@"UIImage (CIGAM)", @"UIImage (CIGAM) resizableImageWithCapInsets 传进来的 capInsets 的水平/垂直方向的和应该小于图片本身的大小，否则会导致 render 时出现 invalid context 0x0 的错误。");
                }
...
+ (UIImage *)cigam_imageWithSize:(CGSize)size opaque:(BOOL)opaque scale:(CGFloat)scale actions:(void (^)(CGContextRef contextRef))actionBlock {
...
- (BOOL)cigam_resizable {
    BOOL result;
    [self cigam_performSelector:NSSelectorFromString(@"_isResizable") withPrimitiveReturnValue:&result];
    return result;
}

- (CGSize)cigam_sizeInPixel {
...
- (UIImage *)cigam_grayImage {
    // CGBitmapContextCreate 是无倍数的，所以要自己换算成1倍
    CGSize size = self.cigam_sizeInPixel;
    ...
    if (self.cigam_opaque) {
        ...
        // 用 CGBitmapContextCreateImage 方式创建出来的图片，CGImageAlphaInfo 总是为 CGImageAlphaInfoNone，导致 cigam_opaque 与原图不一致，所以这里再做多一步
        grayImage = [UIImage cigam_imageWithSize:grayImage.size opaque:NO scale:grayImage.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)cigam_imageWithAlpha:(CGFloat)alpha {
    return [UIImage cigam_imageWithSize:self.size opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawInRect:CGRectMakeWithSize(self.size) blendMode:kCGBlendModeNormal alpha:alpha];
    }];
}

- (UIImage *)cigam_imageWithTintColor:(UIColor *)tintColor {
...
//    }
    BOOL opaque = self.cigam_opaque ? tintColor.cigam_alpha >= 1.0 : NO;// 如果图片不透明但 tintColor 半透明，则生成的图片也应该是半透明的
    return [UIImage cigam_imageWithSize:self.size opaque:opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)cigam_imageWithBlendColor:(UIColor *)blendColor {
    UIImage *coloredImage = [self cigam_imageWithTintColor:blendColor];
...
- (UIImage *)cigam_imageWithImageAbove:(UIImage *)image atPoint:(CGPoint)point {
    return [UIImage cigam_imageWithSize:self.size opaque:self.cigam_opaque scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawInRect:CGRectMakeWithSize(self.size)];
        [image drawAtPoint:point];
    }];
}

- (UIImage *)cigam_imageWithSpacingExtensionInsets:(UIEdgeInsets)extension {
    ...
    return [UIImage cigam_imageWithSize:contextSize opaque:self.cigam_opaque scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawAtPoint:CGPointMake(extension.left, extension.top)];
    }];
}

- (UIImage *)cigam_imageWithClippedRect:(CGRect)rect {
...
- (UIImage *)cigam_imageWithClippedCornerRadius:(CGFloat)cornerRadius {
    return [self cigam_imageWithClippedCornerRadius:cornerRadius scale:self.scale];
}

- (UIImage *)cigam_imageWithClippedCornerRadius:(CGFloat)cornerRadius scale:(CGFloat)scale {
    if (cornerRadius <= 0) {
        return self;
    }
    return [UIImage cigam_imageWithSize:self.size opaque:NO scale:scale actions:^(CGContextRef contextRef) {
        [[UIBezierPath bezierPathWithRoundedRect:CGRectMakeWithSize(self.size) cornerRadius:cornerRadius] addClip];
        [self drawInRect:CGRectMakeWithSize(self.size)];
    }];
}

- (UIImage *)cigam_imageResizedInLimitedSize:(CGSize)size {
    return [self cigam_imageResizedInLimitedSize:size resizingMode:CIGAMImageResizingModeScaleAspectFit];
}

- (UIImage *)cigam_imageResizedInLimitedSize:(CGSize)size resizingMode:(CIGAMImageResizingMode)resizingMode {
    return [self cigam_imageResizedInLimitedSize:size resizingMode:resizingMode scale:self.scale];
}

- (UIImage *)cigam_imageResizedInLimitedSize:(CGSize)size resizingMode:(CIGAMImageResizingMode)resizingMode scale:(CGFloat)scale {
    ...
    if (resizingMode >= CIGAMImageResizingModeScaleAspectFit && resizingMode <= CIGAMImageResizingModeScaleAspectFillBottom) {
        CGFloat horizontalRatio = size.width / imageSize.width;
        CGFloat verticalRatio = size.height / imageSize.height;
        CGFloat ratio = 0;
        if (resizingMode >= CIGAMImageResizingModeScaleAspectFill && resizingMode < (CIGAMImageResizingModeScaleAspectFill + 10)) {
            ratio = MAX(horizontalRatio, verticalRatio);
        } else {
            // 默认按 CIGAMImageResizingModeScaleAspectFit
        ...
        // 默认按照 CIGAMImageResizingModeScaleToFill
        drawingRect = CGRectMakeWithSize(size);
        contextSize = size;
    }
    
    return [UIImage cigam_imageWithSize:contextSize opaque:self.cigam_opaque scale:scale actions:^(CGContextRef contextRef) {
        [self drawInRect:drawingRect];
    }];
}

- (UIImage *)cigam_imageWithOrientation:(UIImageOrientation)orientation {
    ...
    return [UIImage cigam_imageWithSize:contextSize opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)cigam_imageWithBorderColor:(UIColor *)borderColor path:(UIBezierPath *)path {
    if (!borderColor) {
        return self;
    }
...
- (UIImage *)cigam_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius {
    return [self cigam_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:cornerRadius dashedLengths:0];
}

- (UIImage *)cigam_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius dashedLengths:(const CGFloat *)dashedLengths {
    ...
    return [self cigam_imageWithBorderColor:borderColor path:path];
}

- (UIImage *)cigam_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth borderPosition:(CIGAMImageBorderPosition)borderPosition {
    if (borderPosition == CIGAMImageBorderPositionAll) {
        return [self cigam_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((CIGAMImageBorderPositionBottom & borderPosition) == CIGAMImageBorderPositionBottom) {
        ...
        if ((CIGAMImageBorderPositionTop & borderPosition) == CIGAMImageBorderPositionTop) {
            [path moveToPoint:CGPointMake(0, borderWidth / 2)];
            [path addLineToPoint:CGPointMake(self.size.width, borderWidth / 2)];
        }
        if ((CIGAMImageBorderPositionLeft & borderPosition) == CIGAMImageBorderPositionLeft) {
            [path moveToPoint:CGPointMake(borderWidth / 2, 0)];
            [path addLineToPoint:CGPointMake(borderWidth / 2, self.size.height)];
        }
        if ((CIGAMImageBorderPositionRight & borderPosition) == CIGAMImageBorderPositionRight) {
...
- (UIImage *)cigam_imageWithMaskImage:(UIImage *)maskImage usingMaskImageMode:(BOOL)usingMaskImageMode {
...
+ (UIImage *)cigam_animatedImageWithData:(NSData *)data {
    return [self cigam_animatedImageWithData:data scale:1];
}

+ (UIImage *)cigam_animatedImageWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self cigam_frameDurationAtIndex:i source:source];
...
+ (float)cigam_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)cigam_animatedImageNamed:(NSString *)name {
    return [UIImage cigam_animatedImageNamed:name scale:1];
}

+ (UIImage *)cigam_animatedImageNamed:(NSString *)name scale:(CGFloat)scale {
    ...
    return [UIImage cigam_animatedImageWithData:data scale:scale];
}

+ (UIImage *)cigam_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size path:(UIBezierPath *)path addClip:(BOOL)addClip {
    size = CGSizeFlatted(size);
    return [UIImage cigam_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)cigam_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth cornerRadius:(CGFloat)cornerRadius {
    ...
    return [UIImage cigam_imageWithStrokeColor:strokeColor size:size path:path addClip:NO];
}

+ (UIImage *)cigam_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth borderPosition:(CIGAMImageBorderPosition)borderPosition {
    CGContextInspectSize(size);
    if (borderPosition == CIGAMImageBorderPositionAll) {
        return [UIImage cigam_imageWithStrokeColor:strokeColor size:size lineWidth:lineWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath* path = [UIBezierPath bezierPath];
        if ((CIGAMImageBorderPositionBottom & borderPosition) == CIGAMImageBorderPositionBottom) {
...
+ (UIImage *)cigam_imageWithColor:(UIColor *)color {
    return [UIImage cigam_imageWithColor:color size:CGSizeMake(4, 4) cornerRadius:0];
}

+ (UIImage *)cigam_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadius:(CGFloat)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    
    color = color ? color : UIColorClear;
	BOOL opaque = (cornerRadius == 0.0 && [color cigam_alpha] == 1.0);
    return [UIImage cigam_imageWithSize:size opaque:opaque scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)cigam_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadiusArray:(NSArray<NSNumber *> *)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage cigam_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        UIBezierPath *path = [UIBezierPath cigam_bezierPathWithRoundedRect:CGRectMakeWithSize(size) cornerRadiusArray:cornerRadius lineWidth:0];
        [path addClip];
        [path fill];
    }];
}

+ (UIImage *)cigam_imageWithShape:(CIGAMImageShape)shape size:(CGSize)size lineWidth:(CGFloat)lineWidth tintColor:(UIColor *)tintColor {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage cigam_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        switch (shape) {
            ...
            case CIGAMImageShapeNavBack: {
                ...
                break;
            case CIGAMImageShapeDisclosureIndicator: {
                drawByStroke = YES;
                path = [UIBezierPath bezierPath];
                path.lineWidth = lineWidth;
        ...
        if (shape == CIGAMImageShapeDetailButtonImage) {
...
+ (UIImage *)cigam_imageWithShape:(CIGAMImageShape)shape size:(CGSize)size tintColor:(UIColor *)tintColor {
    CGFloat lineWidth = 0;
    switch (shape) {
        case CIGAMImageShapeNavBack:
            lineWidth = 2.0f;
    ...
    return [UIImage cigam_imageWithShape:shape size:size lineWidth:lineWidth tintColor:tintColor];
}

+ (UIImage *)cigam_imageWithAttributedString:(NSAttributedString *)attributedString {
    ...
    return [UIImage cigam_imageWithSize:stringSize opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [attributedString drawInRect:CGRectMakeWithSize(stringSize)];
    }];
}

+ (UIImage *)cigam_imageWithView:(UIView *)view {
    CGContextInspectSize(view.bounds.size);
    return [UIImage cigam_imageWithSize:view.bounds.size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [view.layer renderInContext:contextRef];
    }];
}

+ (UIImage *)cigam_imageWithView:(UIView *)view afterScreenUpdates:(BOOL)afterUpdates {
