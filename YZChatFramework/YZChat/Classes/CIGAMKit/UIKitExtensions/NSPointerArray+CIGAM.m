...
//
//  NSPointerArray+CIGAM.m
//  CIGAMKit
//
//  Created by CIGAM Team on 2018/4/12.
//

#import "NSPointerArray+CIGAM.h"
#import "CIGAMCore.h"

@implementation NSPointerArray (CIGAM)
...
- (NSUInteger)cigam_indexOfPointer:(nullable void *)pointer {
    if (!pointer) {
        return NSNotFound;
    }
...
- (BOOL)cigam_containsPointer:(void *)pointer {
    if (!pointer) {
        return NO;
    }
    if ([self cigam_indexOfPointer:pointer] != NSNotFound) {
        return YES;
    }
    return NO;
}
