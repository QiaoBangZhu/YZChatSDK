...
@interface UIScrollView (CIGAM)

/// 判断UIScrollView是否已经处于顶部（当UIScrollView内容不够多不可滚动时，也认为是在顶部）
@property(nonatomic, assign, readonly) BOOL cigam_alreadyAtTop;

/// 判断UIScrollView是否已经处于底部（当UIScrollView内容不够多不可滚动时，也认为是在底部）
@property(nonatomic, assign, readonly) BOOL cigam_alreadyAtBottom;

/// UIScrollView 的真正 inset，在 iOS11 以后需要用到 adjustedContentInset 而在 iOS11 以前只需要用 contentInset
@property(nonatomic, assign, readonly) UIEdgeInsets cigam_contentInset;

/**
 UIScrollView 默认的 contentInset，会自动将 contentInset 和 scrollIndicatorInsets 都设置为这个值并且调用一次 cigam_scrollToTopUponContentInsetTopChange 设置默认的 contentOffset，一般用于 UIScrollViewContentInsetAdjustmentNever 的列表。
 @warning 如果 scrollView 被添加到某个 viewController 上，则只有在 viewController viewDidAppear 之前（不包含 viewDidAppear）设置这个属性才会自动滚到顶部，如果在 viewDidAppear 之后才添加到 viewController 上，则只有第一次设置 cigam_initialContentInset 时才会滚动到顶部。这样做的目的是为了避免在 scrollView 已经显示出来并滚动到列表中间后，由于某些原因，contentInset 发生了中间值的变动（也即一开始是正确的值，中间变成错误的值，再变回正确的值），此时列表会突然跳到顶部的问题。
 */
@property(nonatomic, assign) UIEdgeInsets cigam_initialContentInset;
 ...
 * @param force 是否无视[self cigam_canScroll]而强制滚动
 * @param animated 是否用动画表现
 */
- (void)cigam_scrollToTopForce:(BOOL)force animated:(BOOL)animated;

/**
 * 等同于[self cigam_scrollToTopForce:NO animated:animated]
 */
- (void)cigam_scrollToTopAnimated:(BOOL)animated;

/// 等同于[self cigam_scrollToTopAnimated:NO]
 ...
 * @param animated 是否用动画表现
 * @see [UIScrollView cigam_canScroll]
 */
- (void)cigam_scrollToBottomAnimated:(BOOL)animated;

/// 等同于[self cigam_scrollToBottomAnimated:NO]
