...
@implementation UIButton (CIGAM)

CIGAMSynthesizeIdStrongProperty(qbt_titleAttributes, setQbt_titleAttributes)
CIGAMSynthesizeIdStrongProperty(qbt_customizeButtonPropDict, setQbt_customizeButtonPropDict)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        ExtendImplementationOfVoidMethodWithTwoArguments([UIButton class], @selector(setTitle:forState:), NSString *, UIControlState, ^(UIButton *selfObject, NSString *title, UIControlState state) {
            [selfObject _markCIGAMCustomizeType:CIGAMCustomizeButtonPropTypeTitle forState:state value:title];
        ...
        ExtendImplementationOfVoidMethodWithTwoArguments([UIButton class], @selector(setTitleColor:forState:), UIColor *, UIControlState, ^(UIButton *selfObject, UIColor *color, UIControlState state) {
            [selfObject _markCIGAMCustomizeType:CIGAMCustomizeButtonPropTypeTitleColor forState:state value:color];
            ...
            if (attributes) {
                ...
                [selfObject cigam_setTitleAttributes:[NSDictionary dictionaryWithDictionary:newAttributes] forState:state];
            }
        });
        ...
        ExtendImplementationOfVoidMethodWithTwoArguments([UIButton class], @selector(setTitleShadowColor:forState:), UIColor *, UIControlState, ^(UIButton *selfObject, UIColor *color, UIControlState state) {
            [selfObject _markCIGAMCustomizeType:CIGAMCustomizeButtonPropTypeTitleShadowColor forState:state value:color];
        });
        ...
        ExtendImplementationOfVoidMethodWithTwoArguments([UIButton class], @selector(setImage:forState:), UIImage *, UIControlState, ^(UIButton *selfObject, UIImage *image, UIControlState state) {
            [selfObject _markCIGAMCustomizeType:CIGAMCustomizeButtonPropTypeImage forState:state value:image];
        });
        ...
        ExtendImplementationOfVoidMethodWithTwoArguments([UIButton class], @selector(setBackgroundImage:forState:), UIImage *, UIControlState, ^(UIButton *selfObject, UIImage *image, UIControlState state) {
            [selfObject _markCIGAMCustomizeType:CIGAMCustomizeButtonPropTypeBackgroundImage forState:state value:image];
        });
        ...
        ExtendImplementationOfVoidMethodWithTwoArguments([UIButton class], @selector(setAttributedTitle:forState:), NSAttributedString *, UIControlState, ^(UIButton *selfObject, NSAttributedString *title, UIControlState state) {
            [selfObject _markCIGAMCustomizeType:CIGAMCustomizeButtonPropTypeAttributedTitle forState:state value:title];
        });
        
        if (@available(iOS 13, *)) {
            ExtendImplementationOfVoidMethodWithoutArguments([UIButton class], @selector(layoutSubviews), ^(UIButton *selfObject) {
                // 临时解决 iOS 13 开启了粗体文本（Bold Text）导致 UIButton Title 显示不完整 https://github.com/Tencent/CIGAM_iOS/issues/620
...
- (instancetype)cigam_initWithImage:(UIImage *)image title:(NSString *)title {
...
- (BOOL)cigam_hasCustomizedButtonPropForState:(UIControlState)state {
...
- (BOOL)cigam_hasCustomizedButtonPropWithType:(CIGAMCustomizeButtonPropType)type forState:(UIControlState)state {
...
- (void)cigam_setTitleAttributes:(NSDictionary<NSAttributedStringKey,id> *)attributes forState:(UIControlState)state {
    if (!attributes) {
    ...
    if (self.qbt_titleAttributes.count && !self.qbt_titleAttributes[@(UIControlStateNormal)]) {
        [self cigam_setTitleAttributes:@{} forState:UIControlStateNormal];
    }
}
...
- (void)_markCIGAMCustomizeType:(CIGAMCustomizeButtonPropType)type forState:(UIControlState)state value:(id)value {
    if (value) {
        [self _setCIGAMCustomizeType:type forState:state];
    } else {
        [self _removeCIGAMCustomizeType:type forState:state];
    }
}

- (void)_setCIGAMCustomizeType:(CIGAMCustomizeButtonPropType)type forState:(UIControlState)state {
    if (!self.qbt_customizeButtonPropDict) {
...
- (void)_removeCIGAMCustomizeType:(CIGAMCustomizeButtonPropType)type forState:(UIControlState)state {
    if (!self.qbt_customizeButtonPropDict || !self.qbt_customizeButtonPropDict[@(state)]) {
        return;
    }
