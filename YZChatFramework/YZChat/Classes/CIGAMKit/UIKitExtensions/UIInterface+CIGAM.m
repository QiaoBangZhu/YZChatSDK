...
//  CIGAMKit
//
//  Created by CIGAM Team on 2018/12/20.
//

#import "UIInterface+CIGAM.h"
#import "CIGAMCore.h"

@implementation CIGAMHelper (CIGAM_Interface)

CIGAMSynthesizeNSIntegerProperty(orientationBeforeChangingByHelper, setOrientationBeforeChangingByHelper)

- (void)handleDeviceOrientationNotification:(NSNotification *)notification {
    // 如果是由 setValue:forKey: 方式修改方向而走到这个 notification 的话，理论上是不需要重置为 Unknown 的，但因为在 UIViewController (CIGAM) 那边会再次记录旋转前的值，所以这里就算重置也无所谓
...
+ (CGAffineTransform)transformForCurrentInterfaceOrientation {
    return [CIGAMHelper transformWithInterfaceOrientation:UIApplication.sharedApplication.statusBarOrientation];
}

+ (CGAffineTransform)transformWithInterfaceOrientation:(UIInterfaceOrientation)orientation {
    CGFloat angle = [CIGAMHelper angleForTransformWithInterfaceOrientation:orientation];
