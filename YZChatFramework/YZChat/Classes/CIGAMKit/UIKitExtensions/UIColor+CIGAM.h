...
#define UIColorMakeWithHex(hex) [UIColor cigam_colorWithHexString:hex]

NS_ASSUME_NONNULL_BEGIN

@interface UIColor (CIGAM)
 ...
 * @return UIColor对象
 */
+ (nullable UIColor *)cigam_colorWithHexString:(nullable NSString *)hexString;

/**
 *  将当前色值转换为hex字符串，通道排序是AARRGGBB（与Android保持一致）
 *  @return 色值对应的 hex 字符串，以 # 开头，例如 #00ff00ff
 */
@property(nonatomic, copy, readonly) NSString *cigam_hexString;
 ...
 */
+ (nullable UIColor *)cigam_colorWithRGBAString:(nullable NSString *)rgbaString;

/**
 将当前色值转换成“255,255,255,1.00”的字符串，如果 alpha 通道为1也会输出出来。其中 alpha 通道必定带两位小数点，其他三个通道都是整数。
 */
@property(nonatomic, copy, readonly) NSString *cigam_RGBAString;
 ...
 */
@property(nonatomic, assign, readonly) CGFloat cigam_red;
 ...
 */
@property(nonatomic, assign, readonly) CGFloat cigam_green;
 ...
 */
@property(nonatomic, assign, readonly) CGFloat cigam_blue;
 ...
 */
@property(nonatomic, assign, readonly) CGFloat cigam_alpha;

/**
 *  获取当前 UIColor 对象里的 hue（色相），注意 hue 的值是一个角度，所以0和1（0°和360°）是等价的，用 return 值去做判断时要特别注意。
 */
@property(nonatomic, assign, readonly) CGFloat cigam_hue;

/**
 *  获取当前 UIColor 对象里的 saturation（饱和度）
 */
@property(nonatomic, assign, readonly) CGFloat cigam_saturation;

/**
 *  获取当前 UIColor 对象里的 brightness（亮度）
 */
@property(nonatomic, assign, readonly) CGFloat cigam_brightness;
 ...
 */
- (nullable UIColor *)cigam_colorWithoutAlpha;

/**
 *  计算当前color叠加了alpha之后放在指定颜色的背景上的色值
 */
- (UIColor *)cigam_colorWithAlpha:(CGFloat)alpha backgroundColor:(nullable UIColor *)backgroundColor;
 ...
 */
- (UIColor *)cigam_transitionToColor:(nullable UIColor *)toColor progress:(CGFloat)progress;
 ...
 */
@property(nonatomic, assign, readonly) BOOL cigam_colorIsDark;
 ...
 *  背景：如果将一个 UIView.tintColor 设置为 nil，表示这个 view 的 tintColor 希望跟随 superview.tintColor 变化而变化，所以设置完再获取 view.tintColor，得到的并非 nil，而是 superview.tintColor 的值，而如果整棵 view 层级树里的 view 都没有设置自己的 tintColor，则会返回系统默认的 tintColor（也即 [UIColor cigam_systemTintColor]），所以才提供这个方法用于代替判断 tintColor == nil 的作用。
 */
@property(nonatomic, assign, readonly) BOOL cigam_isSystemTintColor;

/**
 *  获取当前系统的默认 tintColor 色值
 */
@property(class, nonatomic, strong, readonly) UIColor *cigam_systemTintColor;
...
extern NSString *const CIGAMCGColorOriginalColorBindKey;

@protocol CIGAMDynamicColorProtocol <NSObject>

@required

/// 获取当前 color 的实际颜色（返回的颜色必定不是 dynamic color）
@property(nonatomic, strong, readonly) UIColor *cigam_rawColor;

/// 标志当前 UIColor 对象是否为动态颜色（由 [UIColor cigam_colorWithThemeProvider:] 创建的颜色，或者 iOS 13 下由 [UIColor colorWithDynamicProvider:]、[UIColor initWithDynamicProvider:] 创建的颜色）
@property(nonatomic, assign, readonly) BOOL cigam_isDynamicColor;

/// 标志当前 UIColor 对象是否为 CIGAMThemeColor
@property(nonatomic, assign, readonly) BOOL cigam_isCIGAMDynamicColor;
...
@interface UIColor (CIGAM_DynamicColor) <CIGAMDynamicColorProtocol>
