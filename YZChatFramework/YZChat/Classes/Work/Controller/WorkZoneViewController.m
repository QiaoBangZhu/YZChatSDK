        ...
        if (!error) {
            ...
            if ([result[@"code"]intValue] == 200) {
                ...
                for (NSDictionary* dict in result[@"data"]) {
                    WorkZoneModel* model = [WorkZoneModel yy_modelWithDictionary:dict];
...
{
    WorkZoneTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"WorkZoneTableViewCell" forIndexPath:indexPath];
    if (cell == nil) {
        cell = [[WorkZoneTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"WorkZoneTableViewCell"];
    }
    cell.delegate = self;
    if ([[self data] count] > 0) {
        WorkZoneModel* model = self.data[indexPath.section];
...
- (void)fetchToolToken:(AppInfoModel *)app {
...
-(void)fetchToolWebViewUrl:(NSString *)url appInfo:(AppInfoModel *)app {
