...
//    [self addSubview:self.selectImageBtn];
    ...
    [self.tipLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.flashlightBtn.mas_bottom).offset(25);
        make.left.width.equalTo(self);
...
- (void)rcd_setFlashlightOn:(BOOL)on {
    [QRCodeManager rcd_FlashlightOn:on];
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int index = 0; index < 4; ++index) {
        ...
        [[UIColor colorWithHex:kCommonBlueTextColor] set];
...
- (UIButton *)flashlightBtn {
    if (!_flashlightBtn) {
        ...
        [_flashlightBtn setTitle:@"打开" forState:(UIControlStateNormal)];
        [_flashlightBtn setTitle:@"关闭" forState:(UIControlStateSelected)];
    }
...
- (UIButton *)selectImageBtn {
    if (!_selectImageBtn) {
        ...
        _selectImageBtn.titleLabel.font = [UIFont systemFontOfSize:13];
        _selectImageBtn.backgroundColor = [UIColor colorWithHex:0x0195ff];
        [_selectImageBtn setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
        [_selectImageBtn setTitle:@"选择图片" forState:(UIControlStateNormal)];
