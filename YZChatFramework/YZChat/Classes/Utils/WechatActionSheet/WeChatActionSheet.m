...
-(void)layoutSubviews
{
    ...
    if ([self.title isEqualToString:@""]||self.title == nil) {
        needHeight = ROWHEIGHT*[self.titlesArray count];
    }
    
    CGFloat realyHeight = MIN(maxTableHeight, needHeight);
    
    [bottomView setFrame:CGRectMake(0.0f, self.bounds.size.height-realyHeight-DISTANTHEIGHT-ROWHEIGHT-(IS_IPHONEX ? 34 : 0), self.bounds.size.width, realyHeight+DISTANTHEIGHT+ROWHEIGHT+(IS_IPHONEX ? 34 : 0))];
    
    [cancelButton setFrame:CGRectMake(0, bottomView.bounds.size.height-ROWHEIGHT - (IS_IPHONEX ? 34 : 0), bottomView.bounds.size.width, ROWHEIGHT + (IS_IPHONEX ? 34 : 0))];
   //有标题时候 设置标题frame
