...
+ (void)postData:(NSString *)urlStr params:(NSDictionary* __nullable)dict completion:(void(^)(BaseResponse* responase, NSDictionary *data, NSError*error))completion {
    ...
    if (dict != nil) {
        ...
        NSString *bodyStr = [TCUtil dictionary2JsonStr:dict];
    ...
    NSOperationQueue *queue = [[NSOperationQueue alloc]init];
    [NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *error){
        NSDictionary *dictData = nil;BaseResponse *responseObj = nil;
        if (data != nil) {
            responseObj = [BaseResponse mj_objectWithKeyValues:data];
            dictData = [TCUtil jsonData2Dictionary:data];
        }
        //在主线程操作UI对象
        dispatch_async(dispatch_get_main_queue(), ^(){
            if (completion) {
...
//    [params setValue:@"files" forKey:@"files"];
    [manager POST:urlStr parameters:dict headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        NSDictionary *dictData = [TCUtil jsonData2Dictionary:responseObject];
               NSLog(@"123:%@",dictData);
               dispatch_async(dispatch_get_main_queue(), ^(){
                   if (completion) {
