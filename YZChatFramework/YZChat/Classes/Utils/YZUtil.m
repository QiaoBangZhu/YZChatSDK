...
+ (void)asyncSendHttpRequest:(NSString*)command token:(NSString*)token params:(NSDictionary*)params handler:(void (^)(int resultCode, NSString* message, NSDictionary* resultDict))handler
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSData* data = [YZUtil dictionary2JsonData:params];
        if (data == nil)
        {
        ...
        NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            ...
            else
            {
                ...
                NSDictionary* resultDict = [YZUtil jsonSring2Dictionary:responseString];
                int errCode = -1;
                NSString* message = @"";
...
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSData* data = [YZUtil dictionary2JsonData:param];
        if (data == nil)
        {
            NSLog(@"sendHttpRequest failed，参数转成json格式失败");
            dispatch_async(dispatch_get_main_queue(), ^{
