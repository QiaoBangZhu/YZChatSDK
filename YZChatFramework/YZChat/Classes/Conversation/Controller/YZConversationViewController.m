...
- (void)setupView {
    ...
    if (self.isNeedCloseBtn) {
        UIBarButtonItem* closeItem = [[UIBarButtonItem alloc] initWithImage:YZChatResource(@"icon_back") target:self action:@selector(closeAction)];
...
- (void) onChangeUnReadCount:(NSNotification *)notifi{
    ...
    TUITabBarItem* item = (TUITabBarItem*)[YZBaseManager shareInstance].tabController.tabBarItems[0];
...
- (void)searchAction {
    YZSearchConversationViewController* searchVc = [[YZSearchConversationViewController alloc]init];
