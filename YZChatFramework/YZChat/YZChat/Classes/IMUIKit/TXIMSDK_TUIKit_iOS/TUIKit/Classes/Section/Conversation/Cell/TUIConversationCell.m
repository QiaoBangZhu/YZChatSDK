...
- (void)layoutSubviews
{
    ...
    self.headImageView.mm_width(imgHeight).mm_height(imgHeight).mm_left(TConversationCell_Margin + 4).mm_top(TConversationCell_Margin);
    if ([TUIKit sharedInstance].config.avatarType == TAvatarTypeRounded) {
        self.headImageView.layer.masksToBounds = YES;
    ...
    self.timeLabel.mm_sizeToFit().mm_top(TConversationCell_Margin_Text).mm_right(TConversationCell_Margin + 14);
    self.titleLabel.mm_sizeToFitThan(120, 22).mm_top(TConversationCell_Margin_Text + 3).mm_left(self.headImageView.mm_maxX+TConversationCell_Margin + 1);
    self.unReadView.mm_right(TConversationCell_Margin + 4).mm_bottom(TConversationCell_Margin - 1);
    self.subTitleLabel.mm_sizeToFit().mm_left(self.titleLabel.mm_x).mm_bottom(TConversationCell_Margin_Text + 3).mm_flexToRight(self.mm_w-self.unReadView.mm_x);
}
