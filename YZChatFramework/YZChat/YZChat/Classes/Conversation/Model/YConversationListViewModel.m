...
typedef NS_ENUM(NSInteger, GropMsgType)
{
    GropMsgTypeGroupRecycled = 1,//群已经回收
    GropMsgTypeKickOff = 2,//被踢出群
    GropMsgTypeLeaveFromGroup = 3,//退出群
    GropMsgTypeDismissGroup = 4,//解散群
};

@interface YConversationListViewModel ()
@property BOOL isLoadFinished;
@property BOOL isLoading;
@property (nonatomic, strong)NSMutableArray *localConvList;
@property (nonatomic, assign)GropMsgType type;
@end

@implementation YConversationListViewModel
...
- (void)onGroupDismiss:(NSNotification *)no
{
    ...
    if (data) {
//        [self requestDismissGroupMsgTips:data];
        [self requestGroupMsgTisWithType:GropMsgTypeDismissGroup data:data];
    }
}
...
//        [self removeData:data];
        [self requestGroupMsgTisWithType:GropMsgTypeGroupRecycled data:data];
    }
}
...
//        [self removeData:data];
        [self requestGroupMsgTisWithType:GropMsgTypeKickOff data:data];
    }
}
...
//        [self removeData:data];
        [self requestGroupMsgTisWithType:GropMsgTypeLeaveFromGroup data:data];
    }
}
...
- (void)requestGroupMsgTisWithType:(GropMsgType)type data:(TUIConversationCellData *)data {
   @weakify(self);
   [YChatNetworkEngine requestFetchGroupMsgWithGroupIdList:@[data.groupID] completion:^(NSDictionary *result, NSError *error) {
    @strongify(self);
       if (!error) {
           if ([result[@"code"]intValue] == 200) {
               if (type == GropMsgTypeLeaveFromGroup) {
                   NSArray* dicArr = result[@"data"][@"GroupInfo"];
                   if ([dicArr count] > 0) {
...
- (void)showTipsWithType:(GropMsgType)type tips:(NSString *)tips data:(TUIConversationCellData *)data {
    NSString* info = @"";
    if (type ==  GropMsgTypeGroupRecycled) {
        info = [NSString stringWithFormat:@"%@ 群已回收", tips];
    }else if(type == GropMsgTypeKickOff) {
        info = [NSString stringWithFormat:@"您已被踢出 %@ 群", tips];
    }else if(type == GropMsgTypeLeaveFromGroup) {
        info = [NSString stringWithFormat:@"您已退出 %@ 群", tips];
    }else if(type == GropMsgTypeDismissGroup) {
        info = [NSString stringWithFormat:@"%@ 群已解散",tips];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"POPTOCONVERSIONLIISTNOTIFY" object:nil];
    }
    [THelper makeToast:info];
    [self removeData:data];
}

- (void)requestDismissGroupMsgTips:(TUIConversationCellData*)data {
     @weakify(self);
    [YChatNetworkEngine requestFetchGroupMsgWithGroupIdList:@[data.groupID] completion:^(NSDictionary *result, NSError *error) {
     @strongify(self);
        if (!error) {
            if ([result[@"code"]intValue] == 200) {
                [THelper makeToast:[NSString stringWithFormat:@"%@ 群已解散", result[@"data"][@"Name"]]];
                [self removeData:data];
            }
        }
    }];
