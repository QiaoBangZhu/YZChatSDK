...
@implementation YChatHTTPClient

+ (instancetype)sharedClient {
    static YChatHTTPClient *_sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSURLSessionConfiguration *sessionConfig = [NSURLSessionConfiguration defaultSessionConfiguration];
         sessionConfig.timeoutIntervalForRequest = YChat_Gloabal_Timeout;
        _sharedClient = [[YChatHTTPClient alloc] initWithSessionConfiguration:sessionConfig];
        ...
        //Set Header value
//        [_sharedClient.requestSerializer setValue:[YChatSettingStore sharedInstance].getAuthToken forHTTPHeaderField:@"token"];
//        [_sharedClient.requestSerializer setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
        
        // 嵌入HTTPS证书
//        if (kPublish) {
//            [_sharedClient prepareHTTPS];
//        }
    });
